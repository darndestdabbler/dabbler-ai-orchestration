...
export function cancel(
  ...
  try {
    ...
    // A forced cancel is a person's verb. An engine that uses it -- the
    // proof of 2026-09-08 saw one cancel its own registration and drive
    // another folder's session from the wrong window -- is recorded as a
    // decision a person owns, never refused: the cancel stands, and the
    // person says whether it should have.
    if (options.force === true && options.engine === true) {
      try {
        raiseOwed(repoRootFromSessionsDir(sessionsDir), {
          id: `forced-cancel/s${sessionNumber}`,
          decisionClass: CLASS_ACCOUNTABILITY_SIGNOFF,
          question:
            `Session ${sessionDisplayNumber(sessionNumber)} was cancelled with --force by the engine ` +
            `(${options.reason.trim() || "no reason given"}). Does that stand?`,
          determined:
            "`session cancel --force` is a person's verb: it ends a session in flight and erases " +
            "nothing, but the judgement that the session should end is not the engine's to make. " +
            "The cancellation is recorded and stands until a person says otherwise.",
          options: [
            { label: "It stands", consequence: "The session stays cancelled; the next start registers the next one." },
            {
              label: "Restore it",
              consequence: `\`dabbler session restore ${sessionNumber}\` puts the session back as it was.`,
            },
          ],
          recommendation: "It stands",
          confidence: "medium",
          onNoAnswer: "The cancellation stands.",
          sessionNumber,
        });
      } catch {
        // Deliberately silent: the cancel is written, and a brief that
        // could not be must not unwrite it.
      }
    }
