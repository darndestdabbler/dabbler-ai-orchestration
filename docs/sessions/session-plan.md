...
5. Two honest green events, and no third: **progress resumed**, emitted only
   ...
   itself.
6. **Amended 2026-09-05, on the operator's decision, from two defects session
   91 hit in its own conduct.** Both are the same shape as the rendering this
   session is already about: the framework not noticing what it is not doing.
   1. **A `wait` past its due time stops the turn.** `hookStop` blocks only
   ...
   2. **A finished job nobody collected is an attention row.** Nothing about
      ...
      Explorer say so, through the renderer item 2 builds.
   3. **And the prose half, in the template rather than by hand.** The
      managed body already said "It is a call you make later, never a sleep
      ...
      inside the fence.
   4. **A suite that covers source and is not `expensive` is refused.**
