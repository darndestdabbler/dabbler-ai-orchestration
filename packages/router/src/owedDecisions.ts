...
/** The suites session `session` recorded as owed to another module's session, by name, while the question stands. */
export function suitesOwedElsewhere(repoRoot: string, session: number): Set<string> {
  ...
  for (const row of openDecisions(repoRoot)) {
    const id = String(row["id"] ?? "");
    if (id.startsWith(prefix)) owed.add(id.slice(prefix.length));
  }
  return owed;
}
