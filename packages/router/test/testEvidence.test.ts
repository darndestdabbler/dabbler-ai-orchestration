...
describe("the run record", () => {
  ...
  it("records that the selector ran and chose nothing without claiming a run, and refuses that as a run of record", () => {
    ...
    // A targeted run of record that selected nothing is recorded with no command, and owes no whole run before a release.
    assert.throws(
    ...
    assert.deepEqual(wholeRunsOwedBeforeRelease(readRecords(root), 4), []);
  });
