...
/** The one way on a Start offers where the Configuration cannot start a session by itself. */
export const START_WITH_DIFFERENT_MODELS = "Start with Different Models...";
...
export function configuredStart(repoRoot: string): { picked: EngineChoice; model: string } | string {
  ...
  if (picked === undefined) {
    return (
      "This repository's Configuration names no engine to start a session with. Choose one on the " +
      "Authoring AI's Vehicle row in the Configuration, or start this one session with different models."
    );
  }
  const model = chosenAuthoringModel(repoRoot);
  if (picked.modelRequired && model === "") {
    return (
      `${picked.label} needs a model and this repository's Configuration names none. Choose one on the ` +
      "Authoring AI's Model row in the Configuration, or start this one session with different models."
    );
  }
  return { picked, model };
}
...
export async function runStartSession(
  ...
  let configured = ask ? null : (ui.configured ?? configuredStart)(repository.root);
  if (typeof configured === "string") {
    // One click forward from a Configuration that cannot start a session yet.
    const chose = await ui.choose(configured, [START_WITH_DIFFERENT_MODELS]);
    if (chose !== START_WITH_DIFFERENT_MODELS) return false;
    configured = null;
  }
