...
describe("the start boundary, over a configuration that was typed into", () => {
  ...
  it("measures the reviewers against the model THIS CALL names, not the one on disk", () => {
    // Round 1's second blocking finding. A `--model` typed at `session start`
    // is the author of the session about to begin, and nothing on disk says
    // so until it has been registered -- so the reviewing roles were being
    // resolved against whatever the checkout had configured, and the one rule
    // a reviewer is held to was applied against the wrong author.
    const { root, restore } = machine();
    try {
      // The operator has chosen the openai model to review with.
      writePreferences({ role: "reviewer", selected: "gpt-5.6-terra" });
      // And this call names that same model as the AUTHOR.
      const refusal = configuredModelRefusal(root, "gpt-5.6-terra", "copilot");
      assert.match(String(refusal), /Primary Reviewer/);
      assert.match(String(refusal), /gpt-5\.6-terra/);
      // A different author leaves the same selection perfectly good.
      assert.equal(configuredModelRefusal(root, "claude-opus-5", "copilot"), null);
    } finally {
      writePreferences({ role: "reviewer", selected: "" });
      restore();
    }
...
describe("whose a choice is, as the configuration and `explain` say it", () => {
  it("names this checkout where it chose, the machine's default it shadows, and the default alone elsewhere", () => {
    ...
    try {
      ...
      // One closed word beside each sentence, for EVERY choice, so a surface
      // can behave differently for this repository's and this machine's
      // without reading a sentence for a word.
      assert.equal(elsewhere["selectedLayer"], "machine");
      assert.equal(here["selectedLayer"], "checkout");
      assert.equal(node(configuration["engines"])["decidedLayer"], "checkout");
      assert.equal(node(configuration["auxiliaryReviewer"])["selectedLayer"], "default");
      ...
      assert.equal(node(configuration["authoring"])["selectedLayer"], "default");
      writePreferences({ authoringModel: "claude-opus-5" });
      assert.equal(node(configurationNode(root)["authoring"])["selectedLayer"], "machine");
      writeSettings(root, { [SETTING_AUTHORING_MODEL]: "claude-opus-5" });
      const authoring = node(configurationNode(root)["authoring"]);
      assert.equal(authoring["selectedLayer"], "checkout");
