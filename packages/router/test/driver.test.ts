...
import { ENGINE_MARKERS, reviewingVehicleRefusal } from "../src/session.ts";
  ...
  it("prints no command its own verb would refuse as usage, and the cancel it prints runs as printed", async () => {
    ...
    try {
      ...
      const ran = await capture(() =>
        Promise.resolve(sessionVerb([...shlexSplit(cancelCommand).slice(2), "--sessions-dir", sessionsDir])),
      );
