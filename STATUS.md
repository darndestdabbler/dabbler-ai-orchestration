...
>
> **Owed, deliberately not fixed.** Claude Code is not told its background command died until everything that inherited the command's output has exited; the framework's jobs inherit it, so a command killed under a suite is noticed when the suite ends (12–40 s here, the length of the suite in general). A job should not hold its parent's streams. No case failed for it; the soaks will show its cost. Also seen and left: a message sent with `session interrupt` after the LAST step's instruction has no instruction left to travel with.
>
