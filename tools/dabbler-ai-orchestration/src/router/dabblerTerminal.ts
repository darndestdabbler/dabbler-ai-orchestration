...
/**
 * One *Dabbler* terminal per repository, however many times it is asked
 * for, and whether it is sitting beside a CLI yet.
 *
 * `split` is not decoration: a terminal's location is fixed when it is
 * created, so a terminal made before there was any CLI to sit beside can
 * only ever be a tab of its own. Remembering which kind this one is is
 * what lets Start replace it with the arrangement it promised.
 */
const open = new Map<
  string,
  { terminal: vscode.Terminal; pty: DabblerTerminal; split: boolean }
>();
...
function build(repoRoot: string, beside?: vscode.Terminal): void {
  ...
  open.set(repoRoot, { terminal, pty, split: beside !== undefined });
}
 ...
 *
 * **A terminal that is not beside anything is replaced rather than shown.**
 * Activation creates one before there is any CLI, so on the ordinary path
 * -- an existing repository open when the window starts, then Start -- the
 * cached terminal predates the thing it is meant to sit next to, and
 * showing it produces a separate tab and no arrangement at all. VS Code
 * fixes a terminal's location at creation, so the only way to honour the
 * promise is to build the one that was asked for. What is lost is the
 * scrollback of a terminal nobody had looked at; the new one replays every
 * job log from the start, because it reads them from disk.
 *
...
export function ensureDabblerTerminal(
  ...
  } else if (beside !== undefined && !entry.split) {
    entry.pty.dispose();
