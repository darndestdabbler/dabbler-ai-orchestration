...
describe("the gate that stands in front of a verification round", () => {
  ...
  it("skips evidence only for a declared empty mapping, and blocks on a path nobody mapped even beside a mapped one", () => {
    // "Nothing is affected" and "nobody knows what is affected" look
    // identical from the selected-test list and must never be treated alike.
  ...
  it("measures a remediation by the fix rather than by the whole session", () => {
    // A repository-wide edit buys one full run, at the round that reviewed
    // it. Judging later rounds against HEAD would re-buy it every time.
  ...
  it("asks a suite the selection named no test of for nothing, and is satisfied by the run it asked for", () => {
    const twoSuites = {
      testing: {
        suites: [
