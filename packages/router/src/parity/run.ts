...
export interface ParityCase {
  ...
  /**
   * Environment both invocations get, on top of the pinned git identity.
   * The same for each side -- an env that differed would be comparing two
   * questions rather than two answers.
   */
  readonly env?: (repo: string, sessionsDir: string) => Record<string, string>;
...
export const CASES: readonly ParityCase[] = [
  {
    ...
    dabblerArgs: () => ["metrics"],
    env: (repo) => ({
      AI_ROUTER_METRICS_PATH: join(repo, ...METRICS_FIXTURE_PATH.split("/")),
    }),
    proves:
      `the whole report over ${METRICS_FIXTURE_ROWS} canned call(s), and with ` +
      "it the three-layer config load the report is computed from",
  },
];
...
function runCase(
  ...
  const left = python(
    ...
    parityCase.env?.(pythonRepo, pythonSessions),
  );
  const right = runCli(
    routers,
    typescriptRepo,
    parityCase.dabblerArgs(typescriptRepo, typescriptSessions),
    parityCase.env?.(typescriptRepo, typescriptSessions),
  );
...
function runCli(
  ...
  return runProcess(process.execPath, [routers.cliEntry, ...args], cwd, env);
}
