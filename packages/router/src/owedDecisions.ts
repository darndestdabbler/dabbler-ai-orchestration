...
/**
 * Re-ask a closing session's own accountability signoffs, in the form that
 * is still answerable.
 *
 ...
 *
 * Scoped to rows this session raised about itself: a question about the
 * repository outlives any session and is not this rule's business.
 */
export function reaskSessionScopedSignoffs(repoRoot: string, sessionNumber: number): Row[] {
  const reasked: Row[] = [];
  for (const row of foldOwed(readOwed(repoRoot)).values()) {
    ...
    const options = (Array.isArray(row["options"]) ? row["options"] : []).map((entry) => ({
      ...
      consequence:
        ...
        `(${AFTER_CLOSE_NOTE} answering settles the record and performs nothing. ` +
        "Anything this answer would have undone is now work for a later session.)",
    }));
    ...
    const next = raiseOwed(repoRoot, {
      ...
      determined:
        `${determined}\n\n${AFTER_CLOSE_NOTE} session ${sessionNumber} has closed. Its work is ` +
