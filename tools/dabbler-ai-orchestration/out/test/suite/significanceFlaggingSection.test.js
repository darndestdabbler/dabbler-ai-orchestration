...
suite("significanceFlaggingSection — rendering", () => {
    ...
    test("describes the actual annotation surface (workspace scan command, not auto-on-session-start)", () => {
        ...
        // that doesn't exist.
        const { html } = (0, significanceFlaggingSection_1.render)(baseState());
        assert.ok(html.includes("Dabbler: Scan Workspace for @dabbler:outsource-review Annotations"), "section must name the scan command verbatim");
        assert.ok(html.includes("deduplicated"), "section must mention dedup behavior");
        assert.ok(!/scans? open files at session start/i.test(html), "section must NOT promise automatic session-start scanning");
        assert.ok(!/queued automatically/i.test(html), "section must NOT imply automatic queue population");
    });
