...
function datedPinOf(requested: string, served: string): boolean {
  ...
  return /^[-@]\d{6,8}$/.test(served.slice(requested.length));
}
...
/**
 * A round's requested/served pair as an observation.
 *
 * `served_model` on a verification round is read from the provider's own
 * response, so it is the strong kind -- and it is the only kind that can
 * make a model read as `honoured`.
 */
export function roundObservations(
  rounds: readonly { readonly requested_model?: unknown; readonly served_model?: unknown }[],
): ModelObservation[] {
