...
**Reason:** Gemini was explicitly out of scope for Set 044 (see
...
approach across all backends, not just the two measured. Gemini
...
small cross-provider wrapper makes a per-provider Gemini adapter
indefensible.

**Successor:** Gemini parser, deferred follow-on to Set 045 (or
later). Add when a Dabbler workflow actually needs Gemini
observability.
