...
export function start(sessionsDir: string, options: StartOptions): number {
  ...
  try {
    ...
    //
    // `in-progress`, not `complete`. Step 1 of every session reads "Register;
    // declare", and registering is half of it -- marking it done here showed
    // the operator a finished step before the declaration it names had
    // happened, and pushed the opening bookend onto step 2. The declaration
    // completes it.
...
export function declare(sessionsDir: string, options: DeclareCliOptions): number {
  ...
  // The work has begun, so step 1 is finished and the next is in flight.
  // Nobody is asked to say either.
...
export function advanceStepsAtDeclare(
  ...
  try {
    // The step this call finishes. `start` opened it and did half of what it
    // names; the declaration is the other half.
    const open = planRows.find(
      (row) => latest.get(String(row["stepKey"])) === "in-progress",
    );
    if (open !== undefined) {
      closed = move(open, "complete");
      latest.set(closed, "complete");
    }
    const next = planRows.find(
      (row) => latest.get(String(row["stepKey"])) !== "complete",
    );
    if (next !== undefined) opened = move(next, "in-progress");
  } catch (error) {
