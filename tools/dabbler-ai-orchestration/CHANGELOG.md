...
- **The framework's own shutdown was measured and is sound.** The suspicion
  was that ending a job could quietly fail to take down everything it had
  started, leaving processes behind on your machine. It does not: fourteen
  shutdowns under heavy load all reported success and left nothing running.
  The measurement is written down, so the next person to suspect it can read
  what was tested rather than testing it again.
