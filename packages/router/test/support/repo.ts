...
/**
 * An entry older than this was left by an earlier run. The workers of ONE
 * run are separate processes, started minutes apart on a loaded machine,
 * and they share this root: a sweep of everything would take a template a
 * slower worker is still copying from. An hour is longer than any run.
 */
...
/**
 * The suite's temp root, swept once per process of what earlier runs left
 ...
 * so a normal run's leavings go at once and a backlog goes over a few runs.
 */
export function tempRoot(): string {
  ...
  for (let index = 0; index < names.length && Date.now() < deadline; index += 1) {
    const path = join(ROOT, names[(start + index) % names.length] as string);
    try {
...
function pinGit(): void {
  if (pinned) return;
  const config = join(mkdtempSync(join(tempRoot(), "git-env-")), "gitconfig");
...
export function makeRepo(files: Record<string, string>, options: { origin?: boolean } = {}): string {
  ...
  pinGit();
  const target = mkdtempSync(join(tempRoot(), "walk-"));
...
function templateFor(files: Record<string, string>, withOrigin: boolean): string {
  ...
  pinGit();
  const target = mkdtempSync(join(tempRoot(), "template-"));
