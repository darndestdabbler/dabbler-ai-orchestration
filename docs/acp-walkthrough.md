...
printed whole as `session` on the verb's `{"kind":"open"}` line — carried
`models.availableModels` — every model the seat can dispatch with a
`_meta.copilotUsage` multiplier and `currentModelId: "gpt-5-mini"` —
plus `modes` (agent, plan, autopilot) and `configOptions` (mode, model,
reasoning effort, and an `allow_all` permissions switch). The multipliers
as the seat stated them on the §2 run's `open` line:
...
The verb's 87 lines, in order, by kind (conversation
`b6c61a37-e375-4881-b439-d9ffa98e0654`):

```
open    sessionId, resumed=false, capabilities (§1), session (the session/new reply, §1)
...
usage   usage_update  used=14580 size=272000
thought ×76                            (agent_thought_chunk, a word or two each)
...
usage   usage_update  used=14873
text    "done"
turn    stopReason=end_turn  cancelled=false
        usage={inputTokens:27422,outputTokens:80,thoughtTokens:42,cachedReadTokens:25088}
```

The `tool_call` record, verbatim:

```json
{"sessionUpdate":"tool_call","toolCallId":"custom_call_fnHNjEYnthEYAaxY84Ij6Kla",
...
The 76 `thought` events are the model thinking before the tool call,
streamed as `agent_thought_chunk` updates; an earlier run of the same
prompt produced none, so whether they appear is the model's, not the
channel's. The record keeps them either way.
