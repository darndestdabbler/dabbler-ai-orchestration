...
- **Start Session** registers the session before it opens anything, and
  ...
  is opened by you. The extension marks the terminal it opens for an AI, and
  the framework marks every engine it starts itself; a terminal you open is
  yours, so launch the engine with the marker set --
  `$env:DABBLER_ENGINE_TERMINAL = "1"; <engine>` in PowerShell,
  `DABBLER_ENGINE_TERMINAL=1 <engine>` in bash -- and the verbs that are a
  person's stay a person's there too. Claude Code and Copilot CLI are
  recognised without it, by their own variables. An engine whose variables
  nobody has read is not, and it will not get far without the marker:
  `dabbler session wait` is the AI's verb, and from an unmarked shell it
  refuses and says exactly this, so a shell that can take part in a session
  is always one the person's verbs refuse.
...
engine in the same words: report the step blocked and say why. The router
tells an engine from a person by what the calling shell carries — each
vendor's own marker, read and recorded in
`docs/design/engine-environment-markers.md`, and `DABBLER_ENGINE_TERMINAL`,
which the extension sets on the terminal it opens for the AI's CLI so the
rule holds for an engine nobody measured. The cancel a stop prints runs as
