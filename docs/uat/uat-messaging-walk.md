...
| Copilot CLI (`gpt-5.6-luna`) | 3 min 35 s | ISSUES_FOUND, one non-blocking nit | **8**: an Enter on the typed sentence, seven approvals of `session wait` and `session report` (the folder-trust answer is a permissions choice, not counted) | 0 |
...
| operator | Start Session, the engine pick, the model | 3 | no — the decision to start, and with what |
| operator | a question to the AI | 2 | no |
| operator | Enter on Copilot's folder-trust prompt, the first time it met the folder | 1 | a permissions choice |
| operator | Enter on the sentence Start typed at the prompt | 1 | **yes** |
...
**Which Enter did which** is read from the records, because Copilot's screen
cannot be. Start typed the sentence when the terminal opened, so an Enter on a
CLI already at its prompt would have sent it. Copilot's session started at
12:16:42; the first Enter, at 12:16:46.9, was followed by no message at all —
the CLI was not at its prompt but asking whether to trust
`C:\temp\s179-copilot`, which is not among the folders Copilot's own
configuration trusts. The second Enter, at 12:17:01.4, was followed 0.6 s later
by the sentence arriving as the session's first user message.

This walk's `walk.jsonl` logged both presses under one name, because the
harness could not yet tell them apart. It now makes the same reading as it
presses: an Enter is logged as submitting the sentence only when a user
message follows it within 8 s, and as answering the trust prompt — a
permissions choice, `"deterministic": false` — otherwise, so a rerun's ledger
says which without this paragraph.
...
Copilot's session `6f9ce452-d29e-42af-a463-f1d642bd756d`, copied beside them
as `copilot-events.jsonl`, and the Claude transcript is kept the same way in
`C:\temp\s179-claude-results\claude-walk`; the framework's is the scratch repository's
