...
export function raisePublicationDecision(
  ...
  return raiseOwed(repoRoot, {
    ...
    determined:
      ...
      "whose npm half is missing is the broken half-release. Both publish " +
      "through OIDC or a stored PAT in CI; no credential is read or written " +
      "here.\n\nWhat a wrong answer costs: a published version is public " +
      "from that moment and cannot be recalled -- npm refuses `unpublish` " +
      "after 72 hours, and a Marketplace version slot is never reusable. " +
      "The release-candidate answer exercises the entire path without " +
      "moving what `npm i` gets, and is the cheap way to be sure.",
    options: [
      {
        label: "release-candidate",
        consequence:
          `Pushes \`v${options.routerVersion}-rc1\`, which publishes to npm's ` +
          "`next` dist-tag only. `npm i -g dabbler-ai-router` still returns " +
          "404; `npm i -g dabbler-ai-router@next` works. Nothing about " +
          "`latest` moves, and the Marketplace is not touched.",
      },
      {
        label: "publish",
        consequence:
          `Pushes \`v${options.routerVersion}\` and then ` +
          `\`vsix-v${options.extensionVersion}\`. Both become public and ` +
          "neither can be recalled. `npm i -g dabbler-ai-router` starts " +
          "working, which is the item this closes.",
      },
      {
        label: "not yet",
        consequence:
    ...
    confidence: null,
    onNoAnswer:
      "Nothing is published. The work is verified and closed either way -- an " +
      "unpublished product is not an unverified one -- but a new project " +
      "still cannot install the router, so every adoption walkthrough stops " +
      "at its first step.",
    sessionNumber: options.sessionNumber ?? null,
  });
}
