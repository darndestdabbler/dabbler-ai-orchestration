...
- What comes back is **the bytes on disk**. There is no scrubbing layer
  between the file and the model on this path, so a read here is
...
- The round records `mode: tools` with `read` as its one granted
  operation. Every other round records `mode: none`, which is what a
  round that could not look has always recorded, and is never reported
  as equivalent to one that could.
