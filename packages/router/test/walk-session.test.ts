...
describe("a session-plan.md edit while a session is in progress", () => {
  it("is the step's change between instructions, and moves the tree under a step's checks", async () => {
    ...
    const engine: Engine = {
      ...
      invoke: ({ instruction }) => {
        ...
        if (instruction.kind === "rejection") {
          refusals.push([...(instruction.reasons ?? [])]);
          // Named this time, so the files agree and the checks run.
          report(sessionsDir, {
            seq: instruction.seq, stepId: "widget", status: "done",
            files: ["src/widget.py", "docs/sessions/session-plan.md"], testsRun: null, notes: "walked",
          });
        } else {
          writeFileSync(join(repo, "src", "widget.py"), "def widget():\n    return 2\n", "utf8");
          // A plan edit landing between the instruction and the report.
          writeFileSync(planFile, readFileSync(planFile, "utf8") + "between instructions\n", "utf8");
          report(sessionsDir, {
            seq: instruction.seq, stepId: "widget", status: "done",
            files: ["src/widget.py"], testsRun: null, notes: "walked",
          });
        }
        return Promise.resolve({ exitCode: 0 });
      },
    ...
    await capture(() =>
      driveSession(sessionsDir, { engine: "claude-code", provider: "anthropic", adapter: engine, maxInvocations: 3 }),
    );

    // Between instructions: the edit is in the step's change set, refused
    // where the report leaves it out and carried into the step where named.
    assert.equal(refusals.length, 1);
    assert.match(refusals[0].join(" "), /\[files-changed-omits\]/);
    assert.match(refusals[0].join(" "), /session-plan\.md/);
    // Mid-step: the check's tree moved, so the checks prove nothing.
    const last = readInstruction(repo, 1);
    assert.equal(last?.kind, "rejection");
    assert.match((last?.reasons ?? []).join(" "), /\[check-failed\].*\(the check changed the tree\)/s);
  });
});
