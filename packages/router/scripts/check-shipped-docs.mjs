...
//   - QUALIFIED names a thing that may be spoken of only where the same
//     neighbourhood says what is true of it -- a waiver only where the
//     page says there is none, and `codex` only where it says untested
//     ("undocumented, not unsupported", D268).
//   - DECLARED holds a dotted `dabbler*` id to the manifest that
//     contributes it. This is the half a word list cannot do:
//     `dabblerSessionSets.pythonPath` was never a retired *word*, it was
//     a retired *id*, and the page pointed a reader at it for a hundred
//     sessions.
//
...
const SHIPPED = [
  ...
  // end. They are not "walkthroughs" in the manifest's sense -- those are
  // the three under media/, above -- but they are read the same way, by
  // someone doing what they say, and round 1 of session 137 read the word
  // this way rather than that. Both readings are now covered, which costs
  // nothing and settles the ambiguity.
  "docs/uat/uat-dotnet-json-solution.md",
  "docs/uat/uat-java-json-solution.md",
...
/**
 * A settings id shaped like one of ours, wherever it appears in prose.
 *
 * `dabblerSessionSets.pythonPath` is the reason this rule exists: the
 * Marketplace page told a reader to point that setting at their Python,
 * and the setting had been gone for as long as the Python had. A
 * retired-word list could never have caught it, because the word was
 * never retired -- the *id* was, and an id is checkable against the one
 * place that declares it.
 */
const SETTING_ID = /\bdabbler[A-Za-z]*\.[A-Za-z][A-Za-z0-9]*(?:\.[A-Za-z][A-Za-z0-9]*)*/g;

/** `dabbler.yaml` and friends are files, not settings, and share the shape. */
const FILE_SUFFIX = /\.(ya?ml|json|cjs|mjs|js|ts|md|lock|log|jsonl|png|svg|vsix|schema)$/i;
...
/**
 * Every dotted id this extension actually contributes -- settings and
 * commands alike, because a page names both and the manifest declares
 * both, and a rule that knew only one half would fail an accurate page.
 */
...
function judge(label, text) {
  ...
  lines.forEach((line, index) => {
    ...
    for (const id of line.match(SETTING_ID) ?? []) {
      if (FILE_SUFFIX.test(id) || DECLARED_IDS.has(id)) continue;
      failures.push(
        `${label}:${index + 1}: \`${id}\` is not a setting or command this extension contributes\n      ${line.trim()}`,
      );
    }
  });
}
