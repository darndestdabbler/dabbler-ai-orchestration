# Field trial, session 70 of 71 — the half that needs a published router
...
| Order | Who | What |
|---|---|---|
| First | Session 70 | One version (2.8.0, both halves, enforced by `releaseVersion`), this document, the feedback audit below, and the plan amendment that adds session 71. Lands and closes. |
| Then | The operator | On the landed tree: `dabbler release` raises the brief; `dabbler owed answer --id publication --choice publish` settles it. The answer is the operator's alone — publishing cannot be recalled. |
| Then | The framework | Tags `v2.8.0`, waits for npm to serve the router, then tags `vsix-v2.8.0`. CI publishes over OIDC, gated on a green `Test` run for the tagged commit. |
| Last | Session 71 | `dabbler release --verify-install` as a step **check**, so the install is the record rather than a claim about it; then criteria 1 and 2 below, performed against the published extension from a clean profile; then this document's findings and item 5 of the audit. |
...
Written by session 70; item 5 closes in session 71 on the recorded
...
*Filled in by session 71, against the published pair.*
