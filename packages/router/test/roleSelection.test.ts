...
describe("whether the model asked for is the model that answered", () => {
  const seen = (requested: string, served: string | null) => ({ requested, served });
  ...
  it("answers honoured when every observation names the model itself, dated pins included", () => {
    ...
    // the one warning that matters routine too.
  ...
  it("reads a seat catalog's echoes as observations, keeping a silent entry silent", () => {
    ...
    assert.equal(modelFidelity("claude-haiku-4.5", observations), FIDELITY_HONOURED);
    // Probed and silent, and never probed: both say nothing, and dropping
    // either would make it indistinguishable from one that answered.
