...
describe("what a session would be run with", () => {
  ...
  it("reads the reviewer's list from the reviewer's own vehicle, not the machine's", () => {
    ...
    const root = tempDir("configuration-");
    // Keys, and NO settings file: the machine's vehicle and the role's are
    // both stated in the overlay below, because what this test is about is
    // the two disagreeing at one config layer.
