...
Outside the editor nobody opens that terminal, and the vendor markers are
what there is. An engine with neither is read as a person; that is the limit
of this mechanism, and the reason the guidance says, as well as the guard
enforces, that ending a session by force is never the engine's.
