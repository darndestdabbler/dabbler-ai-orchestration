...
// pid -> parent pid for every process, or null when the OS would not say.
function processTable() {
  ...
  for (const line of String(probe.stdout).split("\\n")) {
    ...
    if (a && b) rows.push([Number(a), Number(b)]);
  }
  return rows;
}

// Everything still alive under the command, after it has exited.
function survivors(commandPid) {
  const table = processTable();
  if (table === null) return [];
  if (!win) {
    // The group is the runner's own; every member but the runner itself.
    return table.filter(([pid, pgid]) => pgid === process.pid && pid !== process.pid).map(([pid]) => pid);
  }
  const found = new Set();
  ...
  while (frontier.length > 0) {
    ...
    for (const [pid, parent] of table) {
      if (frontier.includes(parent) && !found.has(pid)) {
        found.add(pid);
        next.push(pid);
      }
    }
