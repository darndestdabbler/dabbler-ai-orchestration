...
import { type LandFacts, type LandModuleFact, type LandSuiteFact, judgeLandReadiness, receiptCorrespondence } from "./land.ts";
...
${this.stopArtifacts()}`,
  ...
  private bundleCandidates(plan: ImpactPlan): string[] {
    ...
    return sessionModulesOf(this.sessionsDir, this.sessionNumber).filter((slug) => {
      ...
      return entry !== undefined && entry.kind === "application" && entry.package === null && !plan.candidates.includes(slug);
    });
  }
