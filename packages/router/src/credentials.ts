...
export function setCredential(
  ...
  // **What the REQUEST asks for is judged before what the MACHINE has.**
  // The same order the verb above uses for the name and the provider: a
  // request that is wrong is wrong on every platform, and answering it
  // with "this machine has no store" sends somebody to fix the wrong
  // thing -- and makes the refusal untestable anywhere but one operating
  // system.
  const index = readIndex(path);
  // **A name already owned by another provider is not overwritten.**
  // Replacing one is how a key is rotated and is the commonest reason
  // anyone runs this -- but replacing ANOTHER VENDOR's entry under the same
  // label destroys a credential some setting is pointing at, and does it
  // silently. The refusal names both providers and the two ways out.
  const owner = index.entries[name]?.provider;
  if (owner !== undefined && owner !== provider) {
    throw new CredentialError(
      `'${name}' is already a credential stored for ${owner}, and nothing ` +
        `was changed. Storing ${provider}'s key under that name would ` +
        `replace ${owner}'s, which a setting may be pointing at. Use a ` +
        `different \`--name\`, or remove it first with \`dabbler auth remove ` +
        `${name}\`.`,
    );
  }
  if (secret.trim() === "") {
