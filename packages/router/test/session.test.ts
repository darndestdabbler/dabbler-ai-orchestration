...
describe("what a start refuses before a session exists", () => {
  ...
  it("says in one line when the origin does not answer, and registers anyway", async () => {
    ...
    // the land was the first thing to meet it -- after all the work.
    const state = stateDir();
    ...
    try {
      ...
      assert.match(started.out, /did not answer as a git remote \(fatal: unable to update url base from redirection\)/);
      assert.match(started.out, /git remote set-url origin/);
