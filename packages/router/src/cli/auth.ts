...
async function setCommand(argv: readonly string[]): Promise<number> {
  ...
  if (kind === null) {
    // The variable BY NAME, not "the provider's environment variable". A
    // refusal that makes somebody run a second command to find out what to
    // set is a refusal that has not finished.
    const variable = providerVariable(provider);
    return refuse(
      "this platform has no credential store this framework can use " +
        "(Windows uses PowerShell's DPAPI, macOS `security`, Linux " +
        "`secret-tool`), so nothing was stored and no plain file was " +
        "written. Supply the key through " +
        (variable === null
          ? "the provider's environment variable instead"
          : `${variable} instead`) +
        ".",
    );
  }
