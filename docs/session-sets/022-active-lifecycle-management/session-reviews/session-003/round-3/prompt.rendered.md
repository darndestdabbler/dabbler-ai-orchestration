...
> **Round-3 context.**
>
> - **Round 1** (gpt-5-4, $0.096) found 2 Major issues, both in the
>   homehealthcare inventory narrative — a counting error and a
>   verdict that overstated compliance. Both fixed in narrative.
> - **Round 2** (gpt-5-4, $0.163) found 1 Minor finding: the
>   `dabbler-platform` / `dabbler-access-harvester` Lightweight-bullet
>   narrative said those 58 sets would "emit a stderr warning on next
>   boundary write" — but if they stay Lightweight, their next write
>   is a hand edit that does not pass through
>   `compute_effective_completed_sessions` and no warning fires. The
>   stderr warning only applies if a Lightweight set is later
>   transitioned to Full-tier CLI writes. The Lightweight bullets and
>   the verdict summary in the section below have been reworded to
>   distinguish "next hand write" from "future transition to Full-tier."
>
> **The doc-file edits (workflow, schema, close-out) were not flagged
> in either round.** Please verify the doc edits and the corrected
> cross-consumer narrative.
