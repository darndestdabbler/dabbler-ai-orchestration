...
- **The four verification stops stopped sharing a sentence.** A round that
  ended without a verdict, a dispute the framework refused to write, a cap
  reached over findings that cannot be shown remediated and a cap terminal
  met by a moved tree are four situations with four actors and four next
  moves. Each says its own, and a refused dispute now carries the refusal's
  own words — the evidence file over the inline cap, and not `(exit 2)`.
...
> **This is the release.** The operator ruled on 2026-08-11 that the next
> Marketplace push happens **once**, after Set 120 Session 3 and Set 115
> Sessions 1–3 landed, rather than publishing the `0.51.0` staged on
> 2026-08-10 as it stood. They have landed, and the entries below are what
> this artifact ships.
>
> **The release continues in the *staged 2026-08-10 tranche* section
> beneath these entries** — it carries the same version number because it
> is the same release, not a superseded one. Read the two together, with
> one correction that the tranche's own header repeats: the `<- here`
> marker described there **never shipped**. Set 120 S3 removed it from the
> router and Set 115 S4 from this extension (operator ruling: the
> in-progress icon carries the same information without claiming a single
> current step).
...
- **(Set 132 S1) The sidebar reads `AI Work Explorer` — once.** This
  **supersedes the Set 123 S3 entry under *Removed*** ("the activity-bar
  container is `AI Orch`"), which sits in this same `0.51.0` section and
  never reached the Marketplace — both entries publish here for the first
  time. Read them together and only this one describes what ships.
...
## [0.51.0] — the staged 2026-08-10 tranche (ships in the 0.51.0 release above, not separately)
