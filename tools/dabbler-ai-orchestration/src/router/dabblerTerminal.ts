...
export class DabblerTerminal implements vscode.Pseudoterminal {
  ...
  /**
   * The latest `instruction-overdue` the loop recorded since the last look,
   * said once here and once as a notification -- the operator may be in the
   * AI's chat, not in this terminal. An event for an instruction the run has
   * since moved past is history and says nothing.
   */
  private drainOverdue(file: string, run: RunRecord): void {
    const latest = this.newRows(file, true)
      .filter((row) => row["event"] === "instruction-overdue")
      .at(-1);
    if (latest === undefined || latest["seq"] !== run.seq) return;
    const seq = String(latest["seq"]);
    const step = typeof latest["step"] === "string" ? latest["step"] : "?";
    const seconds = Number(latest["outstanding_seconds"] ?? 0);
