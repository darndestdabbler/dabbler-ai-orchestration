 ...
 * that back.
 *
 * **The two sets must be equal.** Every edge taken is declared, and every
 * edge declared is taken. There is no flag saying an edge is exempt and no
 * early return that skips the second half -- either would let the model
 * carry a claim no test reaches, which is the whole failure this replaces.
 * So a restructuring that adds a route fails until the model says so, and one
 * that removes a route, or a test that stops driving one, fails until the
 * model stops saying so.
 *
 * It reads a whole run of THIS FILE, which is how the framework runs it: its
 * own step checks and the suites it selects name files, never `-t`. Under a
 * filtered run the second half fails by construction, and that is the honest
 * trade for having no exemption in it.
 */
describe("the lifecycle model", () => {
  ...
  /**
   * The transitions the driver actually took, by id.
   *
   * A `marker` is how two transitions sharing one (from, to) are told apart:
   * the driver logs `verification-passed` for the round it ran and
   * `verification-settled` for the one the ledger already answered, and the
   * line arrives immediately before the phase it explains.
   */
  function idsTaken(): { ids: Set<string>; undeclared: string[] } {
    ...
    const take = (from: string, to: string) => {
      ...
      if (row === undefined) undeclared.add(`${from} -> ${to}`);
      else ids.add(row.id);
      marker = null;
    };
    for (const line of spoken.join("\n").split("\n")) {
      if (/run-started session=/.test(line)) {
        current = "plan";
        continue;
      }
      ...
      const said = /\] (verification-passed|verification-settled)/.exec(line);
      if (said) {
        marker = said[1];
        continue;
      }
      ...
      if (moved) {
        if (current !== null && current !== moved[1]) take(current, moved[1]);
        current = moved[1];
        continue;
      }
      const stopped = /STOPPED \(([a-z-]+)[^)]*\) in phase '([^']+)'/.exec(line);
      if (stopped) take(stopped[2], `STOPPED(${stopped[1]})`);
    }
    return { ids, undeclared: [...undeclared] };
  }

  it("is exactly the set of edges a whole run of this file takes", () => {
    const { ids, undeclared } = idsTaken();
    // Nothing driven at all means the parser broke or this was run alone;
    // either way the assertions below would be meaningless rather than true.
    expect(ids.size).toBeGreaterThan(0);
    // A route the model does not know about.
    expect(undeclared).toEqual([]);
    // A declaration no test reaches -- the hand-maintained diagram this file
    // exists instead of. Adding a transition means adding the test that
    // drives it, in the same change.
    expect(model.transitions.filter((row) => !ids.has(row.id)).map((row) => row.id)).toEqual([]);
  });
