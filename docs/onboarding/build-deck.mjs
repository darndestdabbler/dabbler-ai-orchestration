...
export const SLIDES = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Run it from the repository you are working in — the sessions root is derived from where you stand, so no command has to name it.",
    ...
    // One line per shell, and one line for the command. A bare
    // `VAR=value` is not exported to the next process in a POSIX shell and
    // is not syntax at all in PowerShell, and a backslash continuation is
    // not syntax in PowerShell either -- so neither appears on a slide
    ...
    code: [
      ...
      "",
      "# bash, zsh",
      "export DABBLER_TRANSPORT=copilot-cli",
      "",
      "dabbler session next --engine copilot --provider openai --model gpt-5-6-luna",
    ],
  },
  {
    ...
    code: [
      "# The three the verifier reads, one per provider:",
      "#   DABBLER_ANTHROPIC_API_KEY, DABBLER_OPENAI_API_KEY,",
      "#   DABBLER_GEMINI_API_KEY",
      "",
      "# PowerShell, for this session; setx for every session after it",
      '$env:DABBLER_ANTHROPIC_API_KEY = "<your key>"',
      "",
      "# bash, zsh",
      "export DABBLER_ANTHROPIC_API_KEY=<your key>",
      "",
      "dabbler session next --engine claude-code --provider anthropic",
    ],
  },
  {
    id: "project-setup",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Later records nothing — dismissing a toast is not a decision, and the row stays.",
      "Other… , or a click on the row, opens a picker, and every item carries its consequence:",
      "Run `next` again (recommended) — the session resumes from the phase it stopped in, and the steps it has already accepted are not asked for again.",
      "Cancel the session — `dabbler session cancel` ends it with a reason on the record, and what the working tree already carries stays where it is.",
      "Nothing is lost either way: the phase, the accepted steps and any running job are on the record.",
      ...
      "For a second opinion — engine-error, framework-defect or plan-defect — from a provider that is not the working engine's:",
      "",
