...
export const ENGINES: readonly EngineChoice[] = [
  ...
  },
  {
    ...
    engine: "codex",
    provider: "openai",
    // Untested, and the description is where that is said. Session 137 took
    // codex out of every engine list a reader is taught from because no
    // session has been driven through it; it stayed HERE because a picker is
    // not a document -- dropping the row would strand an operator who
    // registered under the name the CLI still accepts, at the one command
    // that starts their session. Marked is the honest middle: the row works
    // and claims nothing.
    description: "openai — untested",
    modelRequired: false,
  },
  {
