...
describe("one loop, and a suite a step declares while it runs", () => {
  ...
  it("asks for a configuration a step leaves malformed to be repaired, and carries on from preverify", async () => {
    setProviderKeys();
    resetRouter();
    resetRuntimeMode();
    const repo = makeRepo(SEED, { origin: true });
    ...
    const engine: Engine = {
      ...
      invoke: ({ instruction }) => {
        ...
        report(sessionsDir, { seq: instruction.seq, stepId, status: "done", testsRun: null, notes: "walked" });
        return Promise.resolve({ exitCode: 0 });
      },
    };
    ...
    assert.deepEqual(asked, ["plan", "widget", "fix-configuration"], drive.stderr);
    const issued = fix as { ask: string; then: unknown } | null;
    assert.match(String(issued?.ask), /dabbler\.yaml/);
