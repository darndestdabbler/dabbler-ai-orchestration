...
export async function start(sessionsDir: string, options: StartOptions): Promise<number> {
  ...
  if (options.engine === "gemini") {
    writeErr(
      "start: refused -- Gemini CLI is no longer a supported engine: start the session with " +
        "claude-code or copilot (codex has a launch too); Google models still review the work " +
        "over the direct API.\n",
    );
    return EXIT_USAGE;
  }
