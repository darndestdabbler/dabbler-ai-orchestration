...
export function engineTerminalFor(
  ...
  return {
    // Named for the repository, as the loop's terminal is: Resume replaces a
    // CLI by its name, and a bare engine label would reach another
    // repository's CLI in the same window.
    name: `${choice.label} — ${repository.label}`,
    cwd: repository.root,
    program: cli.program,
...
export interface SessionRunUi {
  ...
  /** Close every open terminal carrying one of these names. */
  closeTerminalsNamed: (names: readonly string[]) => void;
  /**
...
export function defaultSessionRunUi(
  ...
  return {
    ...
    closeTerminalsNamed: (names) => {
      for (const terminal of vscode.window.terminals ?? []) {
        if (names.includes(terminal.name)) terminal.dispose();
      }
    },
...
export async function runResumeSession(
  ...
  ui.closeTerminalsNamed([terminal.name]);
