...
import { loadSuitesChecked, surfaceDigest } from "../src/testEvidence.ts";
...
describe("the root build files", () => {
  ...
  it("adds MSBuild's output to the ignore file bootstrap wrote, so the same source keeps its dev version", () => {
    ...
    // And the digest the dev version is formed on no longer moves with a
    // build: the same source packs to the version the first pack gave it.
    const before = surfaceDigest(repo, ["modules/model"]);
    assert.notEqual(before, null);
    seed(repo, {
      "modules/model/src/CsvModel/bin/Debug/CsvModel.dll": "MZ\n",
      "modules/model/src/CsvModel/obj/project.assets.json": "{}\n",
    });
    assert.equal(surfaceDigest(repo, ["modules/model"]), before);
  });
