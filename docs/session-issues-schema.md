...
| `fixVerdicts` | array | no | machinery (Set 096, omit-null). A remediation-review round's parsed per-finding verdicts: `{"finding": string, "verdict": "fix-accepted" \| "fix-rejected" \| "accepted-with-modification" \| "duplicate-of", "ledgerId"?: "L<n>", "duplicateOf"?: "L<n>"}`. A `duplicate-of` entry is the reviewer's declared identity judgment (fan-out siblings / reworded restatements); its disposition follows its target's. Observability only — blocking classification reads the re-stated Issue blocks, never this field. Present only on FINDINGS-BEARING rounds (the locked Set 055 invariant: envelope presence means issues found); on a clean remediation-review round the immutable, stamp-bound `sN-verification*.md` artifact is the durable record of the fix-verdict enumeration. |
| `verificationQualification` | string | no | Set 123 S2, omit-null. The qualification this round's verdict carries — today the single token `"same-provider"`, meaning **every** verifier call in the round resolved to the orchestrator's own effective provider, so the verdict is real but **not independently corroborated**. Written only for a project whose committed verify type is `DIRECT_API` and which holds no usable API key outside its orchestrator's provider (the operator's ruling: same-provider verification beats none, but the result must be flagged). A round with any cross-provider call is unqualified and omits the key, so **absence means fully corroborated** — which is what makes reading this field worth anything. The machine-checked copy lives on the paired metrics stamp row (`verification_qualification`), where the close gate enforces it as a bijection against the verifier's registry-resolved provider; this envelope field is the human-readable echo. |
