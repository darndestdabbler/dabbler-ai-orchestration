...
> `sort_keys=True, separators=(",", ":")` output. Proved in both directions
> before the case was written: Python wrote and approved a plan that the
> TypeScript reader accepted (which recomputes both hashes and refuses on a
> mismatch), and the TypeScript writer produced one Python accepted, with
> the same `plan_hash` on both sides.
