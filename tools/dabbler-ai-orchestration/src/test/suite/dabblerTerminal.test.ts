...
suite("the Dabbler terminal", () => {
  ...
  test("says a stop out loud, and never speaks a word the engine said", () => {
    ...
    assert.ok(spoken.includes("next=dabbler session next --max-invocations"));
  ...
  test("carries the stop's code to the rendering, so the coded stops keep their own actor and command", () => {
    ...
    const spoken = plain(written.join(""));
    assert.ok(spoken.includes("who=you"));
    assert.ok(spoken.includes("next=dabbler verify adjudicate"));
    assert.ok(!spoken.includes("next=dabbler session next"));
