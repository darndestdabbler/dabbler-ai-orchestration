 ...
 * where the operator has one. They are merged, and a merge nothing enforces
 * comes apart at the next release: npm and the Marketplace each require a
 * literal `version` in their own manifest, so "one source" can only mean one
 * number and one rule that refuses the rest.
 *
 * This is that rule, and `dabbler release` asks it before it tags. A
 * repository whose halves disagree cannot be tagged at all -- which is
 * exactly the moment the disagreement would otherwise become public, as two
 * artifacts nobody can say the version of.
 */
export function releaseVersion(repoRoot: string): ReleaseVersion {
  ...
  if (router === null || extension === null) {
    return {
      version: null,
      reason:
        "this repository does not declare both a router and an extension version, " +
  ...
  if (router !== extension) {
    return {
      version: null,
      reason:
        `the router declares ${router} and the extension declares ${extension}. ` +
        "They are one version: an install that shows two is two things where the " +
        "operator has one, and a tag pair built from them would publish that",
    };
  }
  const dependency = declaredRouterDependency(repoRoot);
  if (dependency !== null && !dependency.includes(router)) {
    return {
      version: null,
      reason:
        `the extension declares ${extension} and depends on dabbler-ai-router ` +
        `${dependency}. The extension bundles the router, so a dependency that ` +
        "does not name this version is a Marketplace build wrapping something else",
    };
  }
  return { version: router, reason: "" };
}
