...
/** One *Dabbler* terminal per repository, however many times it is asked for. */
const open = new Map<string, { terminal: vscode.Terminal; pty: DabblerTerminal }>();

/**
 * The terminal for one repository, created if it is not already there.
 *
 ...
 * workspace folder, a worktree, or one bootstrapped after activation, all
 * of which had no terminal at all when this was created once at startup.
 */
export function ensureDabblerTerminal(
  repoRoot: string,
  beside?: vscode.Terminal,
): void {
  let entry = open.get(repoRoot);
  if (!entry) {
...
export function openDabblerTerminal(repoRoot: string): void {
  ...
  open.set(repoRoot, { terminal, pty });
}
