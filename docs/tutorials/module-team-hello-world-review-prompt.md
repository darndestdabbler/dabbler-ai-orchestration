...
This document contains a reusable AI prompt for reviewing how well a team is practicing the Dabbler module-organized workflow. It is intended to be run by an AI assistant with access to a team's code repository to provide evidence-based, coaching-tone feedback on workflow hygiene, not on code quality. This review is a practical application of the concepts introduced in the [module-organized projects primer](../planning/module-organized-projects-primer.md) and is a useful graduation check after completing either the agent-agnostic [team hello world walkthrough](./module-team-hello-world.md) or its standalone [Copilot + Azure DevOps cut](./module-team-hello-world-copilot-ado.md). The seven principles below are host- and agent-agnostic — they score a repository's end state (modules, trunk hygiene, ownership, tags, CI), so the same prompt applies whether the team ran the GitHub-worked walkthrough or the Copilot + Azure DevOps one.

> **Maintainer line item — audit the tutorial pair for drift.** This prompt is the companion to **both** hello-world tutorials, whose module / trunk / worktree / session-lifecycle **spine is shared verbatim**. When either tutorial changes, confirm the shared-spine content still matches across the pair ([`module-team-hello-world.md`](./module-team-hello-world.md) ↔ [`module-team-hello-world-copilot-ado.md`](./module-team-hello-world-copilot-ado.md), mapped by the latter's sync-map appendix): a shared-spine edit that landed in only one document is drift and must be echoed to the other (the L-065-1 every-echo discipline). Host/agent-specific rows (GitHub CODEOWNERS/Actions vs ADO branch policies + `azure-pipelines.yml`; agent-agnostic vs Copilot-seat) diverge by design and are not drift.
