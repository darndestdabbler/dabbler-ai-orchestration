...
suite("Resume Session", () => {
  test("Resume Session restarts the loop only when no heartbeat says one is driving, and reopens the recorded engine waiting on it", async () => {
    ...
    const closed: string[][] = [];
    const live = driveUi({ closeTerminalsNamed: (names) => { closed.push([...names]); } });
    assert.strictEqual(await runResumeSession(repository, live.ui, cli, () => true), true);
    assert.deepStrictEqual(closed, [["GitHub Copilot — csv-pipeline"]]);
    assert.strictEqual(live.terminals[0].name, "GitHub Copilot — csv-pipeline");
    // Another repository's CLI in the same window carries another name, so
    // it is never among what this Resume closes.
    const other = makeRepository({ root: "D:\\ws\\orders", orchestrator: repository.orchestrator });
    const otherCli = engineTerminalFor(other, ENGINES.find((e) => e.engine === "copilot")!, "gpt-5-6-luna") as EngineTerminal;
    assert.ok(!closed.flat().includes(otherCli.name));
