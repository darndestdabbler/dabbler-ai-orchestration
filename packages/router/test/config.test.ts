...
describe("resolving the transport", () => {
  ...
  it("reads a malformed settings file as silence, never as a value", () => {
    // A half-typed file is a file somebody is in the middle of editing. A
    // recovered parse of it would resolve a session onto whatever survived
    // the syntax error, which is a vehicle nobody chose.
    const root = bare();
    const path = join(root, SETTINGS_RELPATH);
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, '{ "dabbler.transport": "copilot-cli"  "x": 1 }', "utf8");
    assert.equal(resolveTransport(makeConfig(), null, root), "api");
  });
});
