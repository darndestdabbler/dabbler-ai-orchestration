...
>
> **Status:** diagnosed, **not fixed**. No session owns it. Captured as a
> durable note because it surfaced mid-session in another set, and
> because part of Component A is an operator-authored
> verification-reduction that must not be inferred from this note by a
> later orchestrator — it needs its own journalled decision with
> attestation.
...
(`authoring.max_steps_per_session`, currently 5) and seeds those same
