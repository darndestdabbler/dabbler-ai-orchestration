# STATUS — sessions 113–158 CLOSED, all VERIFIED: the deployables block, the Java/Maven walk and its nine defects, the suite off the operator's machine, the principle of who owns a command, the policy a module session runs under, the basics the operator saw go wrong, the focused-or-global session the plan decides with one click to start it, the UAT walk that found ten product defects in the UI path, sessions 131–133 answering all ten of them, session 134 fixing what the verifier is told and widening what it can see, session 135 letting the direct-API verifier ask for a file and giving .NET a root, session 136 measuring what the run of record does to the operator's machine and cutting the load, session 137 shipping the release and paying for four defects on the way, session 138 repairing all four, session 139 preparing 2.0.19 and putting its publication to the operator, who held it, and session 140 making the two surfaces that say where a session is agree with each other and with the record, and shipping it, session 141 taking the answer to "which branch is the trunk" from the repository rather than the host and refusing a checkout that carries no record, session 142 turning that refusal into a choice the operator is offered and a verb that carries it out, session 143 holding the Dabbler Terminal to one rule -- every phase in one tone, one gate row for both screens, and marks painted in a job's bytes -- and session 144 answering whether the model asked for is the model that answered, where three verification rounds found three real things and two of them were defects that would have shipped, session 145 giving the operator one place to see and set what a session is run with -- which uncovered that the model list was still the direct-API registry on a Copilot seat -- session 146 making the code agree with what a model list actually costs to obtain, which is nothing on all three surfaces: the seat states its own models over its protocol, a free record refreshes itself at session start, a model that stopped being served is marked rather than dropped, every cost names its billing platform, and the pane offers a seat's own catalog on a seat, and session 147 making a stop something a developer can act on: what refused in the words of the thing that refused, who acts, what each way on costs and the command for it; **2.1.3 IS PUBLISHED to the Marketplace, carrying 148 and 149. 2.1.0 and 2.1.2 never shipped** — 2.1.0 refused over a walk-jobs timeout on the runner and 2.1.2 died on a Marketplace request timeout, so both tags stand on origin unpublished and 2.1.3 supersedes them; session 152 naming the two reviewing roles by voice and giving each its own vehicle, and session 153 taking the run of record off the host scheduler into a Podman container — 324 s to 26.4 s — with the one file that cannot follow it declared and the reason it cannot owed forward, and a guard that will not read a cancelled test as a pass, and session 154 closing the dark gate behind the extension suite's second door — CI had been red for eleven consecutive runs while three sessions closed VERIFIED through the other one — by arming the seam once where both doors read it, making the refusal cover mocha as well as node:test, and holding every gate to the suite it stands for with a divergence that is declared rather than forbidden, session 155 organising the Configuration section by the thing being configured rather than by the mechanism that configures it — two participants, five leaves, the Auxiliary Reviewer given the surface it has never had, and a projection re-derived whenever it is read, because nothing in a workspace can watch a user-level catalog — and session 156 walking the whole model-catalog block on both transports and both engines BEFORE publishing it, which is the entire argument for the order: twenty-one numbered readings, eleven defects fixed and four raised, five of them stopping an operator and every one of those five sitting in the block that was about to ship, **and 2.2.0 IS PUBLISHED**, and session 157 giving a configured choice one place to live -- four layers deciding a vehicle and no fifth, `DABBLER_TRANSPORT` read by nothing that decides, the `configuration` block out of the projection and three vestigial fields out of the catalog, one reviewing vehicle for both reviewers, and `dabbler configuration options`/`explain` beside a `--authoring-model` that has never existed while the pane offered to set one -- through four rounds of blocking findings and one dispute the reviewer upheld, and session 158 making the model an operator picks the model that runs -- `--model` reaching both CLIs from the launch a person presses for the first time, an engine's refusal read as a refusal where its exit code says the run went fine, the authoring list read for the ENGINE rather than the machine, and the Configuration section DRIVEN in a running editor before it shipped, which found five defects in code that had already closed VERIFIED -- **and 2.3.0 IS PUBLISHED**, and session 159 giving an operator a way to change which API key a session runs on without editing their environment -- a credential store on this machine, a NAME in the committed settings and never a value, the environment still first so nobody's CI changes, and a walk that drove it in a real terminal and a running editor and found three defects in code written the same morning -- **and 2.4.0 IS PUBLISHED**, and session 160 shipping the shim -- the `dabbler` command inside every VSIX since 2.3.0 had died on its first require because the extension's build lacked one bundler option the router's build had, so the option is in both places, the build now runs the bundle it just wrote before packaging it, the pre-commit hook blocks a router that ran and crashed instead of letting the commit through with a stack trace, and the walk installed the VSIX into a scratch editor and ran the printed commands through the shim in a real terminal -- **and 2.4.1 IS PUBLISHED**, and session 161 making the driven declaration, the global session and the stop toast say what they mean -- the plan's reason reaching the declaration so a cross-module session driven through `session next` no longer stops at its first move, no policy and no `scope` for a global session, `Scope: whole repository` winning wherever it appears with a `Module:` line beside it filing the session under that module, and the stop toast carrying the stop's own first sentence -- not released, by design -- and session 162 making what a developer reads and clicks around a focused session say what it does -- the README page before a first focused session, the refusal that names the extension's own settings file, a dash on a gate that judged nothing, two launchers titled for the window they open, a Vehicle row that stops saying "none installed" on a machine with two engines -- and WALKING the sample's own sessions 3 and 4 through the built VSIX, where the first releasable module session stopped four times on rules that hold for one module and not four and each was fixed with its test, and a focused checkout was found unable to run a solution-wide suite, which is owed -- **and 2.5.0 IS PUBLISHED: `vsix-v2.5.0` on origin, CI's publish job green at 16:17Z; the gallery lags the job by about ten minutes**, and session 163 removing the questions nobody needs to answer -- `rebaseline` records and asks nobody, `--approver` gone from three verbs with who was working read from the record `start` wrote, a spent report judged as none written so a stray `next` costs no refusal, every amend folded into the activity log and read out at the close, session 1 told where the brief is instead of told to ask, and a triaged stop printing the amendment it proposes -- not released, by plan, and session 164 holding the framework to the rule that what it wrote is never the step's to account for -- the candidate's files subtracted by provenance, central package management and the notes page born with the first package module at `modules create`, a declared folder feed made where it is needed, no by-hand test advice printed while a driven session is in flight, and the declaration's clean-tree rule asked at `session start` with a `tree` stop of its own -- not released, by plan

**Branch: `master`.** Trunk-based; nothing lives anywhere else.

> ## 2026-09-22 — 231 THE MECHANIC planned on branch `mechanic`; 229 and 230 CANCELLED
>
> **Read this before the block below it, which it supersedes.** 229 was started and had accepted three steps of the proposal design when the operator ruled otherwise: the way past a wall is what a developer does anyway when stuck -- open another chat instance and tell it to fix the repository -- made first-class. A **Mechanic**: a fresh-context model at least as capable as the author, briefed by `dabbler consult --mechanic` with the stop, the record rows, the transcript and the rules; working in PROSE, not a closed menu; acting only once the person agrees; making the **minimum viable unblocking fix** and logging it in `docs/sessions/mechanic-log.md`, which the Primary reviews in the diff. The ruling is point 5 of `docs/operator-decisions.md` (2026-09-22). 229 is cancelled (`ad616640`, its code kept at `3cb6456c` on `experiment/proposals`, not to be merged); **230 is owed a cancel by a person** (`dabbler session cancel 230 --sessions-dir docs/sessions --reason "..."` -- an engine may not cancel a session it is not working). **Work happens on `mechanic`, branched from master and pushed; the framework reads the trunk from HEAD, so every land goes to `origin/mechanic` and master does not move. Run no session on master while the branch lives -- `sessions.json` is one file.** 231 is held; the operator walks a local VSIX (three litmus scenarios on the Java sample, Sonnet authoring); merge to master; 232 releases 3.19.0. Start 231 with `dabbler session next --sessions-dir docs/sessions` on `mechanic`.

> ## 2026-09-22 — (superseded) 229 and 230 PLANNED: a proposal is the way past every wall
>
> **Why.** From a consumer repository's terminal: a session whose plan named `release` reached the close with no publish on its record, `published_when_releasable` said *"made at step (a), never here"* and named no way out, the Authoring AI (Haiku) tried flags that do not exist and gave up, and a sibling repository commented the packaging block out of `dabbler.yaml` to close. The operator's ruling is in `docs/operator-decisions.md` (2026-09-22) and consult round 21 (`docs/design/consults/round21-*.md`, two briefs, Sol and Gemini both): no over-engineering; every impasse has a way out and no rule stands above finishing; plans change at any time with the operator's approval (the SQLite→H2 case); a proposal is the LAST RESORT; the reviewer rules first, on the ordinary round; proposals are counted and shown, never scored.
>
> **229** (held): `dabbler session propose --reason <why> <what>` — a plan amendment or a hold, never free text — handed to the Primary Reviewer as a labelled finding (endorse, or return one finding the author disposes of as normal work); an endorsed engineering change is applied and reported at the close; an outcome change (a hold) reaches the person as a stop of kind `proposal` with Approve/Reject/Cancel, which the extension shows as a MODAL for that kind and no other; the close gate's words and the `close` stop's menu name the verb; the managed body says last resort; the close summary and `metrics` count. Walk page `docs/uat/uat-proposals.md`: the Java sample, Claude Code, `claude-sonnet-5` authoring, two scenarios — a plan that cannot be built as written (SQLite through `hibernate-core` only → H2, nobody asked) and a packaging block with no feed (→ the modal, Approve closes as held). **230**: the walk's findings and the 3.19.0 release.
>
> **How to start 229:** `dabbler session next --sessions-dir docs/sessions` as the managed body says. The consult answers were obtained with `dabbler agent prompt --engine copilot --model gpt-5.6-sol` (the seat; the OpenAI key is out of credits) and Gemini `gemini-3.1-pro-preview` over the API key.

> ## 2026-09-20 — 218 and 219: every answer asks for the next instruction; 3.16.0 walked installed on both engines
>
> **The published 3.16.0 is the package to soak: SHA-256 `7a02d7aba1240696485a659ebb44c31d99d72a6274a3b6f09b071e5128028fb0`** (CI run `35562713225`, artifact `vsix`; kept at `C:\temp\s219-installed\published\`). The gate this block used to hold — compare it with the candidate that was walked — answered `"same": false`, 28 of 67 entries, so the comparison designated nothing and the published file was **walked itself**, all four cases, fail-closed: `published-claude-main-2`, `published-copilot-main`, `published-claude-cancel`, `published-copilot-cancel`, every one accepted, each installing `7a02d7ab…`, nothing left running. What the 28 were: `dist/dabbler.cjs` and `dist/extension.js` are byte-identical; 26 text assets (the schemas, the two prompt templates, the two source maps, the license, the SVGs) are identical once line endings are set aside; `[Content_Types].xml` and `extension.vsixmanifest` differ as two versions of `vsce` write them (entry order, two marketplace properties, the case of `README.md`). **The CRs are this machine's, not the product's:** the index is LF throughout and `.gitattributes` says `eol=lf`, but 152 files in THIS working tree were checked out CRLF before that file landed and have never been checked out again (`git ls-files --eol | grep w/crlf`), so a package built here carries CRs that CI's never does. A locally built VSIX is therefore never the published one on this checkout, and `--compare` will keep saying so until the tree is checked out afresh — a person's call, it rewrites 152 working files. Walk what CI built.
>
> **One void run, kept: `published-claude-main`.** Thirteen of fourteen cases held and the fourteenth was the walk's own bookkeeping: Claude answered step 1 in 16 s, the walk sent its `session interrupt` one second after instruction 3 was written while still holding 2 as the last it had seen, and judged 3 to be "the next instruction". The framework's own reply says the interrupt was filed against instruction 3, and instruction 4 carried it first among its reasons — which is the behaviour asked for. The walk should take "which instruction" from the interrupt's own reply rather than from its last poll; harness work, owed, no product in it.
>
> **Why this block exists.** Session 215 showed the mailbox could be healthy while delivery stopped: the loop wrote step 1 and the interactive AI never re-armed its one-shot waiter. Reminders and Resume leave that gap where it is. A proof outside any session (`docs/design/next-instruction-poc-results.md`) showed both engines can answer with ONE background command and take their next instruction from it; 218 built it on the pull the router already had; 219 walked it installed.
>
> **218** (`5015c02e`, VERIFIED r3): `dabbler session report --next` is the ordinary report and then the existing `sessionNext` with no bound on the wait inside the call — it never returns `wait`, stdout is one instruction, and under the pull every `answer_command` carries it. A repeated answer takes nothing twice; one repeated after the close is told that session's own `done`. A chained call that lost the lease stands down before the add, the commit, the push and the close. The author cancels the session it is working with its number and a reason and no `--force`, and no other session. Start Session opens the AI's CLI and the Dabbler Terminal and no loop; Resume Session is gone; every stop says *ask your AI to run `dabbler session next`*. `session run --mailbox` with `session wait` stays as a fallback a person starts by hand.
>
> **219**: 3.16.0. The Dabbler Terminal draws a replayed router line as it draws its own (no `dabbler [` prefix; the operator's request). `walk-vsix.cjs` stages its own disposable repository, walks `main`, `cancel` and `direct` scenarios, **fails closed** on any unmet case, and `--compare`s two packages' contents. Record: `docs/uat/uat-installed-conversation.md`. Both engines closed VERIFIED from one `session next` and chained background answers, with two questions answered, an interruption on the next instruction, the Dabbler Terminal rebuilt after being closed, a killed exchange repeated with everything once, a cancellation that kept the author's files, and nothing left running.
>
> **The walk rejected five candidates, and three of the reasons were product defects.** (1) An AI told at once that its answer command had died read the framework's progress lines, decided the framework would finish alone, and ended its turn: the managed instruction turned on whether anything had been *printed*; it turns on what the output *ends with* now. (2) The refusal of `cancel --force` told an engine to report the step blocked, which paused a session its operator had asked it to cancel; it says the engine's own form now. (3) A locally built package is whatever is lying in `dist`: five stale or machine-written files CI's build never had. The build empties `dist` first, and the local candidate has CI's 67 names exactly.
>
> **3.18.0 IS PUBLISHED (2026-09-21 12:47 local): `vsix-v3.18.0`, CI publish run `35627072664` green in all four jobs, and `dabbler release --verify-install` says the Marketplace serves 3.18.0.** It carries sessions 224–228 — the operator's six rules for configured choices. The operator walked the local build and ran a full soak in `csv-parser-dotnet` with no errors, then said yes. NOTHING IS BUILT AND UNPUBLISHED NOW; read every "NOT PUBLISHED" below as history. Owed, not scheduled: a README line that Claude Code offers no auto permission mode on Haiku, so a Haiku-authored session stops on a permission prompt the framework cannot see; and the read-side rule for an incompatible machine-default model (below).
>
> **3.17.0 IS PUBLISHED (2026-09-21 06:25 local): `vsix-v3.17.0`, CI publish run `35587873087` green in all four jobs, and `dabbler release --verify-install` says the Marketplace serves 3.17.0.** It carries sessions 220–223. The operator walked the local build twice first — different vehicles, models and platforms (Java and .NET) — with no issues. **224 CLOSED VERIFIED r4 (`98be1ba2`, held): the router's half of the operator's six rules for where a configured choice lives.** The engine and both reviewers' models joined the four layers (`dabbler.engine`, `dabbler.reviewerModel`, `dabbler.auxiliaryModel` — the keys were already declared and read by nothing); `configure` writes the CHECKOUT and seeds a machine that has no default, `--mine` writes the machine's default only; `session start` with no `--engine` reads the layers; the projection carries `decidedLayer` (`checkout`|`machine`|`shipped`|`default`, where a choice is KEPT — `keptIn`) on every choice's node, and `explain` prints what each shadows; a loaded config records the checkout it is OF (`checkoutOf`) so a pane asking about a repository it is not standing in is answered from that one, and a config of NO checkout has no checkout layer (the host suite's no-real-git guard caught the fallback). Rounds 1 and 2 each held the work to the plan's own words, and were right. **225 CLOSED VERIFIED r2 (`5e1bb4ee`, held): the extension's half, version 3.18.0.** Start Session asks nothing — it uses the engine and model the repository's Configuration shows, and where those are missing it starts nothing, says which row, and brings the Solution Explorer forward (round 1 rightly refused a button that led to a one-off picker: a choice made at Start is written nowhere); *Start Session with Different Models...* asks for one session and writes nothing; *Keep as My Default* is *Keep as Machine Default*, on all five choice rows, always `--mine` (the old engine branch wrote without it); each choice row says `· this repository` / `· machine default` from `decidedLayer`, an unlisted choice included. **226 CLOSED VERIFIED r3 (`bf8b8b1a`, held, same 3.18.0): different models means the reviewers too.** The operator, walking 3.18.0 in csv-parser: *Start Session with Different Models...* with a Claude author was refused because that repository's saved Primary Reviewer is Claude's, and the command asked only for the engine and the authoring model; then: *do we also need the Auxiliary?* — yes, since a one-session author and primary routinely land on the saved auxiliary's vendor and nothing says so until the first dispute. `session start --reviewer-model/--auxiliary-model` records each on the session's OWN ledger row (`reviewerModel`, `auxiliaryModel`; schema + generated type) and writes no setting; a role's model is read session → checkout → machine in the one reading (`withSessionReviewers`/`sessionReviewerOf` ride on the configuration as the checkout does; `route.ts` stamps them from `sessionState.namedReviewers`); the start judges the reviewers it NAMES, an auxiliary included — one nobody named is judged at a dispute as always (round 2 claimed otherwise; disputed with code and a test, withdrawn in round 3); the command asks four things, each list narrowed by the picks before it, and is never silent about a role it cannot fill (no primary possible → says so, starts nothing; no third vendor → says what that means and the person decides — refusing outright would make it unusable on the documented two-vendor minimum). **227 CLOSED VERIFIED r3 (`234f6810`, held, same 3.18.0): different models means the reviewing vehicle too.** The operator asked why the command did not ask for it; 226 had written it down as a non-goal on the AUTHOR's own call, against rule 3's "vehicles and models" — and it is inseparable from 226, since the reviewer lists ARE what the reviewing vehicle lists. `session start --reviewer-transport` records it on the session's own row (`reviewerTransport`); `explainReviewingTransport` reads a session's vehicle from the configuration it is given, above every saved layer and beneath a flag typed on the call (`withSessionVehicle`/`sessionVehicleOf`), stamped at the start's check, a round's dispatch (`route.ts`) and a round's grant (`verify/rounds.ts`); `configurationNode` takes `reviewerTransport` and `reviewers` for the session about to start; the command asks FIVE things in the order each decides the next — engine, model, reviewing vehicle, Primary, Auxiliary. **Both of the reviewer's blocking findings were right and mine were the errors:** (r1) `start()`'s EARLIER check, `configuredModelRefusal`, judged the SAVED reviewer before the vehicle-aware check ran — my first integration test passed VACUOUSLY because this host's own third API key made the two-vendor catalog read as another key set's (a test of a catalog block must clear ALL THREE keys and set exactly the ones it means); that refusal also said `preferences.json` for a choice kept in the checkout; (r2) a CONTINUATION (`session start` on the session in flight, which names no reviewers) was judged on the saved reviewer and then had its own ERASED from its row — an omitted one is now "not stated" and carried forward from the row, as an omitted identity field already was. **228 CLOSED VERIFIED r1 (`455887bc`, held, same 3.18.0): a machine's default model belongs to its default engine.** The operator found their machine default engine is Claude Code and its default model `gpt-5.6-luna`: 224 seeds each choice separately, so a Copilot repository's model was seeded beside an engine that cannot run it. Measured first: the pane's row announced it and `session start` refused it before anything was billed — but the refusal named this checkout's settings file for a model the MACHINE held. Now the authoring model's refusal names the saved layer that holds the model (machine default, with both ways on; this checkout; or this call), and `configure` seeds an authoring model only where the machine's default engine, as the call leaves it, is the engine the repository runs, saying why in one line where it does not; `--mine` is a person's act and is not second-guessed. The operator's own `preferences.json` was NOT touched — *Keep as Machine Default* on the model row of a Claude Code repository replaces the pair. Their bar, in their words: a first-time machine says "not set", and a conflict is caught and said before a session runs. OWED, NOT SCHEDULED (walked on a scratch machine profile after 228): a fresh machine configured the ordinary way gets a consistent pair; two paths still show an incompatible default, both announced on the row and refused at Start with the right file named -- (a) a second repository on ANOTHER engine shows the machine's default model with a warning until its own model is picked, (b) *Keep as Machine Default* on the ENGINE row alone leaves the old engine's model as the default. One read-side rule would end both and heal an existing file: a machine-default model applies only where the engine in force lists it. Not built -- by the operator's own rule it neither stops a session nor misleads. Reviewer rows still show the preference order's pick with no owner label where nobody chose; left alone, offered. **A LOCAL 3.18.0 VSIX WITH 224–228 IS BUILT AND INSTALLED (10:38) AND NOT PUBLISHED: the operator walks it from `docs/uat/uat-configured-choices.md` — seven checks, one per rule — and on their yes `dabbler release` tags `vsix-v3.18.0`, one release for 224–228.** What follows is how 3.17.0 came to be.
>
> **(history) 3.17.0 was built and walked before it was published.** Session 221 (`08fcae5c`, VERIFIED r1, closed `8f93a571`, release HELD): a `dotnet test` or Maven run of record reads as one line a test project (`✔ X: 16 pass, 0 fail, 0 not run`, `✘` with the failing tests named, `⚠ X: no tests found`), the suite's own output kept whole under `.dabbler/test-output/`, any other runner shown as before and the outcome still the exit code (`testOutput.ts`, `cli/testEvidence.ts` `spawnSuite`, the terminal's `suiteResultSpans`); and `session start` reads a chosen model by `normalizeModelToken` like every other surface, so a dated vendor id for a model the seat lists under its own name no longer refuses Start. **On the operator's yes: `dabbler release` — it tags `vsix-v3.17.0`, one release carrying 220 and 221 — and nothing else.** Two things to know: (1) the reviewer's model is a MACHINE-WIDE preference, so an AI authoring here on Claude while the operator walks a Copilot-authored sample with a Claude reviewer cannot both be satisfied by one `preferences.json`; 221 ran with `DABBLER_PREFERENCES_PATH` pointing at a session-scoped copy (the suite's own seam) and the operator's file was never touched. (2) The operator reported Maven output not showing in the Dabbler Terminal; their paste showed seven lines of a 9 KB log, the first starting mid-word. **Session 222 (`ed453bba`, VERIFIED r1, closed `8e787f89`, held, in the same 3.17.0)** found and fixed it: the terminal's first look at a run set EVERY log already on disk to its current size — the running job's too — so a terminal opened, reopened or reloaded while a job ran dropped what the job had written so far (`sayEarlier`; the running job's log now starts at zero; a test reproduced the operator's exact screen before the fix). **That was NOT what the operator saw** (they had not reopened anything). **Session 223 (`b2c7c18d`, VERIFIED r1, held, same 3.17.0)** fixed what they did see, and the reason csv-parser's session 4 was cancelled by its AI: (a) in one poll the terminal drained a job's log and only THEN said `working`, so the job's output was cut in two under two dividers and the tail at the bottom read as the whole — `working` is now said before the first read; (b) `packaging.ts` `runStep` started its command with a bare `spawnSync`, so on Windows `mvn`/`npm`/`gradle` (`.cmd` shims) could not start — `could not start: ENOENT: spawnSync mvn ENOENT` in the record, `pack: exit null` in the stop — so EVERY releasing Maven session on Windows died at publish, after its work had landed, and the AI's only move was to cancel. `runStep` uses `spawnSyncProgram`; a step with no exit code is printed with its reason; proved against real `mvn` (exit 0, BUILD SUCCESS). **Left for a person to decide, not fixed:** an AI whose session is stopped at publish by a framework fault has no forward exit of its own but cancel — `hold-release` is a person's verb — so a landed, verified session ends up `cancelled` in the ledger (csv-parser's 4: work is on `master`, 5–7 built on it, nothing lost but the label).
>
> **NOTHING ELSE IS OWED. 3.16.0 is what developers install today; 220 (`0673da0b`, VERIFIED r1, closed `2eaec47d`, release HELD) rides in the next release.** No soak and no session is waiting: the next work is whatever the developers' real use turns up, and a finding gets a session only if it stops a session or misleads the operator.
>
> **What 219 left owed was session 220's, and is done.** One of the three was a wrong diagnosis: 219 said a framework job holds its caller's output, which is why Claude Code repeated a killed command only as the suite ended (12–41 s). Measured in 220 before any change, a detached job started as the router starts one does NOT hold the pipe (it closed 1.5 s after the caller exited, the job ran on for six; `walk-jobs.test.ts` pins it and fails against an inheriting spawn). What holds a pipe is a process left alive in the killed command's own chain. Why Claude Code was late is unproven, no case failed for it, and there is no router fix to make. The other two are 220's steps: `session interrupt` says truthfully what becomes of a message, including one sent after the last step; and the walk reads which instruction an interrupt was filed against from the framework's own reply.
>
> **Traps met.** `View: Close Editor` closes whichever editor the palette hands focus back to — the first walk closed the AI's terminal with it. Keys typed while Copilot shows a permission dialog go to the dialog: paste a message whole, in the quiet after a dialog is answered, and take Copilot's own `user.message` as the word on whether it was heard. Launched as Start launches it, Copilot raises ~16–20 dialogs a session. A packaged file changed after a walk is a new candidate: finish, freeze, build, walk. `vsce` keeps a file matching ANY negation in `.vscodeignore`, whatever else excludes it. Git Bash's `tar` does not read a zip; `C:\Windows\System32\tar.exe` does.

> ## 2026-09-20 — 213 CLOSED VERIFIED r5, `vsix-v3.14.0` tagged; 214 is next (no silent wait)
>
> **Why this block exists.** Two beta tests on the operator's work computer went wrong: one session was read as releasable though nobody had said so, the AI cancelled it, and it ended with uncommitted code; in the other the AI's waiter waited for something that never came. The operator's goals: a human is bothered mid-session only at planning, when an AI service is down, or by their own choice; **no Resume button**; an impasse the framework did not anticipate goes to a reviewing AI before it goes to a person. Consult round 18 (`docs/design/consults/round18-synthesis.md`) set the order, and the operator approved it: **213 authority, 214 no silent wait, 215 the loop outlives its questions and Resume goes, 217 the governor** (the Auxiliary Reviewer, a closed menu, the framework executes). Consult round 19 (`round19-synthesis.md`), approved the same day, put **216 a dispute ends inside the session** before it: the primary says what would settle a finding it upholds, the Auxiliary chooses between that and the author's position at the round that upheld it rather than at the cap, and a ruling is an instruction to the author and never a terminal.
>
> **213 landed** (`df323d40`, closed `2ab10f2a`): the typed `session declare` is gone, so a release is decided in one place; `dabbler session hold-release` (a person's, one way, an amendment the close reads out); the three verbs that are a person's -- `cancel --force`, `close --force`, `hold-release` -- ask what is THERE (`personIsPresent`: the extension's click, or an interactive terminal with no engine marker) and refuse in one sentence; every engine the extension or the framework starts carries `DABBLER_ENGINE_TERMINAL`; every way on a stop prints is held to the CLI's own tables by one test, and a cancel with no number means the session in flight; the loop reads the ledger at every phase, in the mailbox poll, in the job poll and before the add and the push, and ends with a `done` that says who cancelled and why; `session next` refuses under a live loop; `report` takes the session in flight only and never says `session start`. Proof by failure injection: `docs/uat/uat-lifecycle-authority.md`. Readings: `docs/design/engine-environment-markers.md`.
>
> **Verification took five rounds, and the rounds were right.** Rounds 1-3 each re-raised one hole -- an engine nobody has measured, launched by hand, reads as a person -- and each fix was better than the last: a marker list, then a gate on the waiter (removed: it stopped an engine taking part, not typing a verb), then the positive rule. The proof's own "person" had been an AI with its markers stripped, and the final bundle refuses exactly that command. Round 4 named the residual (an unmeasured engine whose tool ALSO gives commands a real terminal); that is caller authentication, the design page says so, it was disputed with evidence and round 5 accepted it.
>
> **Found while driving 213, owed to 214.** (1) Round 4's first attempt stopped the loop with *"'gpt-5.6-terra' is OpenAI's, and so is the authoring model"* -- false. `verify`'s one retry excludes the provider that failed (`verify/rounds.ts`, `dispatchReview`), so a transient seat failure became an exclusion of the CHOSEN reviewer, the original error was written nowhere, and the stop said only the operator could resolve it. Running the loop again was the whole cure. (2) The installed waiter printed the idle `done` instead of the session's own at the close: the ledger moves a moment before the loop writes it. 3.14.0's waiter gives a session that left flight ten polls of grace. (3) Left as found, a reviewer's nit four rounds running: no test cancels in the exact window between the land's readiness judgment and its `git add`; it is read by the same `refuseIfEnded` as the phase boundary, and a deterministic test wants a seam or a git hook.
>
> **The second beta test's record came back and is read into 214's plan** (`betatest2-run.json`, `betatest2-supervision.json`): the loop waited six hours on the AUTHOR's dispositions with nothing recording whether a waiter was listening; two loops ran at once for five hours, because a loop waiting on an answer never reads the lease; the false vendor conflict above happened there too; and two stops were checks that machine could not spawn. The operator proves a build on the personal machine first and beta-tests on the work computer only after that.
>
> **Still unknown from the work computer:** for the first test, its `dabbler.release`, the task-declaration entry's `releasable`/`holdReason`, `cancelledReason` vs `forceClosed`, the engine, `dabbler version`.
...
> | session | what | state |
> | --- | --- | --- |
> | 173 | A source file's tests are the test file named after it. A suite declares `test_name` (`{name}Tests.cs`, `{name}Test.java`, `{name}.test.ts`) and `select`, with `{paths}` or `{names}` joined by `select_separator`; the selector in `checks.ts` reads names and nothing else, and `testing.selection.rules`/`repo_wide` are read and ignored (`session start` says so) with `check-selection-map.ts`, `affected.preverifyGate` and this repository's rules deleted; `smoke` stays. Bootstrap writes .NET (`dotnet test --filter {names}`, separator `\|`) and Maven (`-Dtest={names} -Dsurefire.failIfNoSpecifiedTests=false`, test root `.`). After a step's checks pass the driver runs each `select` suite's named tests and refuses a red run as `check-failed`; the round's task block lists changed source files with no named test. At the close `runsWholeAtClose` runs a suite whole unless it declares `select` and its last whole run in an EARLIER session took longer than the larger of 60 s and 5% of the median of the last five closed sessions; past that `test-evidence run --stage final-targeted` runs the named tests plus every test under a project that references a changed one (`projectGraph`), bound to the tree, and an empty selection records `none-selected` and runs nothing; freshness, the land and the Work Explorer read the owed stage. A releasing session runs whole every suite whose run of record was `final-targeted` (none-selected included) before it packages, and a red whole run holds the release through `releasabilityOf` and the session closes. The plan ask, the managed body (149 of its 150 lines), README, quick start, schema reference and driving-a-session say so | CLOSED VERIFIED r3 (r1: an empty per-suite selection ran the whole suite under `final-targeted` -- now `none-selected`, nothing runs; r2: that record waived the whole run before a release -- now it still owes one), landed `ea058172`, closed `c8eb6054`. **`vsix-v3.1.0` tagged by the publish phase, on origin** |
>
> **174 is next** (what planning asks, and when a session releases).
>
> **Left as found.** The .NET bare-name OR filter was probed on a scratch xUnit project (SDK 11 preview) and runs exactly the named classes; the Maven form and its `.` test root have not met a real reactor yet -- 175's walk is where they do. `test-evidence record` cannot write `final-targeted` by hand (it needs the command `record` refuses), which is the point. The run-event/run-projection `stage` enums are the check executor's vocabulary and did not change. walk-session's check milestone now refuses through the named test, so the step's own check refusal is no longer walked on its own. This repository's three suites ran whole at this close (34 s and 22 s, within 158 s).
>
> **Traps met.** The managed body has a 150-line budget (`bootstrap.test.ts`, "the instruction files"): the first wording made it 154. Editing a schema's description regenerates `src/generated/*`, and the compile control refuses stale types. A walk's step has three refusals in all: a new refusal milestone replaces one rather than adding one. Rebuild the router before reporting the last step (and each fix step) so verification, the run of record and the publish run the code under review; `dist` is gitignored, so the tree does not move. `version.json` is the one version; `stamp-version.ts` writes both manifests and the lock file. The whole-or-targeted decision reads only earlier sessions' whole runs, so a pre-release whole run cannot change, at the close, what the run of record owed.
...
>
> **Decided the same day** (`docs/operator-decisions.md`, consult round 15): with 169 pushed as the restore point, the focused module folder, in-repository packages and `docs/modules.yaml` are to be DELETED — project references for .NET and Maven, the Solution Explorer read from the build files; releasing becomes a project-level setting defaulting to on request (Ship by Default from the solution's context menu; this repository ships by default); planning asks the production split with the shop default application → API → database; tests are named after what they test, and the whole suite runs at a session's end while it costs at most about 5% of the session. **Planned as sessions 170–175** (`docs/sessions/session-plan.md`): 170 one checkout (focused checkout, grants, exposure and policy deleted; held), 171 project references (in-repo packages, candidate job, pins and contract pages deleted; held), 172 the build files are the solution (`projectGraph.ts` replaces `modules.ts` and `impact.ts`; Solution Explorer from `.slnx`/`.csproj`/`pom.xml`; 3.0.0), 173 tests named after what they test (the 5% rule, `final-targeted`, whole suite before a release), 174 the release setting (`dabbler.release`, on request by default, Ship by Default on the solution row) and planning's production split and handoff artifacts, 175 the timed walk (.NET CSV tutorial + a Maven reactor). **170 is next.**
>
...
> | session | what | state |
> | --- | --- | --- |
> | 164 | `stepChangedPaths` subtracts the candidate's paths as written; `ensureRootFiles` writes the .NET root files on the first `contract: package` module before any project, single-module manifest included, and .NET's `writeCentralPin` refuses absent `Directory.Packages.props` naming `modules create`; `modules create` writes a package module's notes page and the candidate's refusal names `module contract` only for the designed seam; `ensureFolderFeed` before the push and before the candidate's pack; `FreshnessFacts.driven` drops the three recipe sentences and `test-evidence record` prints none while driven; `workBegunRefusal` shared by `start` and `declare`, stop kind `tree` | CLOSED VERIFIED at round 2 (round 1: one blocking finding, fixed; verifier gpt-5.6-terra), landed `70635312`, closed `c812b931`. **Not releasable, as planned**: 165 carries the walk and the release |
>
> **Session 165 is planned and next.** `nextSession` is 165. Whole session
> 15:31 → 16:19 local, six steps plus one fix step, every report accepted
> first time; one plan amendment (step 2's files gained `cli.test.ts`,
> because two candidate tests seeded a .NET solution without the pin file).
> No rebaseline, nothing owed.
>
> **Round 1's blocking finding was right.** The plan said CPM is born with
> the FIRST package module, and `ensureRootFiles` returned null for a
> single-module manifest; the fix applies the package-contract fallback
> before the `multi` check, because a module whose contract is the package
> has declared the siblings that will consume it. The notes template also
> named a surface page a package contract never has; it now names it only
> where the contract has one.
>
> **The verifier's nits, for 165's small-things step.** (1) `driven` is
> read from `driver/run.json`, so a hand-driven in-flight session still
> gets the recipe; the plan's words were "while a session is in flight" and
> the code's are "while driven" -- decide which is meant. (2) The
> provenance test feeds `stepChangedPaths` a literal set; no test drives
> `Driver.judge` over a real candidate record. (3) No test runs `modules
> create --contract package` on a fresh repository end to end and asserts
> both the pin file and the notes page (the CLI test asserts the page; the
> ecosystem test asserts the file through `ensureRootFiles`).
>
> **Watch for.** Every `modules create` defaults `contract: package`
> (a package id is always defaulted), so the second module of any .NET
> solution now gets the root build files and every package module a
> notes page -- the sample's next walk sees both. `renderStop` returns
> `happened`, not `what`. `gitAnswers` replaces the whole table and
> matches by prefix.

> ## SESSION 163 CLOSED, 2026-09-13 — the questions nobody needs to answer
>
> | session | what | state |
> | --- | --- | --- |
> | 163 | `rebaseline` records and does not ask; `--approver` removed from `plan amend`, `verify reopen` and `withdraw-release`, the row's `by` read from the session's orchestrator block; a spent report is no report and a pull-mode `next` with nothing new reprints the instruction for free; amends and reopens folded into `activity-log.json` and rendered in the work plan; the plan ask names `plan amend --step`; session 1's template reads the brief from the prompt or `docs/planning/brief.md` and reports `blocked` without one; a triaged stop prints its proposal; the close prints one line per repair and per amend | CLOSED VERIFIED at round 1 (three nits, no findings; verifier gpt-5.6-terra), landed `f2bea15e`, closed `73484756`. **Not releasable, as planned**: 165 carries the release |
>
> **Session 164 is planned and next.** `nextSession` is 164. Whole session
> 14:11 → 15:02 local, seven steps, every report accepted first time; one
> report refused on `report-seq` when the report verb read a `--notes`
> value beginning with `--approver` as a flag and the stale report was
> judged -- the very defect step 3 then fixed. No amendments, no
> rebaseline, nothing owed.
>
> **What changed, in one place each.** `settleRepairSignoffs`
> (`owedDecisions.ts`) supersedes a standing `repair-outside-a-step-*`
> row once at the close; `whoIsWorking` (`progress.ts`) renders the
> orchestrator block as `claude-code (anthropic, claude-fable-5-1)` and
> is what the three verbs write into `by`; the two schemas renamed
> `approver` to `by` and the generated types followed; `reportIsSpent`
> (`drive.ts`) plus a pull-mode short-circuit that throws `Awaiting` with
> the outstanding instruction; `recordAmendment` / `amendmentEntries` /
> `amendmentLine` (`writers.ts`) with a new activity-log kind
> `amendment`; `renderAmendmentProposal` (`driver.ts`) is the one
> rendering the owed row and the stop share; `steppedOverLines`
> (`session.ts`) is what the close prints.
>
> **The verifier's three nits, for 165's small-things step.** (1)
> `verify reopen --approver=someone` (the `=` form) reaches the parser and
> is refused as an unrecognised argument, not with the "is gone" sentence
> -- check the parsed map as `sessionVerb` does. (2) The proposal renders
> `--checks-file <a file holding> [json]`, which is not literally typeable;
> render one path placeholder and state the JSON to write there. (3) No
> test drives a real close after a real `rebaseline` and a real `plan
> amend`; `steppedOverLines` is unit-tested and the walk reads one
> amendment off `close.log`. Also from the verifier: no test reads
> `amendments.jsonl`'s `by` after a CLI `plan amend`.
>
> **Watch for.** The close runs as a JOB: its output is in
> `.dabbler/runs/s<N>/driver/jobs/close.log`, not on `next`'s stderr. A
> job's status file is `{"exit", "ended_at"}` -- there is no `state`
> field; a poller that waits for one waits forever. `session report
> --notes` reads a value beginning with `--` as a flag.

> ## SESSION 162 CLOSED, 2026-09-13 — what the developer reads and sees, the walk in the sample, and 2.5.0 published
>
> | session | what | state |
> | --- | --- | --- |
> | 162 | the README page before a first focused session and the contract modes in `modules create --help`; the declaration refusal that names `.vscode/settings.json` and `configure --mine`; a dash on an inapplicable gate; *Start Session in a New Window* and *Start Focused Session in a New Window*; "2 installed, none chosen"; 2.5.0; and the walk that drove the sample's sessions 3 and 4 through the built VSIX | CLOSED VERIFIED at round 1 (no findings; verifier gpt-5.6-terra), landed `e4c2eca9`, closed `642b9ffe`. **RELEASED: `vsix-v2.5.0` is on origin and CI's publish job was green at 16:17Z**; `dabbler release --verify-install` still read 2.4.1 a minute later (the gallery lags about ten minutes, as in 160) |
>
> **Nothing is planned next: the 160–162 block is complete** (`nextSession`
> is null), and the next plan starts from the owed question below. Seven steps, every report accepted
> first time; three plan amendments (the walk step's files and checks, and
> a split of two extension test files that poison each other in one mocha
> process — pre-existing). The publication decision was answered by the
> operator: the auto-mode classifier refuses the engine that outward act.
>
> **The walk found five router defects and fixed each with its test,
> D279 for the second:** `packaging --dry-run` refused mid-session in a
> releasable session; the candidate demanded `{version}` in a pack the
> publish refuses if it names one (the candidate now takes the default
> pack); modules with no project file were refused as candidates (skipped
> now, and the land expects none of them — `hasProjectFile`); a relative
> folder feed was "invalid" to `dotnet nuget push` (made absolute for the
> tool); and the stop toast's first sentence was the kind's generic line
> (`renderStop` puts the stop's own words first; `declare` hands its
> refusal to the driver through `onRefusal`). Readings 1–17 are in
> `docs/uat/uat-csv-parser-walk.md`.
>
> **OWED, and the next plan's first question.** A solution-wide suite
> (`dotnet test CsvParser.sln`) cannot be the run of record in a focused
> checkout: the solution file lists the sibling's projects (MSB3202).
> Per-module suites belong in `dabbler.yaml`, which a focused session's
> policy does not list; and a notes file under every module's
> `sharedFiles` makes every module a changed module, so every module's
> candidate and suite run. Three defensible rules colliding in the one
> repository shape the framework is for. The sample's session 4 waits on
> `module-grant:person` (deny recommended) in
> `D:\Projects\csv-parser.csv-deserializer`; its log runs to entry 23.
>
> **Watch for.** An interactive engine that *Start Session* opens survives
> its window closing (VS Code persistent terminals) and can start the next
> session on its own — it did, at 09:10. The refresh terminal's tab reads
> "Code" (the editor's title template, not a line here). A `package`
> contract has no scaffold: the stop offers `module contract <slug>`,
> which refuses for that mode. `commandFlows.test.ts` fails three tests
> when run after `workExplorerTreeModel.test.ts` in one process.

> ## SESSION 161 CLOSED, 2026-09-13 — the declaration, the global session, and the stop
>
> | session | what | state |
> | --- | --- | --- |
> | 161 | the plan's reason forwarded to the declaration, no policy for a global session, `Scope:` wins anywhere and a `Module:` line beside it attributes, the stop toast carries the reason | CLOSED VERIFIED at round 1 (no findings; verifier gpt-5.6-terra), landed `aabea2f2`, closed `0df67475`. **Not releasable, as planned**: 162 carries the release (minor, 2.5.0) and the walk that proves the two together |
>
> **Session 162 is planned and next.** `nextSession` is 162. Whole session
> 06:56 → 07:14 local, four steps, every report accepted first time; one
> plan amendment (below).
>
> **What was wrong, and what changed.** (1) `phasePlan` accepted a
> cross-module plan through `judgeWorkPlanModules`, which read
> `plan.reason`, then called `declare` without it, and `declare` ran the
> same judge and refused — every cross-module session driven through
> `session next` stopped at its declaration and every retry re-read the
> stored plan. One line: `reason: plan.reason ?? null`. (2) `declare` wrote
> a policy for a GLOBAL session whenever modules were named, so the first
> step carried a focused-session `scope` and the sentence "the other
> modules are here as packages and contract folders, not source", which in
> the repository itself is false. Now a global session (multi-module, no
> checkout on its row — the same test the judge is handed) gets no policy;
> the modules it names are what it is about. (3) `kindOfSection` read the
> FIRST of `Module:`/`Scope:`; now `Scope: whole repository` wins wherever it
> appears, a `Module: <slug>` beside it is carried as the module the
> session is about, the projected row carries `module` on such a global
> row, and the Work Explorer's `moduleOf` files by `module` for either kind
> while `sessionsHere` keeps a focused checkout to what RUNS there
> (`kind === "focused"`). The bootstrap template's two sentences and a new
> two-row README table teach both lines. (4) `offerDecision`'s toast body is
> the question and, on its own line, the first sentence of `determined`.
>
> **The one amendment.** `doc-paths.mjs README.md` refuses two mentions
> that predate this session (`docs/modules.yaml` and a `rounds.jsonl`
> template path), so that check came off step 3, and the projection schema
> — which said `module` is "present exactly when kind is focused" — and its
> generated type were added to the step with `check-types-fresh` as the
> control. Nothing else needed amending.
>
> **Watch for in 162.** The focused-session policy test in `policy.test.ts`
> now stamps a checkout on its row with `recordSessionCheckout` (path = the
> sandbox root) — that is what makes a session focused for `declare`, and a
> test that registers with no checkout in a multi-module sandbox is a
> GLOBAL session and gets no policy. `judgeSessionKind` was NOT changed:
> `--focused` on an attributed-global section still says "no `Module:`
> line" and asks for `--module`; left alone as beyond "attributes and
> nothing more".
