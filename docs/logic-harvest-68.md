...
## The model was NOT adopted, and this is where it lives instead
...
**It is not adopted.** `packages/router/lifecycle.json` was written, held by a
control in `drive.test.ts`, and then deleted — and the reason is worth more
than the file was.

Holding it *as a source* means the control fails whenever the code or the model
loses an edge, which needs every real transition declared **and** every
declared transition observed. The suite drives 21 of the machine's 38 phase
edges. The other 17 are error, retry, refusal and publication routes that no
test reaches, and six of them are self-loops — a rejected report, a red
targeted suite — which the driver's phase lines cannot show at all: observing
those means instrumenting `setPhase` and the `Stop` constructor rather than
reading what the driver already says.

Two attempts to land less than that were refused by the cross-provider
verifier, correctly and in the plan's own words:
...
2. **deleting the unexercised transitions** so the declared and driven sets
   ...
   named `lifecycle.json` in the router package that reads as authoritative
   while nearly half of the real machine is outside it.

Both are the failure the operator's own rule names: *a hand-maintained diagram
of a state machine is worse than none, because it is trusted and wrong.* So the
whole machine is written down **here**, in a document that says what it is.

> **This is a snapshot, not a source.** It was true of `drive.ts` on
> 2026-09-01, after session 68. Nothing holds it to the code and nothing should
> be decided from it without reading `drive.ts` — which is exactly why it is
> not a `.json` file in the package.

**What adopting it would take**, for the session that wants to: instrument
`setPhase` and the `Stop` constructor so every transition — self-loops
included — is observable rather than inferred from a log line; add tests for
the 17 unexercised edges (D33's unrunnable adapter, D10's malformed suite
declaration, D27's successful publication, D18's refused dispute, and the
rest); then the model lands with a control that fails in both directions. That
is a session, not a step, and it is the honest price of a trusted model.

### The 21 edges this repository's suite drives

Measured, not asserted: a whole run of `drive.test.ts` takes exactly these.

| id | edge | trigger |
|---|---|---|
| D2 | plan → steps | work plan accepted and declaration made |
| D6 | steps → STOPPED(blocked) | `report.status == "blocked"` |
| D7 | steps → preverify | every plan step accepted |
| D9 | preverify → verify | every expensive suite's targeted run is green and recorded |
| D11 | verify → run-of-record | `dabbler verify` exits 0 |
| D12 | verify → dispositions | `dabbler verify` exits 4 (a blocking round was recorded) |
| D13 | verify → STOPPED(verification) | verify refuses for a reason the driver cannot answer, or terminates UNRESOLVED at the cap (exit 5); the stop carries verify's own words |
| D16 | dispositions → fix | at least one disposition is `fix` |
| D20 | fix → preverify | the fix step's report is accepted |
| D21 | run-of-record → land | every expensive suite's full run exits 0 |
| D22 | run-of-record → preverify | a full suite fails; a `fix-run-of-record` step is asked and accepted |
| D24 | land → publish | committed and pushed, or `.dabbler/local-only` present |
| D25 | land → STOPPED(land) | the commit failed, there is no remote, or the push was refused |
| D26 | publish → close | the session's DECLARATION says it is not releasable |
| D28 | publish → STOPPED(publish) | packaging exits non-zero (refused or failed) |
| D29 | close → complete | `dabbler session close` exits 0 |
| D31 | * → STOPPED(interrupted) | an interrupt request with `stop=true` is read |
| D32 | * → STOPPED(budget) | `run.invocations >= run.max_invocations` |
| D40 | verify → run-of-record | the ledger says no further round may open and the tree is still the verified one (session 68) |
| D41 | verify → preverify | verify refused for want of targeted evidence, which the preverify phase produces (session 67) |
| D43 | steps → STOPPED(rejected-thrice) | one step's report was refused three times |

### The 17 the suite does not drive

Each is real; none is exercised; and the gap between them and the twenty-one
above is a statement about this repository's tests, not about its code.

| id | edge | trigger |
|---|---|---|
| D1 | plan → plan | the work plan is absent or unreadable |
| D3 | plan → STOPPED(engine) | `declare()` refused |
| D4 | steps → steps | a step's report is refused |
| D5 | steps → steps | a step's report is accepted |
| D8 | preverify → preverify | the targeted suite is red; a `fix-tests` step is asked and accepted |
| D10 | preverify → STOPPED(tests) | `testing.suites`/`selection` malformed, the change set unmeasurable, or the run unrecordable |
| D14 | dispositions → verify | the latest round is null or non-blocking |
| D15 | dispositions → dispositions | the disposition set is refused |
| D17 | dispositions → verify | every disposition is `reject` |
| D18 | dispositions → STOPPED(verification) | `recordDispute` refused |
| D19 | fix → dispositions | the round and the disposition set are out of step |
| D23 | run-of-record → STOPPED(tests) | the run could not be recorded, or its job vanished |
| D27 | publish → close | `dabbler packaging` published |
| D30 | close → STOPPED(close) | the close exited non-zero, or its job vanished |
| D33 | * → STOPPED(engine) | the adapter could not run the engine, or the tree could not be snapshotted |
| D35 | complete → complete | a further `next` after the close, under the pull |
| D42 | plan → STOPPED(rejected-thrice) | no work plan after three instructions |

Six of them are self-loops — D1, D4, D5, D8, D15, D35 — which a phase line
cannot show at all, because the phase does not change. Those are the ones that
make instrumentation unavoidable rather than optional.

### Movements that are not phase edges

Four, and they are worth keeping because three of them are findings:

- **A job still running** (harvest D36). *No wall-clock bound anywhere*:
  `started_at` is recorded and never compared to now, so a reused pid reads as
  `running` forever — under the pull an endless sequence of `wait`
  instructions. Session 68 did not close this; what it added is the watcher's
  `job-outstanding` rule, which SAYS so without ending anything. The ceiling is
  F7, carried.
- **An interrupted invocation re-issued** (D38), push only.
- **The resume out of a stop** (D34). *The only edge out of `STOPPED`*, and its
  actor is a person: no timer, no retry, no escalation takes it, and the owed
  decision it raises is `value-tradeoff`, which does not block a close.
- **A repeated stop reclassified as a deadlock** (D39), push only, once per
  impasse — and since sessions 67 and 68 the reasons it compares are verify's
  own words rather than one wrapper, so two different causes no longer read as
  one impasse.

### Halted, being repaired

The state session 66 walked into now has an edge out — `dabbler session
rebaseline`, built in this session — and it is deliberately **not** a phase
edge: the phase and the stop are untouched, which is why it is described here
rather than declared in the model. It records the absorbed paths and the reason
to `repairs.jsonl`, moves `baseline_tree`, and raises an
`accountability-signoff` decision. `session interrupt` also queues against a
stopped run (session 63), so the state has a Send channel as well.
