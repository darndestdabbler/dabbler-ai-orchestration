...
## And the blind spot, which is not the flake but is real

`terminateTree` in `packages/router/src/checks.ts` was going to be left
alone, on the grounds that the measurement acquitted it and that a guard
with no incident behind it is the kind this repository has spent sessions
removing. Verification round 1 disagreed, and it was right: the argument
above is about *this flake*, and it says nothing about the separate fact
that a kill which did not happen and a kill which worked were reported
identically. A job is ended from a router process that never held it, so
the pid on the record is the only handle there is — a false success there
has nobody else to catch it, which is the same shape as the 38-hour-old
trees found on the operator's machine on 2026-09-02.

So it reports now. On Windows a `taskkill` that could not be spawned and one
that ran and refused are two different answers, and neither is success; a
`taskkill` naming a pid that had already gone *is* success, because that is
the outcome asked for. On POSIX an `ESRCH` is the same success and any other
refusal is not. `endJob` retries once — the condition that stops a spawn is
a machine momentarily out of process slots, and that passes — and then ends
the runner directly, which leaves the runner's children behind and says so
rather than reporting a clean end it did not achieve.

The failure cannot be produced on demand: a machine that cannot spawn a
process cannot be asked for one. `setTreeKiller` is the seam, and the test
forces all three answers.
