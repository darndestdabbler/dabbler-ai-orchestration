...
/** The `loop-restarted` rows `supervision.jsonl` carries for one progress point. */
function restartsAt(repoRoot: string, sessionNumber: number, phase: string, seq: number): number {
  let text: string;
  try {
  ...
  for (const line of text.split("\n")) {
    try {
      ...
      if (row["event"] === "loop-restarted" && row["phase"] === phase && row["seq"] === seq) count += 1;
    } catch {
...
export function reviveLoop(
  ...
  const attempts = restartsAt(repoRoot, sessionNumber, run.phase, run.seq);
  if (attempts < LOOP_RESTARTS) {
    start(sessionsDir);
  ...
  const lock = acquireLockWithTimeout(sessionsDir, `waiter-crash/${process.pid}`);
  try {
    const current = readRun(repoRoot, sessionNumber);
    if (current === null || current.stop) return "stop";
    const log = relative(repoRoot, loopLogPath(repoRoot, sessionNumber)).split("\\").join("/");
    const entry = {
      kind: "crash" as const,
      code: null,
      reason:
        `the loop died at phase '${current.phase}' (seq ${current.seq}) after it was started ` +
    ...
    writeRun(repoRoot, sessionNumber, {
      ...current,
      stop: entry,
      stop_history: [...(current.stop_history ?? []), entry].slice(-STOP_HISTORY_CAP),
      resumed_from: null,
      updated_at: nowIso(),
    });
  } finally {
