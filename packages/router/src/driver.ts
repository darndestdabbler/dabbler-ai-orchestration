 ...
 *
 * The size is remembered per log path, in this process: the question is
 * "has it moved since I last looked", and a value on disk would answer a
...
function jobLogGrewSince(
  ...
  const seen = jobLogSizes.get(path);
  jobLogSizes.set(path, size);
