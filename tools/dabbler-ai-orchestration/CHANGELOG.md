...
- **A configuration a step leaves malformed is a step, not a crash.** A
  `dabbler.yaml` that no longer loads -- a YAML syntax error included, which
  is now refused naming the file -- becomes a `fix-configuration` step
  carrying the loader's message; repairing it sends the session back through
  the checks, verification and the suite.
