...
// the impact plan reached run, the session lands and closes with no
// candidate job, and the close gate demands the reached suites and no other.
...
const SEED: Record<string, string> = {
  ...
  "global.json": "{}\n",
  // The root build files a multi-module .NET solution has: the solution file
  // lists both projects, and the persister reaches the model by reference.
  "CsvParser.slnx":
    "<Solution>\n" +
...
describe("a module session's run of record", () => {
  it("runs the suites the plan reached and no other, and lands and closes with no candidate job", async () => {
    setProviderKeys();
    resetRouter();
    resetRuntimeMode();
    configure([VERIFIED]);
