...
import { type ImpactPlan, demandedByPlan, readCandidateRecord, readImpactPlan } from "./impact.ts";
import {
  isMachineStatePath,
  repoRelativePath,
  repoRootFor,
...
export function readVerificationFacts(sessionsDir: string): VerificationFacts {
  ...
  const candidate = new Set(readCandidateRecord(root, facts.current as number).paths);
