...
function detectMaven(root: string): Ecosystem | null {
  ...
  return mavenSuite(root);
}

function mavenSuite(root: string): Ecosystem {
  return {
    key: "maven",
...
function dotnetSuite(): Ecosystem {
  return {
    key: "dotnet",
    command: "dotnet test",
    runsWhole: true,
...
/**
 * The suite an ecosystem declares, for a caller that already knows which one
 * it is on.
 *
 * The detectors above read the repository root, and that is right for
 * bootstrap, which knows nothing else. It is wrong for the first pack: a
 * multi-module .NET solution keeps its projects under `modules/` and nothing
 * at its root, so root detection would answer nothing for exactly the
 * repository whose ecosystem the pack has just established from a module's
 * own files. This is the same declaration, asked by key instead.
 */
export function suiteForEcosystem(root: string, key: string): Ecosystem | null {
  if (key === "maven") return mavenSuite(root);
  if (key === "dotnet") return dotnetSuite();
  return null;
}
...
export function ensureRootFilesWithSuite(root: string, shape: SolutionShape): ScaffoldResult | null {
  ...
  const suite = key === null ? null : suiteForEcosystem(root, key);
  const declared = suite === null ? null : appendSuitesToProjectConfig(root, [suite]);
