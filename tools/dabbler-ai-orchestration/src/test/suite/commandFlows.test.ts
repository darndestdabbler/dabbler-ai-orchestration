...
interface FakeTerminal {
  options: { name: string; cwd: string; shellPath: string; shellArgs: string[] };
  shown: number;
...
suite("Start opens the person's own CLI", () => {
  test("opens the picked engine's CLI at the repository root, with the sentence, and launches no driver", async () => {
    ...
    assert.strictEqual(terminals[0].options.shellPath, "claude");
    assert.strictEqual(terminals[0].options.cwd, repository.root);
    assert.strictEqual(terminals[0].shown, 1);
    // Claude Code takes a positional prompt for an interactive session, so
    // the sentence is argv and nothing is typed.
    assert.match(terminals[0].options.shellArgs[0], /dabbler session next .*--engine claude-code/);
    ...
    assert.strictEqual(terminals[1].options.shellPath, "copilot");
    assert.deepStrictEqual(terminals[1].options.shellArgs, []);
    assert.strictEqual(terminals[1].sent.length, 1);
    assert.strictEqual(terminals[1].sent[0].addNewLine, false);
    assert.match(terminals[1].sent[0].text, /--model gpt-5-6-luna/);
    assert.deepStrictEqual(launcher.launched, []);
  });
