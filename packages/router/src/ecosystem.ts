 ...
 * .NET: central package management is born with the first package module,
 * because a solution that turns it on after its projects carry versioned
 * references breaks every one of them at restore (NU1008), and migrating
 * them afterwards is repair machinery. A Maven solution keeps waiting for
 * its first pom, because the parent POM lists the projects. A solution
 * whose modules are empty folders and declare no package gets nothing yet
 * and is told so, and a single-module solution gets nothing ever.
 */
