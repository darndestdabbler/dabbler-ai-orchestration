...
import { readInstruction, readReport, readRun } from "../src/driver.ts";
...
import { EXIT_OK, report } from "../src/session.ts";
...
describe("one session, walked from next to done", () => {
  it("goes through every phase once, in order, and leaves the record each one owns", async () => {
    ...
    // --- the work, and the report that matches it ---------------------------
    writeFileSync(join(repo, "src", "widget.py"), WIDGET_V2, "utf8");
    const right = await answerStep(sessionsDir, rejection.instruction?.seq ?? 0, "widget", [
      "src/widget.py",
    ]);
    ...
    assert.match(readFileSync(join(repo, "src", "widget.py"), "utf8"), /return 2/);
