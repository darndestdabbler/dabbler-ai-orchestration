...
export async function publish({ version, vsixFile, extensionId, pat }, deps = {}) {
  ...
  /**
   * Whether the gallery holds this version: `held`, `absent`, or `unknown`
   * when the gallery could not be read.
   *
   * **`unknown` is not `absent`, and the difference decides whether a second
   * upload may happen.** The outage that loses a release is correlated: the
   * upload times out because the gallery is unavailable, so the query that
   * would confirm whether it landed is unavailable for the same reason.
   * Reading that silence as "not published" is how one upload becomes two,
   * and the second is refused for a version that already exists -- turning a
   * release that succeeded into a red workflow, which is the exact failure
   * this script exists to end.
   */
  const holding = async (when) => {
    try {
      return galleryHasVersion(await gallery(extensionId), version) ? "held" : "absent";
    } catch (error) {
  ...
  const before = await holding("before publishing");
  if (before === "held") {
    log(`${extensionId} ${version} is already in the Marketplace; nothing to publish.`);
    return 0;
  }
  // An unreadable gallery BEFORE the first upload is safe to proceed past:
  // nothing has been sent, so there is nothing that could be duplicated.
  ...
  for (let n = 1; n <= ATTEMPTS; n++) {
    ...
    // Something was sent. From here a second upload requires the gallery to
    // say, in so many words, that the version is not there.
    let confirmed = await holding("after a transient failure");
    for (let ask = 1; confirmed === "unknown" && ask < ATTEMPTS; ask++) {
      const pause = backoffMs(ask);
      log(`the gallery could not be read; waiting ${Math.round(pause / 1000)}s to ask again.`);
      await wait(pause);
      confirmed = await holding("after a transient failure");
    }
    if (confirmed === "held") {
    ...
    if (confirmed === "unknown") {
      log(
        "::error::an upload was sent and the Marketplace could not be asked whether it landed; " +
          "nothing further was sent. Re-run this job once the Marketplace answers: it will " +
          "publish if the version is absent and report success if it is already there.",
      );
      return 1;
    }
    if (n === ATTEMPTS) break;
    const pause = backoffMs(n);
    log(`the gallery confirms ${version} is absent; waiting ${Math.round(pause / 1000)}s before attempt ${n + 1}.`);
    await wait(pause);
  }
