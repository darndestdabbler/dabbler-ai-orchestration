...
| 2b | `dabbler bootstrap` | framework | Set Up New Project runs bootstrap naming no transport, deliberately: setting up one project is not a statement about how it routes. Bootstrap no longer writes the operator's environment at all, so there is no opt-out flag to pass. |
...
| 6 | `dabbler session start --sessions-dir … --engine … --provider … [--model …]` | UI | **Start Session** (`tools/dabbler-ai-orchestration/src/commands/sessionCommands.ts`) picks the engine and, for a seat, the model, opens the engine's CLI at the repository root, and hands it the sentence to run exactly this line once. |
| 6 | `dabbler session next --sessions-dir …`, repeated until `done` | framework | The engine calls it; a person never does. The walkthrough's warning that a `wait` means *call next again yourself* is addressed to the engine. Nothing enforces it inside the engine's turn — the Stop hook that once held a Claude Code turn on a past-due wait was removed in session 126, because it blocked every end of turn in the repository — and what tells a person the call is not being made is the Dabbler terminal's silence watcher and the Work Explorer's attention row. |
