...
import { readInstruction, writeInstruction } from "../src/driver.ts";
...
describe("the sentinel band", () => {
  ...
  it("instruction-execution: a report against a stale attempt is refused", async () => {
    // Two drivers wrote one run on 2026-09-02 and phases were skipped
    // silently; the refusal below is the boundary that specimen proved
    // necessary. A report must answer the OUTSTANDING instruction --
    // yesterday's seq is a stale attempt, and it may not advance state.
    const { repo, sessionsDir } = makeStateDirs();
    ...
    writeInstruction(repo, 1, {
      ...
      answer_command: "dabbler session report --seq 5",
    });
    const stale = await captured(async () =>
      report(sessionsDir, { seq: 4, stepId: "widget", status: "done", files: [], notes: "stale attempt" }),
    );
