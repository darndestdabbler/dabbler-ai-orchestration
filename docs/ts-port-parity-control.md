...
and the same verb, writes **the same bytes** the Python router writes. This
document specifies the check that says so.
...
| **moved-machine** | The disputed shape, cloned to a second directory through `origin` so the round's `completion_tree` object is absent from the clone's store; the anchor ref fetched. | `effective_baseline`, the reanchor path and its refusals, `ledger.append_round`'s anchoring. |
...
- `seat_cost`'s printed result, as the one verb whose output is stdout.
