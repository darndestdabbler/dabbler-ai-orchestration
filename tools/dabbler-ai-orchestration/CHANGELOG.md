...
- **A stale model record brings itself up to date when a session starts.**
  Enumerating a vendor's models is a metadata request that bills nothing, so
  a record older than its declared 24 hours is re-read before the work
  rather than reported and left. A vendor that cannot be reached leaves the
  record exactly as it was and says which one it was; nothing that costs
  money is ever in that path; and a repository that has never run discovery
