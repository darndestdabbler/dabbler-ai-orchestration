...
## Close-backstop rounds 5 and 6 — the class killed, not patched again
...
does not apply to them) refused the close twice, each with a new spelling
of the same class:

- **Round 5:** `pytest tests/sub/test_widget.py` loads
  `tests/fixtures/sample.json` through the **ancestor** directory `tests`,
  so the asset's own directory is not itself an ancestor. The
  loader-asset check compared directories for equality. *(It supplied an
  executable criterion — a `runpy` probe driving the function by path,
...
**Five rounds, five spellings, one class.** Each fix was correct and each
was immediately followed by a new spelling:
...
| 6 backstop | `go test ./...`, and `*_test.go` unrecognised | another language |

**So the rule was inverted rather than extended a sixth time.** A test
runner's scope is now **always the whole repository** — not because it
necessarily collects everything, but because *what it collects cannot be
determined from its argv*, and five rounds of evidence say every attempt
to infer it leaks. This is one rule instead of a growing table, it is
fail-closed, and it deleted the ancestor/loader-walk machinery the
previous two rounds had added (`is_loader_asset` survives only as a
classifier). `is_test_asset` also grew multi-language spellings
(`*_test.go`, `*_spec.rb`, `*Tests.cs`, `testdata/`, `__tests__/`, …).

The cost is only auto-closures that were never trustworthy: a criterion
whose result depends on tests the remediator just rewrote proves nothing
either way. **Criteria that drive product code by path keep precise
scoping and remain the auto-closable path** — and the template now says
so explicitly, so a verifier can pick a criterion that will actually
close. Template minted `session-verification-v6`.

**The honest reading of this sequence** — worth carrying into Session 4 —
is that the bound and the operator's adjudication were right, and that
what finally ended it was not another round but a *design change of a
different kind*: stop enumerating cases, invert the default. A loop that
keeps finding real defects is not necessarily converging.
