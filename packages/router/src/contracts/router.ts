...
export interface LedgerVerbs {
  ...
  /** Every session whose loop stopped without a clean verdict. */
 ...
 *
 * `progress` is the only verb with a typed payload today, because it is
 * the only one whose answer is a projection rather than a record write.
 * The rest print for a reader, and the reader is a person.
 */
