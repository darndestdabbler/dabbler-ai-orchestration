...
- **Codex.** Not installed on this machine. It is covered by
  `DABBLER_ENGINE_TERMINAL`, which row four above shows refused, wherever the
  extension opens its terminal or the framework starts it
  (`engines.test.ts` reads the marker out of a spawned engine's own
  environment). Launched by hand in a person's own terminal outside the
  editor it carries nothing -- which verification raised twice -- so that
  case is closed at the door instead: `dabbler session wait`, the AI's verb,
  refuses from an unmarked shell and says how the terminal is marked, so no
  shell that can take part in a session is one the person's verbs accept
  (`cli.test.ts`). `docs/design/engine-environment-markers.md` says what that
  is and is not.
