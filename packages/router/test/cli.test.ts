...
import { ENGINE_MARKERS } from "../src/session.ts";
...
describe("dabbler session, the whole surface", () => {
  ...
  it("lets only a marked shell wait, so every shell that can take part in a session is one a person's verbs refuse", async () => {
    // Verification twice re-raised one hole: an engine nobody has measured,
    // launched by hand outside the editor, carries no marker and read as a
    // person at the forced verbs. No vendor variable closes that, so the door
    // does: `session wait` is the AI's verb alone, and it does not open from
    // an unmarked shell. The refusal is the whole way on.
    const { sessionsDir } = makeAnsweredSandbox();
    const saved = ENGINE_MARKERS.map((name) => [name, process.env[name]] as const);
    try {
      for (const name of ENGINE_MARKERS) delete process.env[name];
      const unmarked = await run(() => sessionVerb(["wait", "--sessions-dir", sessionsDir]));
      assert.equal(unmarked.code, 3);
      assert.equal(unmarked.out, "");
      assert.match(unmarked.err, /`session wait` is the AI's verb/);
      assert.match(unmarked.err, /DABBLER_ENGINE_TERMINAL=1 <engine>/);
      assert.match(unmarked.err, /\$env:DABBLER_ENGINE_TERMINAL = "1"/);
      // Marked, it waits -- and on a repository with nothing in flight that is
      // the idle `done`, at once.
      process.env["DABBLER_ENGINE_TERMINAL"] = "1";
      const marked = await run(() => sessionVerb(["wait", "--sessions-dir", sessionsDir]));
      assert.equal(marked.code, 0, marked.err);
      assert.match(marked.out, /"kind": "done"/);
    } finally {
      for (const [key, value] of saved) {
        if (value === undefined) delete process.env[key];
        else process.env[key] = value;
