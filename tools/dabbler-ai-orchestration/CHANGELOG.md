...
- No AI ends a session by force, whichever engine it is. `session cancel
  --force` and `session close --force` are refused to Claude Code, Copilot
  CLI and any engine running in the terminal the extension opened for it
  or started by the framework, in the same words. A forced close used to
  ask nothing at all. `dabbler session wait` runs only from a shell marked
  as an AI's, so an engine launched by hand outside the editor is told, once,
  to set `DABBLER_ENGINE_TERMINAL=1` -- nothing inside the editor changes.
