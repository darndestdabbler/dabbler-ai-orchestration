...
## The last case, closed at the waiter
...
today -- would read there as a person. Verification raised exactly this,
twice, and a convention ("remember to set the variable") was not an answer.

The answer is a door rather than a list. **`dabbler session wait` is the AI's
verb and nobody else's, and it does not run from an unmarked shell.** Under a
mailbox loop an AI that cannot wait cannot receive an instruction, so it
cannot take part in a session at all; and every shell that CAN take part
carries a marker, which is precisely what `session cancel --force`, `session
close --force` and `session hold-release` refuse. That holds for an engine
nobody has measured and for one that renames its variables tomorrow. The
refusal is the whole way on, said once, to whoever is looking at that
terminal:

    PowerShell:   $env:DABBLER_ENGINE_TERMINAL = "1"; <engine>
    bash / zsh:   DABBLER_ENGINE_TERMINAL=1 <engine>

Nobody inside the editor ever meets it, and neither does Claude Code or
Copilot CLI anywhere: their own markers open the door.

What this is not is authentication. A command line cannot prove who typed
it, and a process that unsets its own environment is a person as far as any
of this can tell. It stops an AI acting on a sentence it misread -- which is
what happened -- and not an adversary; the guidance says, as well as the
guard enforces, that ending a session by force is never the engine's, and
the cancel a stop prints is worded as the operator's and offered on no stop
that is the engine's to clear. When Codex's own variables are measured they
join `ENGINE_MARKERS`, and a hand-launched Codex stops needing the variable
set for it.
