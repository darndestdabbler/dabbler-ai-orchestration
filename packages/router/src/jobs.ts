...
// Its own group on POSIX, so ending it reaches everything it forked; on
// Windows \`taskkill /T\` walks the tree without one.
...
// What the runner started, the runner ends: a signal it can observe ends
// the child's whole tree, and the close that follows writes the status as
// usual, with no code. A pid is never ended after its close -- the OS
// reuses them.
function endTree() {
