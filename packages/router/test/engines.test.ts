...
describe("the engine adapters", () => {
  ...
  it("has one measured argv per engine, continues after the first invocation, and refuses an engine it has none for", () => {
    ...
    const fresh = { first: true, resumeId: null };
    const again = { first: false, resumeId: "conv-1" };
    ...
    // The one seat with no measured resume-by-id, and it reports none.
    expect(copilot.argv(again, prompt).at(-1)).toBe("--continue");
