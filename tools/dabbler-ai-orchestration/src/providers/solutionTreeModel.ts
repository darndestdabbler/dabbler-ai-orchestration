...
const LOCATION_NOTE: Record<ExternalLocation, string> = {
  here: "",
  remote: "not cloned here",
  unknown: "nobody has said where this lives",
};
...
export function descriptorFor(
  ...
  switch (node.kind) {
    ...
    case "external": {
      ...
      if (location !== "here") bits.push(LOCATION_NOTE[location]);
      return {
        id: `external:${e.id}`,
        label: e.id,
        description: bits.join(" · "),
