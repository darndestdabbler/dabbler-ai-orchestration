...
export function attentionNodes(node: RepositoryNode): AttentionNode[] {
  ...
  if (repository.possiblyStalled && repository.currentSession !== null) {
    rows.push({
      kind: "attention",
      repository,
      subject: "stalled",
      label: "Nothing has been written for a while",
      // Deliberately not "stalled" or "stuck". It reports that the record
      ...
      detail:
        "The session is still in flight. This says the record stopped " +
        "moving, not that the work stopped.",
      urgent: false,
    });
  }
