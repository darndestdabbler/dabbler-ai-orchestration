...
//
// Four members carry the rules. `allowed` is the scope: what the session
...
// protection is a list of its own. `siblings` names every other module
...
import { MACHINE_DIRNAME, RUNS_DIRNAME, atomicWriteJson, nowIso } from "./journal.ts";
 ...
 * module manifest, the run record and the projection. Not the whole of
 * `.dabbler/`: its `scratch` directory is where the framework tells the
 * engine to write its answers, and a wall in front of the answer is an
 * impasse.
 */
...
/** Allow or deny one call against the policy, with the reason. */
