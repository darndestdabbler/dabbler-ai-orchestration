# Cross-provider verification — Set 13 Session 1, ROUND 3

## Round 2 verdict

`ISSUES_FOUND` — see `session-001.md` (round 2). Critical PASS;
3 Major + 1 Minor remained. All addressed in round 3.

## Round 3 fixes

### Major: arbitrary-path discipline scope-creep

**Issue (round 2):** the absolute-path discipline was scoped only to
Step 3, leaving Step 2 fast-path checks and Step 8 writes
under-specified for arbitrary-path mode.

**Fix:** State B sub-paths (2) and (3) now explicitly say the
discipline applies through Step 8 — every read, glob, command, and
write must use the absolute target path until end-of-bootstrap. Step
3's tooling note is reinforced. Step 8 has a new "target-path
reminder" paragraph at the top requiring writes to use the absolute
target path or, if write tools are workspace-scoped, stop and ask
for workspace reopening.

```
[Step 1 sub-path (2) — replaced with two-outcome structure]

- **If (2):** ask for the absolute path. Then check whether your
  tooling can read files at arbitrary paths. Two outcomes:
  - **Workspace-scoped tools:** ask the human to reopen VS Code at
    the target. Wait for it. Once reopened, run in normal workspace
    mode.
  - **Arbitrary-path-capable tools:** the absolute-path discipline
    applies through Step 8 — every read, glob, command, and write
    must use the explicit absolute path. If at any point uncertain
    whether write tools will respect the target path, stop and ask
    for workspace reopening.

[Step 8 — new target-path reminder paragraph at top]

**Target-path reminder:** if you've been operating against an
arbitrary path, every write in this step must use the absolute
target path established in Step 1. If your write tools are
workspace-scoped and the workspace was never reopened at the
target, **stop here** — do not try to write outside the active
workspace.
```

### Major: clone destination ambiguity in no-workspace mode

**Issue (round 2):** State B sub-path (3) said "sensibly-named local
folder" with default "sibling of the current folder," but in
no-workspace mode there is no current folder to be a sibling of —
ambiguous resolution.

**Fix:** Sub-path (3) now requires an absolute destination path
(with platform-specific examples) before any approval prompt, and
the approval prompt echoes the fully-resolved absolute clone target
back to the human.

```
[Step 1 sub-path (3) — replaced]

- **If (3):** ask for the URL **and** for an **absolute** destination
  path (e.g., `C:\Users\<name>\projects\<repo-name>` on Windows,
  `/home/<name>/projects/<repo-name>` on Linux/macOS). Don't accept a
  relative or "sensibly-named" answer. Resolve the absolute clone
  target fully before asking for approval. The approval prompt
  echoes it back:

  > **I'd like to run `git clone <url> <absolute-destination>` so I
  > can canvas the project. The clone target is the fully-resolved
  > absolute path `<echo-here>`. ... Approve the clone?**
```

### Major: `threshold_scope` missing-field compatibility rule

**Issue (round 2):** `threshold_scope` was added to the schema but
the doc didn't define behavior for older `budget.yaml` files that
predate the field.

**Fix:** Added explicit compatibility rule to both the bootstrap
doc's schema reference AND the workflow-doc section. Older / missing
fields default to safe values: `threshold_scope` → `project-lifetime`,
`verification_method` → `api`. Readers must apply defaults rather
than erroring.

```
[bootstrap doc threshold_scope field reference — appended]

**Compatibility rule:** if `threshold_scope` is absent from a
`budget.yaml` file (older or hand-authored without it), readers
must treat it as `project-lifetime` — that's the safe default and
matches the original bootstrap-flow behavior.

[workflow-doc Cost-budgeted-modes section — new paragraph after the
budget.yaml schema reference]
...
absent; `verification_method` → `api` if absent. Readers (current
and future enforcement code) must apply these defaults rather than
erroring on a missing field.
```

### Minor: residual tier-count / example drift

**Issue (round 2):** workflow-doc said "Three tiers, with two
sub-options under the zero tier" — should be Four. Step 8 example
said "$25 threshold, limited-budget mode" — should be middle-tier.

**Fix (both):**

```
[workflow-doc Cost-budgeted-modes section — fixed]
"Four tiers, with two sub-options under the zero tier"

[bootstrap doc Step 8 example — fixed]
✓ 2. Saved ai_router/budget.yaml ($25 threshold, middle-tier mode)
```

## Final state of the two deliverables

The full updated content of both docs follows for round 3 review.

---

## Full content of `docs/adoption-bootstrap.md` (round 3)
...
- **If (3):** ask for the URL **and** for an **absolute** destination path where the clone should live (e.g., `C:\Users\<name>\projects\<repo-name>` on Windows, `/home/<name>/projects/<repo-name>` on Linux/macOS). Don't accept a relative or "sensibly-named" answer — the working directory of your tool environment is opaque, and a relative path can land in an unintended location. Resolve the absolute clone target fully before asking for approval. Then echo it back in the approval prompt:
  ...
  Wait for explicit approval (`yes` / `clone it` / etc.). On approval, run the clone with the absolute destination. After the clone completes, follow the same workspace-open vs. arbitrary-path-tools logic as sub-path (2) above.
...
## Updated `docs/ai-led-session-workflow.md` "Cost-budgeted verification modes" section (round 3)
...
## Verification request (round 3)

Re-evaluate the round-2 issues. Specifically:

1. **Arbitrary-path discipline.** Is the discipline now coherent from
   Step 1 through Step 8? Are there any remaining steps or operations
   that could trip in arbitrary-path mode?
2. **Clone destination.** Is requiring an absolute destination + echo
   in the approval prompt sufficient? Are there platform-specific
   gotchas the doc doesn't cover?
3. **`threshold_scope` compatibility.** Is the rule clear enough that
   future enforcement code would implement it correctly without
   ambiguity?
4. **Tier-count drift.** Are all instances now consistent on "four
   tiers"? Is the Step 8 example aligned?
5. **Anything new.** Any regressions or new issues from the round-3
   changes?
