...
${this.stopArtifacts()}`,
  ...
  private async phasePublish(): Promise<void> {
    ...
    if (code !== EXIT_OK) {
      ...
      //
      // **This is safe only because of the change above it.** A rewind
      // re-enters the loop, and a loop that can re-enter needs something
      // that notices it going nowhere. With a literal refusal every publish
      // stop was already a deadlock and none of them meant anything; with
      // the real refusal carried, a rewind that changes nothing arrives here
      // a second time with the same reason and IS classified a deadlock,
      // triaged, and put in front of a person. The two changes are one
      // change in two places, and neither is correct without the other.
