...
/**
 * Whether an open terminal is the one `spec` would open: the same name,
 ...
 * created with are what say where it was launched. The name is read from
 * those options too: a terminal whose shell never reaches process-ready, as
 * the loop's does not, is labelled from its executable instead.
 */
export function isEngineTerminalOf(
  ...
  if (options.name !== spec.name || options.shellPath !== spec.program || cwd === undefined) return false;
