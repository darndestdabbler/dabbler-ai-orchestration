...
export function checkVerificationClean(sessionsDir: string): Check {
  ...
  const standing = judgeLatestRound(rounds, current, sessionsDir);
  ...
  return judgeTreeSinceRound(latest, changed, sessionsRel(root, sessionsDir), sessionsDir);
}
...
export function parsePorcelain(porcelain: string): PorcelainEntry[] {
  ...
  for (const line of porcelain.split("\n")) {
    ...
    path = unquotePorcelainPath(path.trim());
...
export function materialPaths(porcelain: string, setRel: string): string[] {
  ...
  for (const entry of parsePorcelain(porcelain)) {
    // Git spells every path with forward slashes; a backslash that reaches
    // here came from somewhere else and is read as a separator.
...
export function materialWorktreeChanges(sessionsDir: string): WorktreeChanges {
  ...
  if (status.error) return { paths: [], error: status.error };
  return { paths: materialPaths(status.text, sessionsRel(root, sessionsDir)), error: "" };
}
