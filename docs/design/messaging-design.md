...
**The framework and the AI wait on each other, and neither calls the other to
ask whether there is anything yet.**

- The framework posts an instruction (the existing instruction file) and waits
  for its report.
- The AI keeps a **background** waiter that waits until there is an instruction
  with no accepted report, prints it and exits, and then ends its turn, so its
  chat stays free for the operator. The waiter is a plain process checking a
  local file; the POC's checked its inbox folder twice a second, which calls no
  model and costs nothing. The engine CLI's own background-task machinery wakes
  the AI when the waiter exits. The AI does the work, answers with the report
  command, and re-arms the waiter.
- An instruction stays pending until its report is accepted. The waiter session
  177 builds consumes nothing, so a second waiter prints the same instruction,
  and an AI that dies mid-step loses nothing. **The POC's waiter did not work
  this way:** it advanced a cursor when it printed a message, so a waiter
  re-armed after a lost step would have skipped that message. That difference
  is deliberate in the build, and was not exercised by the POC.
