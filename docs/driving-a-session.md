...
Beside your CLI is a terminal named *Dabbler*. It shows what the framework
is doing while nobody is typing — the phase the run moved to, the
background job it started, and that job's own output as it arrives:
...
the count. From the walk, after `session interrupt --stop`:
