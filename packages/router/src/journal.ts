...
export function snapshotWorktreeTree(repoRoot: string): string | null {
  ...
  try {
    // The throwaway index starts as a copy of the real one, not as HEAD's
    // tree: in a focused checkout (a sparse cone) the real index carries
    // the skip-worktree bit on every entry outside the cone, and an index
    // read from HEAD does not -- so `add -A` over it would stage every
    // sibling's file as deleted, and a tree diff would report the whole
    // solution as this session's change. HEAD's tree is the fallback for a
    // repository with no index yet.
    const indexPath = runGit(repoRoot, ["rev-parse", "--git-path", "index"]).stdout.trim();
    let seeded = false;
    if (indexPath !== "") {
      const absolute = isAbsolute(indexPath) ? indexPath : join(repoRoot, indexPath);
      try {
        copyFileSync(absolute, tempIndex);
        seeded = true;
      } catch {
        // No index yet: the tree below seeds it.
      }
    }
    if (!seeded && runGit(repoRoot, ["read-tree", "HEAD"], { env }).code !== 0) {
      if (runGit(repoRoot, ["read-tree", "--empty"], { env }).code !== 0) {
        return null;
      }
    }
    // `--sparse`: an untracked file outside the cone -- an engine's
    // settings under `.claude/`, a root-level directory and not a
    // root-level file -- is refused by a plain add, and the snapshot would
    // fail for a file the full checkout snapshots without comment. The flag
    // says the paths are known to be outside the cone and are wanted anyway.
