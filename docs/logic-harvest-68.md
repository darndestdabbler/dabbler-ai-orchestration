...
`packages/router/lifecycle.json` holds **21 driver transitions and nothing
else**, and `drive.test.ts` asserts that they are *exactly* the edges a whole
run of that file takes — every edge taken is declared, every edge declared is
taken. There is no `exercised` flag, no exemption and no early return, because
each of those is a way for the file to carry a claim no test reaches, which is
the hand-maintained diagram the control exists instead of. **Adding a
transition means adding the test that drives it, in the same change.**

That rule is what this section is for. The harvest derived more of the machine
than the suite drives, and the rest is recorded here — in prose, in a document
that says it is prose — rather than in a source that would then be trusted.

### Driver edges the harvest declared that no test drives

Seventeen. Each is real; none is held; and the gap between them and the
twenty-one is a statement about this repository's tests, not about its code.
...
Six of them are self-loops, which a phase line cannot show at all: holding
those would need the driver instrumented rather than read, which is a change to
`drive.ts` for a test's benefit and was not this session's to make.
