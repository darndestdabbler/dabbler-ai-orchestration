 ...
 */
export type ChoiceLayer = "call" | "checkout" | "machine" | "shipped" | "default";

/** Every source a reading can name, and the layer it is. A source missing here is a bug a test finds. */
export function layerOfSource(source: string | null): ChoiceLayer {
...
export function explainRoleModel(role: string, root?: string | null): TransportReading {
  ...
  const checkout = root ?? projectRoot();
  return explainChoice([
    ...(setting === null ? [] : [[setting[1], checkoutSettings(checkout)[setting[0]] ?? null] as const]),
