...
- Prefer a check over the **product** code (a probe that drives the public entrypoint, a targeted existing test) over one that depends on a **test file the fix is expected to add or change**: the harness invalidates any criterion whose test assets the remediation modified, because a criterion that can be edited by the person it judges is not evidence. **Any test-runner command (`pytest`, `go test`, `npm test`, …) is judged against every test asset in the repo** — what a runner collects cannot be read off its command line — so a runner-based criterion will not auto-close in a session that also touches tests. A path-based probe over product code is the criterion most likely to close cleanly.
