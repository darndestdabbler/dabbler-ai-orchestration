...
describe("what a build file says about publishing", () => {
  ...
  it("derives Maven's lifecycle from the POM being a POM", () => {
    const found = detectPackaging(repoWith({ "pom.xml": "<project></project>" }));
    expect(found.recipe?.key).toBe("maven");
  });

  it("says an aggregator pom publishes nothing itself", () => {
    const found = detectPackaging(
      repoWith({ "pom.xml": "<project><packaging>pom</packaging></project>" }),
    );
    expect(found.recipe).toBeNull();
    expect(found.reason).toContain("aggregator");
  });

  it("prefers the committed wrapper over a tool that may not be installed", () => {
    const root = repoWith({ "pom.xml": "<project></project>", mvnw: "#!/bin/sh\n" });
    expect(detectPackaging(root).recipe?.pack[0]).toBe("./mvnw");
  });
});
