...
| 136 | 228 s | 1165 | run by hand as `dabbler.yaml` declares it, WITH the sampler below attached — its own CIM queries cost 40 s of the 228 on the same box, so this row is not comparable with the ones above it and is here for the measurement beside it |
...
record all the same. What the run does instead is CREATE PROCESSES: 363 of
them in 228 seconds — 146 `git`, 124 `conhost`, 66 node workers and CLI
children, 17 `sh` running a `pre-commit` hook, 8 `git-receive-pack`, 3
`cmd` — a sustained 1.6 a second for four minutes. That is the load the note
above names, measured.
...
**The seventeen jobs `walk-session` spawned became seventeen function
calls.** `jobs.setJobStarter` is the seam, in the shape of
`journal.setGitSource`, and `test/support/inProcessJobs.ts` runs the verb the
driver asked for with the same log, the same status file and the same moment
each appears — so the driver still starts a job, issues a `wait` and collects
an exit code, and the walk still proves the order it was written to prove.
Measured alone on an unloaded box: 105.9 s before, 80.2 s after, and about 60
processes that are no longer created. `walk-jobs` keeps the real spawn, and
says so at the top of the file: it is the one that is ABOUT the boundary.
