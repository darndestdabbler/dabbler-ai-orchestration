...
class Driver {
  ...
  /**
   * A configuration that stopped loading while the loop ran.
   *
   * Read where it is acted on, `dabbler.yaml` can be left malformed by a
   * step mid-session. That is a step to repair it, never a crash and never
   * a stop with no exit, and it returns to `preverify` as the run of
   * record's own fix step does, so every check, verification and the suite
   * run again over the repaired file.
   */
  private async fixConfiguration(error: ConfigError): Promise<void> {
    ...
    await this.runSynthesisedStep(
      ...
      `The repository's configuration does not load: ${error.message}. Repair ` +
        `${PROJECT_CONFIG_FILENAME} (or the file the message names); the framework will run ` +
    ...
    try {
      for (;;) {
        ...
        try {
          ...
          if (!(error instanceof ConfigError)) throw error;
          await this.fixConfiguration(error);
        }
      }
    } catch (error) {
