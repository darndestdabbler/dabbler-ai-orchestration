 ...
 * the USER level, where no `RelativePattern` this window could build reaches
 * them, so nothing can ever tell the cache they moved. Re-reading is a few
 * file reads with no network and no process, and the choice is therefore
 * between a window that shows a `dabbler configure` typed in a terminal and
 * one that does not until something unrelated happens. It shows it.
 *
