...
export function appendSuitesToProjectConfig(
  ...
  // A file that already declares suites is not this function's to edit: a
  // list somebody authored gets merged by them, not appended to by a tool
  // that cannot know what the existing entries mean.
  if (/^\s+suites:/m.test(existing)) return null;
