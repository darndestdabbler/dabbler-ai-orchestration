...
import { TRANSPORT_SEAT, blockOf, readCatalog } from "./catalog.ts";
import {
...
import { REFRESH_COMMAND, explainRoleCandidates } from "./transports/copilot.ts";
...
function roleReading(root: string, config: RouterConfig, transport: string): RoleReading {
  ...
  const block = blockOf(readCatalog(), TRANSPORT_SEAT);
  if (block === null) {
    return {
