 ...
 *
 * Undefined is a dismissed pick, which cancels the start. A role this machine
 * can offer nothing for is left unnamed rather than blocked: the start then
 * judges the repository's own, in its own words, as it always has.
 */
export async function askReviewers(
  ...
  if (primaries.length === 0) return named;
  ...
  if (auxiliaries.length === 0) return named;
