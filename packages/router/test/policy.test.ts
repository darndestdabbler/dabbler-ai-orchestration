...
describe("the module policy", () => {
  it("renders a two-module session's scope once, the sibling named and the record protected, and writes nothing for one module", () => {
    ...
    for (const path of ["docs/sessions/sessions.json", "docs/sessions/activity-log.json", "docs/modules.yaml", ".dabbler"]) {
      assert.ok(written.protected.includes(path), `${path} is protected: ${written.protected.join(", ")}`);
    }
    assert.deepEqual(written.writable, [".dabbler/scratch"]);
  ...
  it("answers the three rules with their reasons, allows the rest, and allows unobserved what it cannot read", () => {
    ...
    // The record under .dabbler/ is protected whole, and the one place the
    // framework tells the engine to write its answers is carved out of it.
    const record = decide(policy, { tool: "edit", path: ".dabbler/runs/s4/policy.json" });
    assert.equal(record.rule, "protected-path");
    ...
    for (const allowed of [
      ...
      decide(policy, { tool: "Write", path: ".dabbler/scratch/plan.json" }),
    ...
    // A policy file with a member the decision cannot read is no policy.
    seed(root, {
      ".dabbler/runs/s5/policy.json": JSON.stringify({
        schema_version: 1,
        root: policy.root,
        allowed: [],
        protected: [],
    ...
    assert.equal(readPolicy(root, 5), null);

    for (const open of [
      decide(null, { tool: "edit", path: "modules/model/src/CsvModel/Person.cs" }),
      decide(readPolicy(root, 5), { tool: "view", path: "modules/model/src/CsvModel/Person.cs" }),
