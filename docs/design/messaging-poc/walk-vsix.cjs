...
function processes(pattern) {
  // As JSON: a shell's command line can hold newlines, and a line-per-process listing breaks on one.
  const script =
    "@(Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -and $_.CommandLine -match $env:WALK_PATTERN } | " +
    "ForEach-Object { [pscustomobject]@{ pid = $_.ProcessId; name = $_.Name; command = $_.CommandLine } }) | ConvertTo-Json -Compress";
  const done = cp.spawnSync("powershell", ["-NoProfile", "-Command", script], { encoding: "utf8", env: { ...process.env, WALK_PATTERN: pattern } });
  let rows = [];
  try {
    const parsed = JSON.parse((done.stdout ?? "").trim() || "[]");
    rows = Array.isArray(parsed) ? parsed : [parsed];
  } catch {
    rows = [];
  }
  return rows
    .map((row) => ({ pid: Number(row.pid), name: String(row.name), command: String(row.command) }))
    .filter((row) => row.pid !== process.pid && !/walk-vsix\.cjs|WALK_PATTERN/.test(row.command));
}
...
const LEFT_BEHIND = /^(code|node|claude|copilot|git|tar|npm)(\.exe|\.cmd)?$/i;
...
function chainedExchanges() {
  ...
  return processes("dabbler\\.cjs.*session\\s+report.*--next").filter((p) => mine(p.command) && /^(code|node)(\.exe)?$/i.test(p.name));
}
...
async function mainScenario(router) {
  ...
  while (Date.now() - started < deadline * 1000) {
    ...
    if (state.instructions.length > 0 && processes("Run .dabbler session next").length === 0) return "the engine's CLI is no longer running";
...
async function cancelScenario() {
  ...
  while (Date.now() - started < deadline * 1000) {
    ...
    if (row && row.status === "cancelled") {
      ...
      const at = { head: git(REPO, "rev-parse", "HEAD").out, remote: git(ORIGIN, "rev-parse", "master").out, tree: git(REPO, "status", "--porcelain").out };
      await sleep(60_000);
      state.cancel = {
        reasonAsked: REASON, reasonRecorded: row.cancelledReason ?? null, before: state.cancelBefore ?? null,
        treeAfter: git(REPO, "status", "--porcelain").out.split("\n").filter(Boolean),
        treeBefore: at.tree.split("\n").filter(Boolean),
        headMovedAfterCancel: git(REPO, "rev-parse", "HEAD").out !== at.head,
        remoteMovedAfterCancel: git(ORIGIN, "rev-parse", "master").out !== at.remote,
        landCommits: git(ORIGIN, "log", "--format=%s", "master").out.split("\n").filter((subject) => /^Session 0*1\b/.test(subject)).length,
        tags: git(REPO, "tag").out.split("\n").filter(Boolean),
        packagingRows: jsonl(path.join(REPO, ".dabbler", "runs", "s1", "packaging.jsonl")).length,
      };
      return "cancelled";
    }
...
function summarize(outcome, startedAt) {
  ...
  return {
    ...
    engineCommands: {
      ...
      sleepsOrPolls: commands.filter((row) => /\bsleep\b|Start-Sleep|run\.json|instruction\.json/i.test(row.what)).length,
    ...
    reopen: state.reopen ?? null,
    dabblerTerminal: { linesBeginningDabblerBracket: [dabblerAfter, dabblerEnd].join("\n").split(/\r?\n/).filter((line) => /^\s*dabbler \[/.test(line)).length },
...
(async () => {
  ...
  state.processesLeft = processes(escapeRegex(`${path.basename(SCRATCH)}\\${run}\\`) + "|" + escapeRegex(`${path.basename(SCRATCH)}/${run}/`))
    .filter((p) => LEFT_BEHIND.test(p.name))
    .map((p) => `${p.pid} ${p.name} ${p.command.replace(/\s+/g, " ").slice(0, 200)}`);
  log("harness", "processes-left", false, { processes: state.processesLeft });
  if (scenario !== "direct") {
    const dir = driverDir();
  ...
  fs.writeFileSync(path.join(OUT, "summary.json"), JSON.stringify(summarize(outcome, startedAt), null, 2) + "\n");
  log("harness", "done", false);
  process.stdout.write(`${JSON.stringify({ run, outcome, summary: path.join(OUT, "summary.json") })}\n`);
})().catch(async (error) => {
