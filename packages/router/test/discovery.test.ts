...
describe("reading a vendor's models endpoint", () => {
  ...
  it("calls a closed port a network error, on the real fetch", async () => {
    // The classifier reads what Node actually throws, which is a TypeError
    // whose `cause` carries the syscall code -- so a hand-built error is not
    // proof. Port 1 on the loopback refuses instantly and reaches no network.
    process.env["TEST_OPENAI_KEY"] = "k";
