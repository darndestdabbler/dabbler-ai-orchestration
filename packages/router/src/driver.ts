...
export function judgeWorkPlanModules(plan: DriverWorkPlan, shape: SolutionShape): string[] {
  ...
  if (!shape.multi) {
    for (const slug of named) {
      if (!declared.has(slug)) {
        reasons.push(
          `the plan names module '${slug}', and this repository is a single-module ` +
            "solution whose one module is " +
            `'${shape.modules[0]?.slug ?? ""}'; a single-module plan leaves \`modules\` out`,
        );
      }
    }
    return reasons;
  ...
  if (named.length === 0) {
    reasons.push(
      `the plan names no module, and docs/modules.yaml declares ${declared.size}: a session ` +
  ...
  for (const slug of named) {
    if (!declared.has(slug)) {
      reasons.push(`the plan names module '${slug}', which docs/modules.yaml does not declare`);
    }
  }
  if (named.length > 1 && !(plan.reason ?? "").trim()) {
    reasons.push(
      `the plan names ${named.length} modules (${named.join(", ")}) and gives no reason; ` +
