 ...
 * (`files_from_diff`), its own list otherwise. Listing what changed is the
 * framework's to do; a report that lists them is still held to them.
 */
export function reportedFiles(report: DriverReport, changed: readonly string[]): readonly string[] {
  return report.files_from_diff === true ? changed : report.files_changed;
}
    ...
    const poll = setInterval(() => {
      ...
      if (multiple !== null) {
        ...
        if (reading.state === WATCHER_OUTSTANDING) {
          ...
          // On the record for the extension to show. Nothing types into the
          // AI's chat: a typed nudge would be logged as the operator's words.
          appendSupervision(this.repoRoot, this.sessionNumber, {
            event: "instruction-overdue",
            seq: instruction.seq,
            step: instruction.step_id ?? null,
            outstanding_seconds: reading.sinceSeconds,
          });
        }
      }
...
export function instructionAnswered(
  ...
  try {
    ...
    if (instruction.answer_schema === DISPOSITION_SCHEMA) {
      return (readDispositions(repoRoot, sessionNumber)?.seq ?? 0) >= instruction.seq;
    }
    return (readReport(repoRoot, sessionNumber)?.seq ?? 0) >= instruction.seq;
  } catch {
    return false;
  }
}
