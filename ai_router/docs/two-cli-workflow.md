...
The ai_router enforces `outsourceMode: last` requires a `verifierRole`
...
`ai_router/__init__.py:_resolve_outsource_mode` for the resolution
order.
...
Configuration block. If `verifierRole` is missing, ai_router refuses
...
`ai_router/docs/close-out.md` Section 6 for reconciler details.
...
- The remaining message budget for the subscription window. The
  ai_router has no view into provider-side accounting; the operator
...
`ai_router/docs/close-out.md` Section 5 for the flag-combination
rules.

For close-out internals (gate checks, lock contention, idempotency,
event-log shape) see `ai_router/docs/close-out.md`. For the workflow
