...
// package.json form names `src/test/suite/x.test.ts`, relative to the
// package; the framework's targeted form appends the paths the selector
// chose, which are relative to the REPOSITORY. Each is resolved where it
// was written and handed to mocha absolute, so neither caller has to know
...
/**
 * One spec path, made absolute.
 *
 * Where the caller stood decides it: a path that exists from the working
 * directory is that file, and one that does not is tried against the
 * package. A path that is neither is passed through unchanged so mocha
 * reports the miss -- silently dropping it would turn a typo into a green
 * run of nothing.
 */
function specPath(spec) {
  if (isAbsolute(spec)) return spec;
  const fromCwd = resolve(spec);
  if (existsSync(fromCwd)) return fromCwd;
  const fromPackage = resolve(packageRoot, spec);
  return existsSync(fromPackage) ? fromPackage : spec;
}

const specs = process.argv.slice(2).map(specPath);
