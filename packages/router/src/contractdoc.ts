...
export interface ModuleContract {
  ...
  /** The notes page as read, with the contract.yaml rendering appended when one exists. */
...
export function renderModuleContract(
  ...
  let notes = readText(join(root, notesPath));
  ...
  if (existsSync(yamlPath)) {
    ...
    notes = `${notes.trimEnd()}\n\n${render(load(yamlPath), graph)}`;
  }
  const packageId = entry.package ?? slug;
  if (mode === "package") {
    return { slug, mode, notes, api: null, apiPath: null };
  }
