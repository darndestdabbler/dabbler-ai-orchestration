...
>
> **DO THIS FIRST, before any soak.** The published 3.16.0 is not designated for the four soaks until it has been compared with the package that was walked. Once CI's publish run for `vsix-v3.16.0` is green: `gh run download <run id> --repo darndestdabbler/dabbler-ai-orchestration --name vsix`, then `node docs/design/messaging-poc/walk-vsix.cjs --compare <that file> C:\temp\s219-installed\released\dabbler-ai-orchestration-3.16.0.vsix`. On `"same": true` record the published file's SHA-256 here and soak **that** file. On anything else, soak nothing: the difference it lists is the next session's work. The walked candidate's SHA-256 is `b6548e32674ce0c6212d18fa4f78b62e9353a568141764b1361d57900095c2e4`; a local file can never share a checksum with CI's (a zip carries its timestamps), which is why the gate is on contents.
>
