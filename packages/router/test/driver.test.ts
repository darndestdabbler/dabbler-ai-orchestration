...
import { captured, makeSandboxRepo, removeTempDirs } from "./support/fixtures.ts";
...
describe("the four answer schemas", () => {
  ...
  it("a report has no word for a verdict and no path outside the repository", () => {
    ...
    for (const path of ["src\\widget.py", "../elsewhere.py", "C:/abs/widget.py", "./src/widget.py"]) {
  ...
  it("a work plan's checks are argv, and its step ids are unique", () => {
