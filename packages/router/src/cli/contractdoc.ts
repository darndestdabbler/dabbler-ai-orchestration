...
function runModule(workspaceRoot: string, slug: string, out: string | null): number {
  ...
  // The bundle on disk: the notes page as the author left it, the rendered
  // contract.yaml beside it when there is one, and the surface page for a
  // designed or generated contract. Each written file is named.
  writeOut(`notes ${bundle.notesPath}\n`);
  if (bundle.rendered !== null && bundle.renderedPath !== null) {
    const renderedTarget = join(workspaceRoot, bundle.renderedPath);
    mkdirSync(dirname(renderedTarget), { recursive: true });
    writeFileSync(renderedTarget, platformNewlines(bundle.rendered), { encoding: "utf8" });
    writeOut(`wrote ${bundle.renderedPath} (from contract.yaml)\n`);
  }
