...
describe("the store this platform has", () => {
  ...
  it("refuses to store over a name another provider already owns", () => {
    // Replacing a credential of the same name is how a key is rotated and
    // is the commonest reason anyone runs this. Replacing ANOTHER vendor's
    // entry under that name is not a rotation: it destroys a credential
    // some setting is pointing at, and it used to do it silently.
    const path = inTemp();
    seed(path);
    assert.equal(credentialProvider("work", path), "anthropic");
    assert.throws(
      () => setCredential("work", "openai", SECRET, { path, platform: "win32" }),
      (error: unknown) =>
        error instanceof CredentialError &&
        /already a credential stored for anthropic/.test(error.message) &&
        /dabbler auth remove work/.test(error.message) &&
        !error.message.includes(SECRET),
    );
    // Untouched: still anthropic's, still the blob it was.
    assert.equal(credentialProvider("work", path), "anthropic");
