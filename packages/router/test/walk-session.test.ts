...
describe("a second driver taking the lease mid-run", () => {
  it("stops the stale one before it advances the run, and says which epoch lost", async () => {
    ...
    const thief: Engine = {
      ...
      invoke: (invocation) => {
        ...
        if (stolenFrom === null && run !== null) {
          ...
          writeRun(repo, 1, { ...run, lease_epoch: stolenFrom + 5 });
        }
    ...
    // And it advanced nothing: the record still carries the epoch the holder
    // wrote, which is the whole of what the fence protects.
    assert.equal(readRun(repo, 1)?.lease_epoch, taken + 5);
