...
// necessary plus the cheap deterministic controls that cover it;
// `final-full` runs the relevant complete suites once, as the run's final
// proof. The separation exists to delete one specific expense -- a full
// suite bought before anyone knows whether the change is right -- so this
// module refuses to accept a complete suite as targeted evidence outside the
...
//
// The run core's heartbeat is retired and never ported (D130), so `execute`
// takes no run id: what it emitted was a liveness file the six-step driver
// read, and nothing reads one now.
...
export const FULL_ALLOWED_SMALL = "suite-declared-small";
/**
 * The suite declared it has no targeted form at all. Distinct from `small`,
 * which is about cost: a slow Maven build is not small, and a fast one still
 * cannot be handed a list of source files.
 */
export const FULL_ALLOWED_WHOLE = "suite-runs-whole";
export const FULL_ALLOWED_ALL_AFFECTED = "all-tests-affected";
export const FULL_ALLOWED_OPERATOR = "operator-override";
...
export interface CheckPlan {
  ...
  readonly fullAllowedReason: string;
}
...
export function changedPathsFor(
  root: string,
  baseCommit: string,
  treeDigest: string,
): string[] {
  const paths = changedPathsBetween(root, baseCommit, treeDigest);
 ...
 * §5.3 `no-declared-check` condition rather than a reason to run everything.
 */
export function plan(
  ...
  options: {
    ...
    treeDigest: string;
    baseCommit: string;
    allowFullReason?: string;
  },
): CheckPlan {
  const { stage, treeDigest, baseCommit } = options;
  const allowFullReason = options.allowFullReason ?? "";
  if (!STAGES.includes(stage)) {
    throw new Error(`unknown stage ${pythonRepr(stage)}`);
  }
  const declared = loadChecks(config);
  const touched = changedPathsFor(root, baseCommit, treeDigest);
  const covered = touched.filter((path) =>
    declared.some((check) => coversAny(check, [path])),
  );

  let selectionResult = new SelectionResult();
  if (stage === STAGE_TARGETED) {
    const loaded = loadSelectionConfig(config);
    if (!loaded.ok) {
      throw new CheckConfigError(
        "testing.selection is invalid: " + loaded.errors.join("; "),
      );
    }
    selectionResult = selectTests(root, touched, loaded.config);
  }

  const planned: Array<readonly [Check, string]> = [];
  let fullReason = "";
  for (const check of declared) {
    if (!coversAny(check, touched)) continue;
    if (stage === STAGE_FINAL_FULL) {
      if (isSuite(check)) planned.push([check, displayCommand(check)]);
      continue;
    }
    if (!isSuite(check)) {
      planned.push([check, displayCommand(check)]);
      continue;
    }
    const [command, reason] = targetedSuiteCommand(
      check,
      selectionResult,
      allowFullReason,
    );
    if (command) {
      planned.push([check, command]);
      fullReason = fullReason || reason;
    }
  }
 ...
 */
function targetedSuiteCommand(
  check: Check,
  result: SelectionResult,
  allowFullReason: string,
): [string, string] {
  const base = displayCommand(check);
  if (result.allTestsAffected) return [base, FULL_ALLOWED_ALL_AFFECTED];
  if (check.small) return [base, FULL_ALLOWED_SMALL];
  if (check.runsWhole) return [base, FULL_ALLOWED_WHOLE];
  if (allowFullReason) return [base, FULL_ALLOWED_OPERATOR];
  return [targetedCommand(base, result), ""];
}

export function selectionUnknownPaths(result: SelectionResult): string[] {
  return result.unknownPaths;
}
 ...
 * EINVAL), where Python's `CreateProcess` runs one directly. So an argv whose
 * program is a shim is handed to the interpreter that can run it, with the
 * declared arguments quoted here rather than reassembled from a string --
 * `shell: true` would let a shell re-parse a line this module built, which is
 * exactly what an argv declaration exists to avoid.
 */
