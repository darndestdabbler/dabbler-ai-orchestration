...
- **Built** in session 23, beside `tsc --noEmit` (kind `typecheck`),
  ESLint (kind `lint`) and the generated-type staleness check (kind
  `compile`) — the first controls this repository declares.
- **Declared** in `dabbler.yaml` as kind `analyzer`, `required: true`, in
  session 26 — the session that gives it its first case.

  > **Amended in session 23, on a verifier's finding.** The design said
  > declared in 23. But no router verb exists in TypeScript until session
  > 25, so a control declared in 23 would return exit 0 having compared
  > nothing, and write a green `analyzer: pass` row into
  > `deterministic-facts.jsonl` on every round from here — a record saying
  > parity was checked when nothing was. Making it red instead would block
  > sessions 23 to 25 on a condition no work can clear. Neither is
  > acceptable, and the third option is honest: `facts` writes
  > `not_applicable` for a kind nobody declared, which is what today
  > actually is. The control is built, runnable by hand and unit-tested
  > from session 23; its declaration waits for its first case.
