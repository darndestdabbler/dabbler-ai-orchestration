...
`,
  ...
  if (subcommand === "hold-release") {
    ...
    return holdRelease(sessionsDir, { reason, engine: callerIsEngine() });
  }
  ...
  if (subcommand === "cancel" || subcommand === "restore") {
    ...
    return cancel(sessionsDir, target, { reason, force: switches.has("--force"), engine: callerIsEngine() });
  }
  ...
  if (subcommand === "wait") {
    ...
    //
    // And it is the AI's verb alone, which is what makes it the door: a
    // shell that carries nothing marking an engine cannot wait, so it cannot
    // take part in a session, and every shell that CAN take part is one the
    // person's verbs already refuse. No vendor's variable is needed for that
    // to hold -- an engine nobody has measured, launched by hand outside the
    // editor, is told here, once, how its terminal is marked.
    if (!callerIsEngine()) {
      writeErr(`dabbler session wait: ${UNMARKED_SHELL}\n`);
