...
// take, which would have been the sampler measuring itself. Its own cost is
// reported in the summary for the same reason.
...
async function selfCheck() {
  // Four seconds of a child that itself has a child, sampled every second:
  // enough to prove the sampler collects, finds the tree, and reports a
  // priority class -- without running a suite to find out.
  const script =
