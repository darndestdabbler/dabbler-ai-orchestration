...
describe("parsing ledger text", () => {
  it("refuses a line that is not valid JSON, naming the source and the file's own line number", () => {
    assert.throws(() => parseJsonlText("{}\n\n{not json}\n", (row) => row, "rounds.jsonl"), /rounds\.jsonl line 3 is not valid JSON/);
  });

  it("refuses a line that is not an object, and a row the validator refuses, naming the line either way", () => {
    assert.throws(() => parseJsonlText("[1, 2]\n", (row) => row), /line 1 is not an object/);
    assert.throws(() => parseJsonlText(`${dumps(ROUND)}\n${dumps({ round: 2 })}\n`, validateRound, "rounds.jsonl"), /rounds\.jsonl line 2: .*schema validation/);
  });

  it("stamps a round with the framework version and refuses a number already recorded, from the rows alone", () => {
    const stamped = roundToAppend([], 1, { ...ROUND });
    assert.equal(stamped["framework_version"], VERSION);
    assert.throws(() => roundToAppend([{ ...ROUND }], 1, { ...ROUND }), /round 1 is already recorded for session 1; rounds are append-only/);
    assert.throws(() => roundToAppend([], 1, { round: 1 }), LedgerError);
  });

  it("skips blank lines and reads either line ending, keeping every row in order", () => {
    const second = { ...ROUND, round: 2, previous_tree: "1".repeat(40) };
    const rows = parseJsonlText(`${dumps(ROUND)}\r\n\r\n${dumps(second)}\n`, validateRound);
...
describe("the step execution record", () => {
  ...
  it("folds an open step out of the rows, and has none once it closed", () => {
    // The fold over literal rows, then the reader that feeds it.
    assert.equal(openStepIn([opened])?.["step_id"], "one");
    assert.equal(openStepIn([opened, closed]), null);
    assert.equal(openStepIn([opened, closed, { ...opened, step_id: "two" }])?.["step_id"], "two");
