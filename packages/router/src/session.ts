  ...
  if (forced && options.engine === true) {
    writeErr(
      `close: ${isAPersonsVerb(
        ...
        "a person forces a close from their own terminal, and only to abandon a whole plan",
      )}\n`,
    );
    return EXIT_BOUNDARY;
  }
...
/**
 * What `session wait` says from a shell that carries no engine marker. The
 * way on is the whole of it: the variable, in both shells a person types in,
 * and who sets it without being asked.
 */
export const UNMARKED_SHELL =
  "refused -- `session wait` is the AI's verb, and nothing in this shell says an AI's CLI is " +
  "running in it. The extension marks the terminal it opens for an AI, and the framework marks " +
  "every engine it starts; an engine you launch yourself is launched with the marker set -- " +
  '`$env:DABBLER_ENGINE_TERMINAL = "1"; <engine>` in PowerShell, `DABBLER_ENGINE_TERMINAL=1 ' +
  "<engine>` in bash -- and then this waits. It is what keeps the verbs that are a person's " +
  "(`cancel --force`, `close --force`, `hold-release`) a person's. To see where a session is " +
  "without waiting, run `dabbler status`.";
  ...
  if (options.force === true && options.engine === true) {
    writeErr(
      `cancel: ${isAPersonsVerb(
        ...
        "a person cancels from the Work Explorer (Cancel Session) or their own terminal",
      )}\n`,
    );
    return EXIT_BOUNDARY;
  }
  ...
  if (options.engine === true) {
    writeErr(
      `hold-release: ${isAPersonsVerb(
        ...
        "a person holds a release from their own terminal",
      )}\n`,
    );
    return EXIT_BOUNDARY;
  }
