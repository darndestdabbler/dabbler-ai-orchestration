 ...
 *
 * Paths are relative where they can be, so moving the whole set of checkouts
...
export function workspaceFolders(repoRoot: string): WorkspaceFolder[] {
  ...
  const out: WorkspaceFolder[] = [
    { name: basename(resolve(repoRoot)), path: "." },
  ];
  ...
  for (const member of members.slice(1)) {
    ...
    seen.add(full);
    const relative_ = relative(repoRoot, full).replace(/\\/g, "/");
    out.push({
      name: member.id,
      path: relative_ && !relative_.startsWith("..\\") ? relative_ : full,
    });
  }
  return out;
}
