...
describe("the state a session start writes, from the state read and the plan's titles", () => {
  const titles = new Map([[1, "First"], [2, "Second"]]);
  const at = "2026-01-02T03:04:05-04:00";

  it("grows a fresh ledger to the plan, titles from it, and stamps only the registered row", () => {
    const state = stateAfterStart(null, titles, 1, { engine: "claude-code" }, at);
    const sessions = state["sessions"] as Record<string, unknown>[];
    assert.equal(sessions.length, 2);
    assert.deepEqual(sessions.map((s) => [s["number"], s["status"], s["title"]]), [[1, "in-progress", "First"], [2, "not-started", "Second"]]);
    assert.equal(sessions[0]["startedAt"], at);
    assert.equal(sessions[0]["frameworkVersion"], VERSION);
    assert.equal("frameworkVersion" in sessions[1], false);
  });

  it("refuses to re-open a closed session, carries an earlier stamp, and drops a stale verification summary", () => {
    const raw = {
      schemaVersion: 5,
      sessions: [
        { number: 1, title: "First", status: "complete", startedAt: at, completedAt: at, frameworkVersion: "1.1.0", verification: { rounds: 3 } },
        { number: 2, title: "Second", status: "not-started", verification: { rounds: 9 } },
      ],
    };
    assert.throws(() => stateAfterStart(raw, titles, 1, { engine: "claude-code" }, at), /already in completedSessions \[1\]/);
    const sessions = stateAfterStart(raw, titles, 2, { engine: "claude-code" }, at)["sessions"] as Record<string, unknown>[];
    assert.equal(sessions[0]["frameworkVersion"], "1.1.0");
    assert.equal(sessions[1]["frameworkVersion"], VERSION);
    assert.equal("verification" in sessions[1], false);
    assert.equal(sessions[1]["status"], "in-progress");
  });
});
