...
function run(argv: string[]): number {
  ...
  // called, and the framework does the typing. The block is written once both
  // are answered, because half a packaging block is a configuration error
  // rather than a partial feature.
  if (id === ID_PACKAGING_FEED || id === ID_PACKAGING_SECRET) {
    const value = values.get("--value") ?? choice;
    if (choice === "publishes nothing") {
      ...
      const answered = answeredChoice(root, other);
      if (answered === null) {
