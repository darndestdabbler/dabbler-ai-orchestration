...
> **Set 112 migration owed.** `dabbler-homehealthcare-accessdb` ran on the
> Lightweight tier, which Set 112 removed. Any of its sets still declaring
> `tier: lightweight` will be refused by the loader as of
> `dabbler-ai-router` **`1.0.0`** (staged 2026-08-09, publish
> operator-gated). The remedy and the two replacement options are in
> [`docs/cross-repo-lightweight-removal-notice.md`](cross-repo-lightweight-removal-notice.md);
> **sending that notice is operator-gated** and the repo's own migration is
