...
export function materialPaths(porcelain: string, setRel: string): string[] {
  ...
  for (const entry of parsePorcelain(porcelain)) {
    ...
    if (isFrameworkInstalledPath(path)) {
      continue; // the framework's own install, committed by the land
    }
    blocking.push(path);
  }
  return blocking;
 ...
 * `session start` for a claude-code registration. It is not the session's
 * work, and a declaration made after it must not be refused for it -- the
 * land commits it with everything else.
 */
