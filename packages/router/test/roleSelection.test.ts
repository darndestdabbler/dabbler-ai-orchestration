...
describe("whether the model asked for is the model that answered", () => {
  ...
  it("reads a seat catalog's echoes as echoes, and a round's served ids as the provider's word", () => {
    ...
    // A round's pair is the provider's own statement, and does establish it.
    const rounds = roundObservations([
      { requested_model: "gpt-5.6-terra", served_model: "gpt-5.6-terra" },
      { requested_model: "gpt-5.4", served_model: null },
      { served_model: "orphan" },
    ]);
