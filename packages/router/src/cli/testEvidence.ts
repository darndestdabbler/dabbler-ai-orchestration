...
export async function testEvidenceVerb(argv: string[]): Promise<number> {
  ...
  // The second bookend. A session's tasks are rendered while it is in flight
  // and the close ends that, so the complete suite passing against the final
  // tree is the last transition anyone can watch happen.
  if (stage === STAGE_FINAL_FULL && record.outcome === OUTCOME_PASSED) {
    const number =
      typeof sessionNumber === "number" ? sessionNumber : currentSessionNumber(sessionsDir);
    if (number !== null) closeLastStep(sessionsDir, number);
  }
