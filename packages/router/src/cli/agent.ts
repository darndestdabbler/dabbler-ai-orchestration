...
function promptUsage(): string {
  return [
    ...
    "",
    "Open a conversation with the agent, send TEXT as one prompt, print every event",
    "as one JSON line on stdout as it arrives, then the turn's outcome as a final",
    '{"kind":"turn"} line, and close. On a resume, the events the agent replays',
    "come first, each marked replayed.",
    "",
    "options:",
    "  -h, --help            show this help message and exit",
