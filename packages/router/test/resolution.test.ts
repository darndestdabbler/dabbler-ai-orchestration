...
import { DEPS_FILENAME, assembleSolution } from "../src/solutionDeps.ts";
...
describe("what the producer has published", () => {
  it("reads the version out of the producer's own manifest", () => {
    ...
    expect(publishedVersion(root)).toBe("2.0.0");
  });
  ...
  it("reports a version only the build tool can resolve as unknown", () => {
    ...
    expect(publishedVersion(root)).toBeNull();
  });
  ...
  it("names a pin that is behind what the producer builds", () => {
    const { app } = solution();
