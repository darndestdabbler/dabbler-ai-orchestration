...
function engineItems(projection: Projection | null): vscode.QuickPickItem[] {
...
function modelItems(models: readonly ConfigurationModel[]): vscode.QuickPickItem[] {
  return models.map((model) => ({ label: model.model, description: model.provider, detail: model.alias }));
}
...
export async function setEngine(
  ...
  const items = engineItems(target.projection ?? null);
  if (items.length === 0) {
    ui.showWarningMessage("No engine is known here yet — the Configuration section reads them from the projection.");
    return;
  }
...
export async function setRoleModel(
  ...
  const items = modelItems(role?.candidates ?? []);
  if (items.length === 0) {
