...
/** This repository's Copilot session log: the newest events.jsonl that names the repository. */
function copilotEvents() {
  ...
  for (const dir of fs.existsSync(root) ? fs.readdirSync(root) : []) {
    const file = path.join(root, dir, "events.jsonl");
    try {
      const stat = fs.statSync(file);
      if (stat.mtimeMs < t0) continue;
      const head = fs.readFileSync(file, "utf8").slice(0, 4000);
      if (!head.includes(path.basename(REPO))) continue;
  ...
  while (Date.now() - started < deadline * 1000) {
    ...
    if (engine === "claude") {
      ...
      if (/Quick safety check|trust this folder|Do you trust|trust the files|Yes, proceed/i.test(text.split(/\r?\n/).slice(-14).join("\n"))) {
        ...
        if (answer) log("operator", "answer-trust-prompt", true, answer);
      ...
      if (!copilotSubmitted) {
        if (events.some((event) => event.type === "user.message")) copilotSubmitted = true;
          ...
          // Start typed the sentence at the prompt and left it for Enter.
          await focusEngine(page);
          await page.keyboard.press("Enter");
          log("operator", "press-enter-on-typed-sentence", true);
          await sleep(8000);
        }
      }
