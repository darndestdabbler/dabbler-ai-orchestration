 ...
 */
export function publishedVersions(memberRoot: string): string[] {
  ...
  for (const entry of sessions) {
    ...
    for (const line of readText(path).split("\n")) {
      ...
      for (const artifact of (row["artifacts"] as string[] | undefined) ?? []) {
        const version = versionFromArtifact(String(artifact));
        if (version !== null) found.add(version);
      }
    }
  }
  return [...found];
...
/** `Dabbler.Csv.Model.2.0.0.nupkg` -> `2.0.0`, or null when it is not one. */
export function versionFromArtifact(name: string): string | null {
  const base = name.replace(/\\/g, "/").split("/").pop() ?? name;
  const match = /^(?:.*?)\.(\d+(?:\.\d+)*(?:-[0-9A-Za-z.-]+)?)\.(?:nupkg|snupkg)$/.exec(base);
  if (match) return match[1];
  const jar = /-(\d+(?:\.\d+)*(?:-[0-9A-Za-z.-]+)?)\.jar$/.exec(base);
  return jar ? jar[1] : null;
}
...
export function reconcileResolution(
  ...
  for (const member of members.slice(1)) {
    ...
    published.set(member.id, publishedVersions(member.root));
  ...
  for (const edge of self?.deps?.consumes ?? []) {
    const releases = published.get(edge.producedBy.id) ?? [];
...
export interface SourceSwap {
  ...
  /**
   * The declaration as it stood, so the restore puts back a file and not an
   * approximation of one. Empty when there was none.
   */
...
export function refuseIfResolvingFromSource(
  ...
  what: string,
  options: { readonly ranFrom?: string | null } = {},
): string | null {
  ...
  const ranFrom = options.ranFrom;
  if (ranFrom) {
    // Compared as instants, not as strings. The record stamps local time with
    // an offset and a caller may hand over UTC, and two correct spellings of
    // the same moment do not sort against each other.
    const started = Date.parse(ranFrom);
    const overlapping = allSwaps(repoRoot).filter((swap) => {
      if (swap.restoredAt === null) return true;
      const restored = Date.parse(swap.restoredAt);
      return Number.isNaN(restored) || Number.isNaN(started) || restored >= started;
    });
    if (overlapping.length > 0) {
      const names = [...new Set(overlapping.map((swap) => swap.packageId))].join(", ");
      return (
        `${what} is refused: ${names} was resolving from source while this run ` +
        "was executing. Restoring the reference afterwards does not change " +
        "what the tests ran against -- run the suite again against the " +
        "restored tree."
      );
    }
  }
...
export function switchToSource(
  ...
  const declarationPath = join(repoRoot, DEPS_FILENAME);
  const declarationBefore = existsSync(declarationPath) ? readText(declarationPath) : "";
  const swap: SourceSwap = {
    ...
    originalDeclaration: declarationBefore,
...
export function restoreFromSource(repoRoot: string, packageId: string): SourceSwap {
  ...
  // The declaration second, and by the same rule: restored to exactly what it
  // was, or the edge put back to `feed` when there was no file to restore.
  const declarationPath = join(repoRoot, DEPS_FILENAME);
  if (swap.originalDeclaration) {
    writeText(declarationPath, swap.originalDeclaration);
  } else if (existsSync(declarationPath)) {
...
function writeDeclaredResolve(repoRoot: string, packageId: string, mode: string): void {
  ...
  if (!existsSync(path)) return;
  const text = readText(path);
  const at = text.indexOf(`"${packageId}"`);
  if (at < 0) return;
  const resolveAt = text.indexOf('"resolve"', at);
  if (resolveAt < 0) return;
  const match = /"resolve"\s*:\s*"[^"]*"/.exec(text.slice(resolveAt));
  if (!match) return;
  writeText(
    path,
    text.slice(0, resolveAt) +
      `"resolve": "${mode}"` +
      text.slice(resolveAt + match[0].length),
  );
}
