  ...
  const entriesOf = (file) => {
    ...
    const walk = (dir) => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        else found[path.relative(into, full).replace(/\\/g, "/")] = crypto.createHash("sha256").update(fs.readFileSync(full)).digest("hex");
      }
    };
    walk(into);
...
function accept(s) {
  ...
  hold("nothing left running", Array.isArray(s.processesLeft) && s.processesLeft.length === 0, JSON.stringify(s.processesLeft));
