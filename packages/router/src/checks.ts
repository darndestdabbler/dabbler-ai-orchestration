...
import { hiddenSpawn, isMachineStatePath, snapshotWorktreeTree } from "./journal.ts";
...
export const SUITE_FIELDS: ReadonlySet<string> = new Set([
  ...
  // `against` named a declared module; they are read and ignored, so a
  // repository that still carries them is not refused for it.
...
// uncommitted work; the task declaration, which refuses to be made after
// work exists; the driver, which asks what a step touched; and the focused
// checkout, which refuses to reset a clone somebody has work in.
//
// It lived in `gates.ts` while three of the four were close-out callers.
// The fourth is why it is here: `checkout.ts` importing `gates.ts` closes
// the knot `checkout -> gates -> exposure -> checkout`, which the boundary
// control refuses, and the alternative -- a second copy of the rule inside
// the checkout -- is the one thing that would make the two disagree. This
// module is the home both sides already import, and the judgement is the
// same kind as the two beside it: what a changed path MEANS, before
...
export function isSetBookkeeping(forwardPath: string, setRel: string): boolean {
