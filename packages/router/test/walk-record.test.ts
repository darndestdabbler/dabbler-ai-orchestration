...
describe("a repository walked through the record one session leaves", () => {
  ...
  milestone("a decision takes its identifier from the record and renders into the log", () => {
    ...
    // The decision is an entry of the activity log the record derives its
    // ordinals and the decisions log from; the projection itself carries
    // sessions, tasks and verification, and no decisions.
    const entries = (readActivityLog(sessionsDir)?.["entries"] ?? []) as Record<string, unknown>[];
    assert.ok(entries.some((entry) => entry["kind"] === "decision" && entry["decisionId"] === "D1"));
  });
