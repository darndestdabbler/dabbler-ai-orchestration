...
import { reprojectSolution, solutionConfiguration } from "../router/host";
  ...
  implements vscode.TreeDataProvider<SolutionNode>, vscode.Disposable
{
   ...
   *
   * The module graph comes off the disk, and the CONFIGURATION is asked for
   * from the router right here, every time the cache is cold. It is not in
   * the file: its inputs -- the model catalog, this operator's preferences
   * -- live at the user level, outside every `RelativePattern` this window
   * could watch, so a stored copy goes stale with nothing able to say so.
   * Derived at the moment it is read, it is also THIS extension's router's
   * reading rather than whichever router last wrote a file.
   */
  private projection(): Projection | undefined {
    ...
    try {
      const solution = JSON.parse(fs.readFileSync(file, "utf8")) as Projection;
      this.cached = {
        ...solution,
