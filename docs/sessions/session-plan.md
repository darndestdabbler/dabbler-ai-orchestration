...
8. Affected tests as preverify. For a prose session the selector should
   report no test affected, and then **nothing is recorded** — a run
   recorded against an empty selection is a `policy_violation`, and session
   2's record shows the shape. In this repository it instead reports every
   tracked ledger file under `.dabbler/runs/` as `selection_unknown` and
   selects the smoke test, because the round-1 baseline is HEAD's raw tree
   while the snapshot drops `.dabbler/` (a standing defect, recorded as a
   decision in this session; sessions 19–21 met the same rows). The smoke
   run is recorded as the selector asks.
