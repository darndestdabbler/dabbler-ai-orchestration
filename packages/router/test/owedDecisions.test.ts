...
describe("what raising a decision writes, from the record's current row", () => {
  const at = "2026-01-01T00:00:00-04:00";

  it("appends a new open row with severity from the class, and refuses a malformed brief", () => {
    const next = raiseDisposition(undefined, BRIEF, at);
    assert.equal(next?.supersede, false);
    assert.equal(next?.row["state"], STATE_OPEN);
    assert.equal(next?.row["severity"], SEVERITY_ADVISORY);
    assert.equal(raiseDisposition(undefined, { ...BRIEF, decisionClass: CLASS_VERIFICATION_REDUCTION }, at)?.row["severity"], SEVERITY_BLOCKING);
    assert.throws(() => raiseDisposition(undefined, { ...BRIEF, options: [BRIEF.options[0]] }, at), OwedDecisionError);
    assert.throws(() => raiseDisposition(undefined, { ...BRIEF, decisionClass: "because-i-said" }, at), OwedDecisionError);
  });

  it("writes nothing over an answered row or the same open brief, and supersedes an open brief that changed", () => {
    const open = raiseDisposition(undefined, BRIEF, at)?.row as Row;
    assert.equal(raiseDisposition({ ...open, event: EVENT_ANSWERED }, { ...BRIEF, question: "Changed?" }, at), null);
    assert.equal(raiseDisposition(open, BRIEF, at), null);
    const changed = raiseDisposition(open, { ...BRIEF, recommendation: "local-only" }, at);
    assert.equal(changed?.supersede, true);
    assert.equal(changed?.row["recommendation"], "local-only");
  });
});
...
describe("a signoff whose session has ended", () => {
  ...
  it("is asked again in a form a person can act on, once, and leaves other questions alone", () => {
    ...
    // Not this session's, and not a signoff: neither is this rule's business.
    ...
    assert.deepEqual(reasked.map((row) => String(row["id"])), ["repair-outside-a-step-7"]);
    ...
    // Both answers survive by name; neither still promises to act on a
    ...
    for (const option of offered) {
      assert.ok(String(option["consequence"]).includes(AFTER_CLOSE_NOTE), option["consequence"]);
      assert.match(String(option["consequence"]), /performs nothing/);
    }
    ...
    for (const id of ["repair-outside-a-step-6", "elsewhere"]) {
