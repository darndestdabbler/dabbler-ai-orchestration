...
/** The policy as the runner receives it: one variable, or none at all. */
function priorityEnv(env: NodeJS.ProcessEnv): Record<string, string> {
  const priority = jobPriority(env);
  return priority === null ? {} : { [JOB_PRIORITY_VAR]: String(priority) };
}
...
export function spawnDetachedJob(
  ...
  const child = spawn(process.execPath, [runner, status, log, repoRoot, ...options.argv], {
    ...
    env: { ...process.env, [DRIVEN_MARKER]: "1", ...priorityEnv(process.env) },
  });
  child.unref();
