...
The credential store's own test file, under `packages/router/test`, is **host-only**, declared in
`scripts/suite-membership.json`. The container image the suite otherwise runs
in has neither PowerShell nor `secret-tool`, so every assertion about a real
store is true only on this host; what the container could still prove — that
a platform with no store refuses — is proved on the host too, through the
same seam.
