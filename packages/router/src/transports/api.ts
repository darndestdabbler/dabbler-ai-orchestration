...
function refusedThinkingParam(error: unknown, params: Json): string | null {
  ...
  return THINKING_PARAMS.find((key) => key in params) ?? null;
}
...
export async function callModel(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      // asked once more without it, and the result says what was dropped.
      const params = generationParams ?? {};
      const refused = refusedThinkingParam(error, params);
      if (refused !== null) {
        const reason = (error as HttpStatusError).words;
        const rest = Object.fromEntries(Object.entries(params).filter(([key]) => key !== refused));
        writeErr(
          `[dabbler] NOTE: ${providerName} refused '${refused}' for '${modelId}' ` +
            `(${reason}); the call ran without it.\n`,
        );
        const result = await callModel(providerName, modelId, systemPrompt, userMessage, maxTokens, config, rest);
        return { ...result, metadata: { ...result.metadata, dropped_param: { param: refused, reason } } };
      }
