...
import { type ContractGraph, ContractError, load, render, renderModuleContract } from "../src/contractdoc.ts";
...
describe("the module form", () => {
  ...
  it("reads a designed surface from the abstractions project's source, with its summaries, and marks it designed", () => {
    ...
    // The notes page is read as the author left it; a contract.yaml beside
    // it renders to contract.md, with the module's place in the graph.
    assert.equal(bundle.notesPath, "modules/persister/contract/README.md");
    assert.match(bundle.notes, /Saving twice keeps one/);
    assert.equal(bundle.renderedPath, "modules/persister/contract/contract.md");
    assert.match(bundle.rendered ?? "", /the person is findable by name/);
    assert.match(bundle.rendered ?? "", /persister --> model/);
    ...
    // A declared seam with no notes page is refused by path.
