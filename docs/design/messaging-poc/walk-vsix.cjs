...
(async () => {
    ...
    if (engine === "claude") {
      ...
      if (!copilotSubmitted) {
        if (events.some((event) => event.type === "user.message")) copilotSubmitted = true;
          ...
          await page.keyboard.press("Enter");
          log("operator", "press-enter-trust-or-sentence", true, { screenshot: shot });
          await sleep(8000);
        }
      }
  ...
  // What the AI ran is counted from the engine's own record, so the record is kept with the walk.
  const engineLog = engineLogFile();
  if (engineLog) fs.copyFileSync(engineLog, path.join(OUT, `${engine}-${path.basename(engineLog)}`));
