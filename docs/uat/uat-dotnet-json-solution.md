...
**Expect** session 1 to have written the model's project and, because it was
the solution's first, `uat-json.slnx` at the root listing it. The plan
instruction tells a session that writes a module's first project to add it to
the solution file, and to write that file where there is none.
...
- **Framework —** it adds `bin/` and `obj/` to `.gitignore` with the root
  build files, and it writes those from `dabbler modules create` only when a
  module already holds a project file. Here every module was declared before
  any code existed, so nothing added them. Walk finding 3 — the
...
**Do this before anything else builds.** `dotnet build` and `dotnet test`
