...
/** Which invocation this is, and which conversation it continues. */
...
export function engineShape(engine: string, model: string | null): EngineShape | string {
  ...
  switch (engine) {
    ...
    case "copilot":
      ...
      //
      // The one adapter still resuming by recency, and deliberately: the
      // seat reports no conversation id on its output and no resume-by-id
      // has been measured against it. It carries the hazard the other two
      // shed, and saying so is better than a shape that pretends.
...
export function builtInEngine(
  ...
  return {
    ...
    invoke(invocation: EngineInvocation): Promise<EngineOutcome> {
      ...
      const context = { first: invocation.first, resumeId: invocation.resumeId };
      const argv = [program, ...leading, ...shape.argv(context, prompt)];
