...
export type Rounds = {
  ...
  transport?: string;
  /**
   * What failed on the way to this round's reviewer, in the transport's own words, where the call had to be made more than once. A call that is not delivered is retried against the SAME reviewer -- the one the operator chose -- and a retry that succeeds must not erase the fact that one was needed: a service that fails every round is a different problem from one that hiccuped, and only the record can tell them apart. Absent on a round whose first call answered, which is nearly every round.
   */
