...
import { treeDigest } from "../src/testEvidence.ts";
...
describe("what belongs to a step and what does not", () => {
  ...
  it("counts a session-plan.md edit as the step's work, mid-step and between instructions alike", () => {
    // Mid-step: the checks run against a digest the plan file is part of, so
    // an edit while they run moves the tree they were proving.
    const root = tempDir("plan-edit-");
    seed(root, { "src/widget.py": "x", "docs/sessions/session-plan.md": "### Session 1 of 1: A\n" });
    const restore = gitAnswers([
      [(args) => args.includes("--others"), { stdout: "" }],
      [(args) => args.includes("ls-files"), { stdout: "src/widget.py\0docs/sessions/session-plan.md\0" }],
    ]);
    try {
      const before = treeDigest(root, { sessionsDir: join(root, "docs", "sessions") });
      seed(root, { "docs/sessions/session-plan.md": "### Session 1 of 2: A\n" });
      assert.notEqual(treeDigest(root, { sessionsDir: join(root, "docs", "sessions") }), before);
    } finally {
      restore();
    }
    // Between instructions: the edit is in the step's change set, so a report
    // that leaves it out is refused, and one that names it carries the edit.
    const changed = stepChangedPaths(["src/widget.py", "docs/sessions/session-plan.md"], "docs/sessions");
    assert.deepEqual(changed, ["src/widget.py", "docs/sessions/session-plan.md"]);
    assert.deepEqual(rules(judgeReportFiles(report(), changed, () => true)), ["files-changed-omits"]);
  });
