...
export interface ModuleContract {
  ...
  /** The notes page as read: the human half, never rewritten by the framework. */
  readonly notes: string;
  readonly notesPath: string;
  /**
   * A `contract.yaml` beside the notes page, rendered through the file
   * form into `contract.md` beside it; null when the module keeps none.
   */
...
/** The rendered form of a `contract.yaml`, beside it. */
export function renderedPageOf(slug: string): string {
  return `${contractDirOf(slug)}/contract.md`;
}
...
export function renderModuleContract(
  ...
  const notes = readText(join(root, notesPath));
  ...
  if (existsSync(yamlPath)) {
    ...
    rendered = render(load(yamlPath), graph);
    renderedPath = renderedPageOf(slug);
  }
  const packageId = entry.package ?? slug;
  const base = { slug, notes, notesPath, rendered, renderedPath };
  if (mode === "package") {
