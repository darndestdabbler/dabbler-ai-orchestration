"""Route Set 029 Session 3 verification — Round B (reader + model + provider + tests).

Round B scope: reader source, SessionSetsModel.ts (extraction),
SessionSetsProvider.ts (refactored), and CHANGELOG excerpt.
Round A (separate script, already VERIFIED) covered the writer and
the marker schema doc. Pinned to gemini-pro to dodge the gpt-5-4
429s observed on the OpenAI Responses endpoint earlier this session.
"""
...
def main() -> int:
    ...
    spec_src = _read(
        ext_root / "src" / "test" / "playwright" / "orchestrator-indicator.spec.ts",
    )
    changelog = _read(ext_root / "CHANGELOG.md")
    # Bound the changelog excerpt to the [0.15.0] section only.
