...
so nothing it writes moves the repository's tree, and it keeps the VS Code
profile in `<repo>-vscode`.
...
which the walk copies beside `walk.jsonl` when it ends. A rerun under the same
`--run` keeps the previous log as `walk-attempt<N>.jsonl` rather than adding to
it.
...
with a screenshot taken just before it; answer the trust prompt once for the
scratch folder before a walk that must count the two apart.
