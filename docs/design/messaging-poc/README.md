...
| wait-inbox.mjs | The AI's background waiter: blocks until the next unread message in the inbox, prints it and exits. `--timeout <seconds>`, or a `waiter.config.json` holding `timeoutSeconds`, makes it exit with `NO MESSAGE` (a keep-alive). |
...
`tools/dabbler-ai-orchestration/scripts/vscode-launch.js`. The path to the
extension is a constant at the top of the driver.
