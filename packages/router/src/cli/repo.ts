...
export function retrunk(repoRoot: string, options: RetrunkOptions): RetrunkOutcome {
  ...
  // The operator kept the host's answer, so the work has to become it. The
  // two share no history in the case this verb was written for, which makes
  // this a force-push and the operator's approval an approval of a rewrite.
  ...
  if (shared !== true && options.rewriteHistory !== true) {
    return refused(
  ...
  const renamed = run("branch", "-m", record, target);
  if (renamed.code !== 0) {
    return { ...refused(`renaming the local '${record}' to '${target}' failed: ${renamed.stderr.trim()}`, commands) };
  }
  run("branch", "--set-upstream-to", `origin/${target}`, target);
  // The same rule as above: the branch origin's own HEAD names cannot go
  // until a person moves that setting.
  const deleted: string[] = [];
  if (record !== was) {
    const dropped = run("push", "origin", "--delete", record);
    if (dropped.code !== 0) {
      return { ...refused(`deleting '${record}' at origin failed: ${dropped.stderr.trim()}`, commands) };
    }
    deleted.push(record);
  }
  const pending = was !== null && was !== target ? was : null;
  ...
  return { refusal: null, commands, record, was, deleted: deleted[0] ?? null, pending, rewrote: true };
}
