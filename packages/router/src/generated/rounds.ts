...
export type Rounds = {
  ...
  transport?: string;
  /**
   * The verifier's read surface for this round: what it was granted, and what it did with it. mode 'none' is a round that could not look at the tree at all and is never equivalent to one that could; mode 'tools' is a round that could, and operations_granted says by which means. The seat holds all three tools (list, search, read). The direct-API path holds none by default and records 'none' — but where verification.settings.api_file_requests is on it is granted the single 'read', which it exercises by naming paths in its own answer for the framework to open, and such a round records 'tools' with 'read' as its one granted operation. in_scope means the operation was confined to the grant: a read names a path and is placed against the scope, while a search or listing is confined only when it also names a path — a pattern on its own reaches the whole tree and is recorded as unconfined. Each read carries a fidelity: 'verbatim' means the shown lines matched the bytes on disk, 'transformed' means they did not (the credential scrubber rewrites text before a model sees it, and a finding resting on a transformed read is weighable rather than trustable), 'unverified' means the comparison could not be made. A read the framework performed itself is 'verbatim' by construction and can never be 'transformed': the same process opened the file and sent it, so there is nothing in between to rewrite a line. On that path the boundaries are enforced rather than measured — a path outside the scope, a directory, or one past the read budget never reaches the filesystem and is recorded as the refusal it met — so over_budget there counts paths ASKED for past the budget, where on the seat it counts reads performed past it. 'writes' is the operation no tool performs: the verifier proposes a test file in its answer and the framework writes the bytes, so a proposal outside the declared test root is refused before anything reaches the filesystem. Every proposal and every requested path is recorded, applied or refused, because a boundary nobody can see being enforced is indistinguishable from one that is not there.
   */
  agency?: {
    mode: "tools" | "none";
