...
def planned(sandbox_repo, monkeypatch):
    ...
    register_session_start(set_dir, 1, engine="claude-code",
                           provider="anthropic")
    run_dir = ledger.session_run_dir(repo, set_dir.name, 1)
    write_plan(run_dir, new_plan(set_dir.name, 1, "first-things", [
...
class TestOpen:
    def test_refuses_a_second_step_while_one_is_in_flight(
        ...
        repo, set_dir, _ = planned
        assert run_step_open(set_dir, step_id=STEP_ID) == EXIT_OK
        assert run_step_open(set_dir, step_id=OTHER_STEP_ID) == EXIT_USAGE
        assert STEP_ID in capsys.readouterr().err
        assert ledger.open_step(repo, set_dir.name, 1)["step_id"] == STEP_ID

    def test_refuses_a_step_the_plan_does_not_declare(self, planned, capsys):
        repo, set_dir, _ = planned
        assert run_step_open(set_dir, step_id="invent-a-step") == EXIT_USAGE
        assert "not declared" in capsys.readouterr().err
        assert ledger.open_step(repo, set_dir.name, 1) is None
...
class TestEnvelope:
    def test_a_write_outside_the_envelope_refuses_the_close(
        ...
        repo, set_dir, _ = planned
        run_step_open(set_dir, step_id=STEP_ID)
        (repo / "widget.py").write_text("w = 1\n", encoding="utf-8")
        (repo / "rogue.py").write_text("r = 1\n", encoding="utf-8")

        assert run_step_close(set_dir) == EXIT_BLOCKING
    ...
    def test_an_approved_amendment_widens_the_envelope(
        ...
        assert run_step_amend(
            set_dir, reason="the widget needs a helper",
            added_files=["helper.py"],
        ) == EXIT_OK
        assert run_step_close(set_dir) == EXIT_OK
        assert ledger.open_step(repo, set_dir.name, 1) is None
    ...
    def test_a_closed_step_s_paths_do_not_refuse_the_next_step(self, planned):
        ...
        run_step_open(set_dir, step_id=OTHER_STEP_ID)
        (repo / "polish.py").write_text("p = 1\n", encoding="utf-8")
        assert run_step_close(set_dir) == EXIT_OK

        closed = ledger.read_step_events(repo, set_dir.name, 1)[-1]
        assert closed["envelope"]["inside"] == ["polish.py"]
        assert ledger.closed_step_ids(repo, set_dir.name, 1) == [
    ...
    def test_editing_a_closed_step_s_file_again_is_outside_this_envelope(
        ...
        assert ledger.closed_step_ids(repo, set_dir.name, 1) == [STEP_ID]
