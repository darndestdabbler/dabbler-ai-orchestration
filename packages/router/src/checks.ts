...
export function selectTests(
  ...
  /** The module a changed path belongs to: under its roots, or among its shared files. */
  const moduleOf = (rel: string): string | null => {
    if (moduleForm === null) return null;
    for (const entry of moduleForm.shape.modules) {
      const roots = entry.codeRoots.map((root) => (root === "." ? "" : root));
      if (roots.some((root) => root === "" || matchingPrefixes(rel, [root]).length > 0)) {
        return entry.slug;
      }
    }
    for (const entry of moduleForm.shape.modules) {
      const shared = moduleForm.sharedFiles?.get(entry.slug) ?? [];
      if (shared.length > 0 && matchingPrefixes(rel, shared).length > 0) return entry.slug;
    }
    return null;
  };
