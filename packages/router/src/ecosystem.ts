...
const MAVEN: Ecosystem = {
  key: "maven",
  contractProjectNames(packageId: string): ContractProjectNames {
    // `groupId:artifactId` as one string; the api and contract-test modules
    // take the artifact's name, the way Maven projects are laid out.
    const artifact = packageId.includes(":") ? packageId.slice(packageId.indexOf(":") + 1) : packageId;
    return {
      abstractions: `${artifact}-api`,
      contractTests: `${artifact}-contract-tests`,
      compatibility: (providerPackage: string) => {
        const provider = providerPackage.includes(":")
          ? providerPackage.slice(providerPackage.indexOf(":") + 1)
          : providerPackage;
