...
class Driver {
  ...
  private waiting(owner: Waiting["owner"], forWhat: string, by: string | null): Waiting {
    return waitingRecord(
      this.run.waiting ?? null,
      {
        ...
        for: forWhat,
        by,
        // Asked of the author's wait and of no other, because it is the
        // only one a waiter can have read. A new wait starts unread: the
        // beacon is compared against THIS wait, so a stamp from the last
        // instruction never answers for this one.
        ...(owner === "author" ? { waiter: false } : {}),
      },
      this.progressAt,
      nowIso(),
    );
  ...
  /**
   * The waiter's beacon, re-read while the author owes an answer.
   *
   * The AI picks the instruction up a moment after it is issued, and "no
   * waiter has read it" is the sentence a person acts on -- so it has to
   * stop being true the moment one does. Written only when the reading
   * CHANGES, and a refused save is swallowed: a poll is not where a lease
   * is contested, and the loop meets the same refusal at its next
   * transition, where it belongs.
   */
  private refreshWaiterReading(): void {
    const waiting = this.run.waiting ?? null;
    if (waiting === null || waiting.owner !== "author") return;
    if (waiting.waiter === true) return;
    const read = waiterSeenSince(this.repoRoot, this.sessionNumber, waiting.since);
    if (!read) return;
    this.run = { ...this.run, waiting: { ...waiting, waiter: true } };
    try {
      this.save();
    } catch {
      // Another driver holds the record; this reading is not worth ending
      // an invocation for.
    }
  }
  ...
  private async invoke(instruction: DriverInstruction): Promise<string | null> {
    ...
    const poll = setInterval(() => {
      // Whether anybody is listening is a reading, not a moment: a waiter
      // started after the instruction was issued changes the answer, and
      // the record is what every surface reads it from.
      this.refreshWaiterReading();
        ...
        latest:
          latest === undefined
            ...
            : {
                ...
                treeDigest: latest.treeDigest,
                surfaceDigest: latest.surfaceDigest,
                recordedAt: latest.recordedAt,
              },
        surfaceNow: surfaceDigest(this.repoRoot, suite.covers, { sessionsDir: this.sessionsDir }),
      };
    });
