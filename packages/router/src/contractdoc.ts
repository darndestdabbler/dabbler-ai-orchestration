...
export function renderModuleContract(
  ...
  // The notes page comes from the definition when there is one and from
  // the author otherwise; a declared seam with neither is refused by path.
  let notes: string;
  ...
  if (existsSync(yamlPath)) {
    ...
    notes = render(load(yamlPath), graph);
    notesRendered = true;
  } else if (existsSync(join(root, notesPath))) {
    notes = readText(join(root, notesPath));
  } else {
    throw new ContractError(
