 ...
 * total-timeout.
 *
 * A Windows batch shim -- which is what `copilot` resolves to on a seat
 * installed through VS Code -- cannot be spawned directly (Node answers
 * `EINVAL`), so it goes to `cmd.exe` with each argument quoted HERE rather
 * than reassembled from a string. `shell: true` would join the argv and let a
 * shell re-split it, which is precisely what an argv exists to avoid, and
 * cmd's own 8,191-character line would gut the handoff headroom. The
 * resolution and the quoting are `checks`', so the router has one answer
 * about how a program on this platform is reached.
 */
