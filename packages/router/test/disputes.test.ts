...
// undisputed. Path arithmetic against a temp directory, and the one git
// question a config load asks is answered from a table rather than spawned.
...
describe("an adjudication over a chosen third voice", () => {
  ...
  it("stops on a selection the round excludes rather than adjudicating with the provider that already reviewed", () => {
    ...
    const repo = tempDir();
    // The config is loaded from a temp root, and loading one asks git where
    // the repository is. The suite answers that rather than spawning it: a
    // test that shelled out would be reading the machine it runs on.
