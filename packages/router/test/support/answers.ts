...
export interface AnsweredRepo {
  ...
  /** A tag origin answers `ls-remote --tags` with. */
  remoteTag(name: string): void;
}
...
export function makeAnsweredRepo(
  ...
  const state = { status: "", ahead: 0, headTree: diskTree(repo), remoteTags: new Set<string>() };
  ...
  const restore = gitAnswers([
    ...
    [
      ...
      (args) => {
        ...
        return state.remoteTags.has(wanted)
          ? { stdout: `${HEAD_COMMIT}	refs/tags/${wanted}` }
          : { stdout: "" };
      },
    ],
    [["push"], { code: 0 }],
  ...
  return {
    ...
    remoteTag: (name) => { state.remoteTags.add(name); },
  };
}
