...
function candidateSubcommand(rest: readonly string[]): number {
  ...
  try {
    ...
    for (const slug of slugs) {
      ...
      // An application ships, and does not pack: its candidate is the bundle
      // record -- what it ships, at the versions the central pins name --
      // written into the tree before the run of record so the land carries
      // it. Refused, and the candidate with it, while a pin is a dev version.
      if (entry !== undefined && entry.kind === "application" && entry.package === null) {
        ...
        const record = bundleRecord(shape, entry, centralPins(props), readRecords(workspaceRoot), project === undefined ? "" : readFileSync(join(workspaceRoot, project), "utf8"), {
          session,
          sourceCommit: runGit(workspaceRoot, ["rev-parse", "HEAD"]).stdout || null,
        });
