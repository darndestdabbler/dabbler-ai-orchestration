...
export interface ProjectionExternal {
   ...
   */
  usedBy?: string[];
  /** What each consuming repository pins it to, read from their build files. */
  pins?: { repository: string; version: string }[];
  pinned?: string | null;
...
export function childrenOf(node: SolutionNode, p: Projection): SolutionNode[] {
  switch (node.kind) {
    ...
    case "external": {
      ...
      // Only when more than this repository takes it. One consumer is what
      // the row already says.
      return (e?.usedBy?.length ?? 0) > 1
        ? [{ kind: "externalUsedBy" as const, id: node.id }]
        : [];
    }
    case "externalUsedBy": {
      const e = externals(p).find((row) => row.id === node.id);
      return (e?.usedBy ?? []).map((repository) => ({
...
export function descriptorFor(
  ...
  switch (node.kind) {
    ...
    case "externalUsedBy": {
      const e = externals(p).find((row) => row.id === node.id);
      const versions = new Set((e?.pins ?? []).map((pin) => pin.version));
      return {
        id: `externalUsedBy:${node.id}`,
        label: "Used by",
        description: `${(e?.usedBy ?? []).length}`,
        tooltip:
          versions.size > 1
    ...
    case "externalConsumer": {
      ...
      return {
        ...
        description: pin ? `v${pin.version}` : undefined,
        icon: { id: "arrow-small-right", tone: "muted" },
        expandable: false,
      };
    }
