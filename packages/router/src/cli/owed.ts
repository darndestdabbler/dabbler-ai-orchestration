...
function attachRemote(repoRoot: string, url: string): string {
  ...
  if (pushed.code !== 0) {
    // The remote is added and the push is not; say both, because the next
    // attempt must not try to add it again.
    return (
      `the remote was added but the push failed: ${
        pushed.stderr.trim() || "unknown error"
      }.`
    );
  }
  return "";
