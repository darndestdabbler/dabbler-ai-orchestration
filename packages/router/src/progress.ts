...
export function readRawSessionState(
  ...
  const path = join(sessionsDir, STATE_FILENAME);
  if (!existsSync(path)) return null;
  let raw: unknown;
  try {
    raw = JSON.parse(readFileSync(path, "utf8"));
  } catch (error) {
    // A malformed or undecodable document is unusable state, which is what
    // Python answers for it. Anything else -- a permission denial -- is the
    // caller's to see.
