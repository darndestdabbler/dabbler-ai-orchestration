...
class Driver {
  ...
  private async invoke(instruction: DriverInstruction): Promise<string | null> {
    ...
    if (interrupted && stopRequested) throw new Stop("interrupted", String(reason));
    return interrupted ? reason : null;
  }

  /**
   * A request that arrived while no invocation was running. A plain one had
   * nothing to end and is discarded rather than ending the next invocation
   * on its first tick -- the log says so. A stop is not discarded: the
   * person meant the loop, and the loop halts here instead.
   */
  private honourPendingStop(): void {
    ...
    this.log("interrupt-discarded", { reason: pending.reason, why: "no invocation was running" });
  }
