...
>
> **Still unknown from the work computer:** its `dabbler.release`, the task-declaration entry's `releasable`/`holdReason`, `cancelledReason` vs `forceClosed`, the engine, `dabbler version`; and for the second test `run.json`, `supervision.jsonl`, `driver/loop.log`.
