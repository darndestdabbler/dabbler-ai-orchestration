...
export function seatScope(
  ...
  // Only what is known. An empty host is not a fact about this machine, and
  // a scope that carried one would make two readings of the same seat
  ...
  if (cliVersion) scope["cli_version"] = cliVersion;
  return scope;
}
...
export function seatCatalogBlock(
  ...
  return {
    ...
    source: SOURCE_SEAT,
    scope: seatScope(enumeration.cli_version, identity),
...
export function recordSeatEnumeration(
  ...
  const previous = blockFor(readCatalog(), TRANSPORT_SEAT, seatScope(seat.cli_version, identity));
...
export function seatModels(): readonly CatalogModel[] | null {
  return blockOf(readCatalog(), TRANSPORT_SEAT)?.models ?? null;
}
