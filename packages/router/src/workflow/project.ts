...
export function externalComponents(root: string): Node[] {
  ...
  try {
    ...
    // A declaration that is present and wrong is the dependency check's
    // finding to report, not the Explorer's to crash on.
    return [];
  }
  if (deps === null || deps.consumes.length === 0) return [];
  ...
  for (const ref of self?.refs ?? []) {
    if (ref.version !== null && !pins.has(ref.id)) pins.set(ref.id, ref.version);
  }
  const published = new Map<string, string>();
  for (const member of members.slice(1)) {
    if (member.root === null || member.duplicateOf !== null) continue;
    for (const artifact of publishedVersions(member.root, deps.consumes.map((e) => e.id))) {
  ...
  return deps.consumes.map((edge) => {
    const where = locateProducer(root, edge.producedBy, deps.solution);
    const mine = findings.filter((finding) => finding.id === edge.id);
    ...
    return {
      ...
      pinned: pins.get(edge.id) ?? null,
      published: published.get(edge.id) ?? null,
      ...
      // At most one, and the order is by what it costs the reader: a pin
      // behind a release is an upgrade to do, a producer ahead of its
      // releases is not one yet, and a feed nobody registered is why a
      // restore is about to fail.
