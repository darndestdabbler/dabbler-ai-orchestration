// The review that gates a plan: the free checks, the reviewer's answer, the
// anti-grind rules, and a review round driven by an injected dispatch.
// Plans live in a temp directory; nothing asks git or a model.
import assert from "node:assert/strict";
import { mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { describe, it } from "node:test";

import { approvePlan, deriveRiskFlags, newPlan, readPlan, writePlan } from "../src/approvedPlan.ts";
import {
...
function approvedPlanIn(directory: string): void {
