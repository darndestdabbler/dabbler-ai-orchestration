...
/**
 * The checkout these tests stand in: a scratch one that names nothing.
 *
 * A role's selection is read from the checkout first, so a suite standing in
 * the real one would read whatever reviewer the operator's own Configuration
 * last saved there.
 */
let checkout = "";
let leaveCheckout: () => void = () => undefined;

beforeEach(() => {
  setProviderKeys();
  checkout = tempDir("role-selection-");
  leaveCheckout = gitAnswers([[["rev-parse", "--show-toplevel"], { stdout: checkout.split("\\").join("/") }]]);
...
describe("a model a person selected", () => {
  ...
  it("is this checkout's choice where it names one, over the machine's default", () => {
    ...
    assert.equal(roleDeclaration(makeConfig(), ROLE_PRIMARY_REVIEWER).selected, "g-one");
    assert.deepEqual(ids(resolveRole(makeConfig(), ROLE_PRIMARY_REVIEWER, CANDIDATES)), ["g-one"]);
  });
});
