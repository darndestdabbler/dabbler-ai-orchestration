...
session** (`rejected-thrice`) — the reasons are on the run state and a
re-run asks for the step afresh. If a step genuinely cannot be done, say
so instead: `--status blocked` with the reason in `--notes`.
