...
the engine never writes the ledger directly).
...
each) and `checks`, each an `argv` spawned with no shell — a check is run,
never read. The reader refuses a duplicate `id`, which the schema cannot.
