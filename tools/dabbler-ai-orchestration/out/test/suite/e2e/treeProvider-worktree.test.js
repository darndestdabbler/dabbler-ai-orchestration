...
// bare remote).
function seedSetInWorktree(worktreeDir, slug, totalSessions) {
    ...
    if (addProc.status !== 0) {
        throw new Error(`git add failed in worktree ${worktreeDir}: status=${addProc.status} ` +
            `stderr=${addProc.stderr?.toString() || ""}`);
    }
    const commitProc = cp.spawnSync("git", [
        "-c", "user.name=Harness",
    ...
    if (commitProc.status !== 0) {
        throw new Error(`git commit failed in worktree ${worktreeDir}: status=${commitProc.status} ` +
...
suite("Layer 2 e2e — sibling-worktree discovery", function () {
    ...
    let tmpPath;
    setup(() => {
        tmpPath = (0, e2eHarness_1.makeTmpDir)("e2e-worktree");
    });
    teardown(() => {
    ...
    test("session sets in a canonical sibling worktree appear in the tree", async () => {
        ...
        const primary = (0, e2eHarness_1.makeSet)(tmpPath, "wt-primary", 3);
        ...
        const wtPath = (0, e2eHarness_1.makeSiblingWorktree)(primary, "feature-branch");
        seedSetInWorktree(wtPath, "wt-sibling", 4);
        await (0, e2eHarness_1.replaceWorkspaceFolders)(primary.repo_root);
        ...
        assert.deepStrictEqual(labels, new Set(["wt-primary", "wt-sibling"]), `both sets should surface (presence only — sort coverage is in multiset suite); got ${JSON.stringify([...labels])}`);
    });
    test("a set present in BOTH worktrees deduplicates by name AND state-rank precedence wins", async () => {
        ...
        const primary = (0, e2eHarness_1.makeSet)(tmpPath, "wt-dup", 3);
        (0, e2eHarness_1.startSession)(primary, 1);
        const wtPath = (0, e2eHarness_1.makeSiblingWorktree)(primary, "feature-branch-2");
        ...
        assert.strictEqual(dupCount, 1, `set 'wt-dup' must appear exactly once across all buckets; saw ${dupCount}: ${JSON.stringify(allLabels)}`);
        ...
        assert.ok(inProgressLabels.includes("wt-dup"), `STATE_RANK tie-break must surface the in-progress copy; ` +
            `got in-progress=${JSON.stringify(inProgressLabels)}`);
        const notStartedLabels = (0, e2eHarness_1.childrenOfGroup)(provider, "not-started")
            .map((it) => String(it.label));
        assert.ok(!notStartedLabels.includes("wt-dup"), `not-started copy must NOT win; got not-started=${JSON.stringify(notStartedLabels)}`);
    });
    test("primary-only sets still surface when no sibling worktrees exist", async () => {
        ...
        const primary = (0, e2eHarness_1.makeSet)(tmpPath, "wt-primary-only", 2);
        const b = (0, e2eHarness_1.makeAdditionalSet)(primary, "wt-primary-second", 3);
        void b;
        await (0, e2eHarness_1.replaceWorkspaceFolders)(primary.repo_root);
        const provider = (0, e2eHarness_1.buildProvider)();
        ...
        assert.deepStrictEqual(labels, ["wt-primary-only", "wt-primary-second"], `primary sets must surface in alphabetical not-started order; got ${JSON.stringify(labels)}`);
    });
});
//# sourceMappingURL=treeProvider-worktree.test.js.map