...
export function cleanRepoAnswers(repo: string): () => void {
  return gitAnswers([
    ...
    // The worktree snapshot seeds its throwaway index from the real one; an
    // answered checkout has none on disk, and the snapshot falls back to HEAD.
...
export function makeAnsweredRepo(
  ...
  const restore = gitAnswers([
    ...
    // The worktree snapshot seeds its throwaway index from the real one; none
    // is on disk here, and the snapshot falls back to HEAD's tree.
    [["rev-parse", "--git-path", "index"], { stdout: ".git/index" }],
    [
