...
import { type SolutionShape, dependencyOrder, parseEntries, impliedDeployables, solutionShape } from "../src/modules.ts";
...
describe("the root build files", () => {
  ...
  it("adds MSBuild's output to the ignore file bootstrap wrote, so the same source keeps its dev version", () => {
    ...
    // obj/ moved the source digest with every build.
    const bootstrapped = "# machine-side state\n.dabbler/\n";
    const repo = makeRepo({
    ...
    const first = ensureRootFiles(repo, shape);
    ...
    // And the pack is idempotent through a build: the module is packed for
    // real (against a scripted `dotnet pack`, over the repository's own
    // digest), MSBuild's output then appears under its code root, and the
    // second pack of that same source gives back the version the first one
    // did rather than minting a new one.
    const pack = (): string =>
      packModule(repo, shape, "model", {
