...
import { roundRef } from "../src/evidence.ts";
...
describe("appending a round", () => {
  /**
   * A repository holding one real tree, so the anchor has an object to
   * reach. `appendRound` anchors in the same call as the append, and a
   * tree the store does not hold gets no anchor -- so a fixture without a
   * real object would test only the silent half.
   */
  function repoWithTree(): { repo: string; tree: string } {
    const repo = makeLedgerRepo();
    writeFileSync(join(repo, "widget.py"), "x = 1\n", "utf8");
  ...
  it("makes the completion tree reachable from the round's ref", () => {
    // A snapshot written through a throwaway index is garbage-collectable
    // and unpushable on its own; the anchor is what makes the next round's
    // baseline survive.
    const { repo, tree } = repoWithTree();
    const row = appendRound(repo, 1, { ...ROUND, completion_tree: tree });
    const anchored = execFileSync(
      "git",
      ["rev-parse", `${roundRef(1, 1)}^{tree}`],
      { cwd: repo, encoding: "utf8" },
    ).trim();
    expect(anchored).toBe(tree);
    expect(row["anchor_commit"]).toMatch(/^[0-9a-f]{40}$/);
  });

  it("records the row the reader reads back", () => {
    const { repo, tree } = repoWithTree();
  ...
  it("carries no anchor for a tree this store does not hold", () => {
    ...
    expect(row).not.toHaveProperty("anchor_commit");
  });

  it("refuses a second row for the same round", () => {
    // Rounds are immutable history, never rewritten.
    const { repo, tree } = repoWithTree();
    appendRound(repo, 1, { ...ROUND, completion_tree: tree });
    expect(() => appendRound(repo, 1, { ...ROUND, completion_tree: tree })).toThrow(
  ...
  it("refuses a row that does not match the schema, writing nothing", () => {
    const repo = makeLedgerRepo();
    expect(() => appendRound(repo, 1, { round: 1 })).toThrow(LedgerError);
  ...
  it("takes the next round number from the last row", () => {
    const { repo, tree } = repoWithTree();
    appendRound(repo, 1, { ...ROUND, completion_tree: tree });
    expect(nextRoundNumber(repo, 1)).toBe(2);
  });
});
