...
export interface AgentConnection {
  ...
  /** What the agent said it can do, as it said it: the handshake's reply. */
  readonly capabilities: Record<string, unknown>;
  /**
   * What the agent answered when the conversation was opened, as it said
   * it: the reply to `session/new` or `session/load` (the Copilot seat lists
   * its models with their multipliers, its modes and its options there).
   * Empty for a protocol that negotiates nothing at that point.
   */
  ...
  return {
    ...
    async open(options: OpenOptions): Promise<AgentConnection> {
      ...
      if (options.resumeId) {
        loading = true;
        try {
          session = await request("session/load", { sessionId: options.resumeId, cwd, mcpServers: [] });
        } finally {
          loading = false;
        }
        ...
        sessionId = asString(session["sessionId"]) || null;
  ...
  return {
    ...
    open(options: OpenOptions): Promise<AgentConnection> {
      ...
      return Promise.resolve({
        ...
        // stream-json negotiates nothing at the open: the `init` event is
        // the whole of what the CLI says about itself, and it is above.
        session: {},
        history: [],
        async prompt(text, onEvent): Promise<TurnOutcome> {
          if (turn !== null) throw new Error("a turn is already in flight on this conversation");
