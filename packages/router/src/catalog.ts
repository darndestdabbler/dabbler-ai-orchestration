 ...
 *
 * A block read for another scope is unread, not stale. That is the one guard
 * this file has already needed: the catalog it replaces carried `seat_id`
 * as provenance that nothing ever compared against the machine reading it,
 * which is exactly how one developer's seat came to be authoritative on
 * every other.
 */
...
/**
 * The block for one transport, whatever scope it was read under.
 *
 * What a reader gets: a machine reads the catalog it has. The scope check is
 * the REFRESH's question -- whether this reading may be believed for this
 * machine, or must be taken again -- and asking it of every read would make
 * a pane need a seat's identity before it could draw a row.
 */
export function blockOf(catalog: Catalog | null, transport: string): TransportBlock | null {
