  ...
  for (const entry of sessions) {
    ...
    for (const line of readText(path).split("\n")) {
      ...
      for (const artifact of (row["artifacts"] as string[] | undefined) ?? []) {
        const parsed = artifactIdentity(String(artifact), known);
...
export function artifactIdentity(
  ...
  const body = base.replace(/\.(?:nupkg|snupkg)$/, "").replace(/\.jar$/, "");

  // A known id first, and it is the only reading that is not a guess.
  // `Foo.2.0.1.0.0.nupkg` splits four ways -- package `Foo` at `2.0.1.0.0`,
  // package `Foo.2.0` at `1.0.0` -- and nothing in the file name decides
  // between them. What decides is that one of those is a package somebody
  // declared.
  ...
  const nuget = /^(.*?)\.(\d+(?:\.\d+)*(?:-[0-9A-Za-z.-]+)?)$/.exec(body);
  if (base.endsWith("nupkg") || base.endsWith("snupkg")) {
    return nuget ? { packageId: nuget[1], version: nuget[2] } : null;
  }
  const jar = /^(.*?)-(\d+(?:\.\d+)*(?:-[0-9A-Za-z.-]+)?)$/.exec(body);
  return base.endsWith(".jar") && jar ? { packageId: jar[1], version: jar[2] } : null;
}
...
export function reconcileResolution(
  ...
  // never cut.
  // The ids somebody declared, which is what makes an artifact name readable.
  const consumedIds = [
    ...new Set(members.flatMap((member) => (member.deps?.consumes ?? []).map((e) => e.id))),
  ];
  const published = new Map<string, string[]>();
  const built = new Map<string, string>();
  for (const member of members.slice(1)) {
    if (member.root === null || member.duplicateOf !== null) continue;
    for (const artifact of publishedVersions(member.root, consumedIds)) {
 ...
 *
 * Located by POSITION in the parsed array and then walked in the text, not by
 * searching for the first `"id"`: an edge whose producer happens to be named
 * the same as the package would otherwise match the nested `producedBy`
 * object and leave the edge itself untouched.
 */
function edgeSpan(text: string, packageId: string): readonly [number, number] | null {
  let wanted = -1;
  try {
    const parsed = JSON.parse(text) as { consumes?: { id?: string }[] };
    wanted = (parsed.consumes ?? []).findIndex((edge) => edge.id === packageId);
  } catch {
    return null;
  }
  if (wanted < 0) return null;

  const key = /"consumes"\s*:\s*\[/.exec(text);
  if (key === null || key.index === undefined) return null;
  ...
  while (index < text.length) {
    ...
    if (seen === wanted) return [start, index] as const;
    seen += 1;
  }
  return null;
}
