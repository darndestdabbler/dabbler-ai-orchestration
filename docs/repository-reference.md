...
| `dabbler-homehealthcare-accessdb` | not used | VS Code Marketplace |

> **Set 112 migration owed — CLOSED by the operator (2026-08-09).**
> `dabbler-homehealthcare-accessdb` ran on the Lightweight tier, which Set
> 112 removed. Any of its sets still declaring `tier: lightweight` will be
> refused by the loader as of `dabbler-ai-router` **`1.0.0`**. At the Set
> 112 UAT walk the operator declined the cross-repo notice: **that repo is
> the operator's own**, so the migration is theirs to do directly and no
> notice needs sending. [`docs/cross-repo-lightweight-removal-notice.md`](cross-repo-lightweight-removal-notice.md)
> stays as the written remedy for anyone who arrives at the break later —
> it is a reference now, not an outbound action.

### Current release status

| Surface | Current version | State | Canonical details |
|---|---|---|---|
| `dabbler-ai-router` | `1.0.0` (live) | **`1.0.0` PUBLISHED 2026-08-15** — confirmed live on PyPI by Set 133 S2 against the registry itself rather than the workflow's exit code: `https://pypi.org/pypi/dabbler-ai-router/json` reports `info.version` `1.0.0`, with `dabbler_ai_router-1.0.0-py3-none-any.whl` and `dabbler_ai_router-1.0.0.tar.gz` both uploaded 2026-08-15T15:02:45Z. Tag `v1.0.0`, tagged commit `6f195bd8`, release.yml run [31891571451](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891571451), Test green on the tagged commit run [31891207265](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891207265). Staged 2026-08-09 by Set 112 S3 and held six days while twenty sets (113-132) landed on top of it: the operator deferred the publish until Set 114 completed (decision at the Set 112 UAT walk), Set 116 S3 recorded that condition satisfied, and Set 133 S1 made the release content describe itself truthfully before the tag — the read-only tool grant's missing changelog entry written, and the breaking change's symptom, one-line fix and remedy link moved to the top of the `1.0.0` section. The first MAJOR release, and a genuinely BREAKING one: a session set can no longer declare `tier: lightweight`, and a spec that does fails to load with a one-line migration message. `docs/cross-repo-lightweight-removal-notice.md` is the consumer remedy and **sending it is operator-gated**. The release also carries the previously unpublished router-side work of Sets 105, 107, 109, 110, 111, 114 and 116 (the phased verification loop's enforced bounds, `decision_journal`, `acceptance_harness`, `run_of_record` + the `test_run_fresh` gate, `disposition.uat` + the `uat_walk_recorded` gate, `session_checklist` + its recorded posting cadence and the `checklist_posted` gate, the seeded session plan, `spec_admission`, dispatch-time provider-key validation; and from Set 116 — `pytest-xdist` as the documented default with structured `durationSeconds` in `test-runs.jsonl`, ONE round budget covering the close backstop with its rounds in `sN-rounds.jsonl`, and the operator's gate ruling: three blocking gates plus two transactional preconditions plus five warn-not-block checks, with `test_run_fresh` now governing every test layer). **Set 116 adds two consumer-visible changes**: every `gate_results` row carries a `blocking` boolean, and a session touching a suite's covered surfaces owes that suite a fresh green run of record before close. An anti-resurrection gate (`ai_router/scripts/lightweight_resurrection_guard.py`, CI-wired) is the executable proof the tier stays removed. Previous registry-live: **`0.34.0` PUBLISHED 2026-07-15** (Set 104 — Copilot CLI large-prompt file handoff; tag `v0.34.0`, operator-authorized 2026-07-15), superseding the then-live `0.33.0`. *(Corrected 2026-08-09 by Set 112 S3 round-1 verification, which found this row still naming `0.33.0` as live nine releases into the changelog's account — the publish had happened and the row was never updated. Same failure mode the extension row records for `0.46.0`.)* Previous registry-live: **`0.33.0`, published 2026-07-14 (coordinated release, operator-authorized before vacation)** — Set 096's consequence-graded severity rubric + the phased verification loop (tag `v0.33.0`, tagged commit `40f94e5`, release.yml run [29363250286](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/29363250286), Test green on the tagged commit; confirmed live on PyPI at `/project/dabbler-ai-router/0.33.0/`). Interim registry-live before this (the earlier "0.30.0" claim below was stale): `0.32.0` — the out-of-band verification-loop remediation, published 2026-07-10 (release.yml run [29104969444](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/29104969444)); the Set-086 `copilot_preflight` shipped in `0.31.0`, folded into `0.32.0`. Earlier lineage — **Published (Set 085).** `0.30.0` — the preload manifest + ratcheting ceiling gate (`guidance:` `preload:` config, `guidance_report` per-file/total `--check`, `stamp:` opt-in headers, the bundled-default manifest-source guard) — published 2026-07-07 on operator authorization (tag `v0.30.0`, tagged commit `15f4823`, release.yml run [28894990925](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28894990925), Test green on the tagged commit). Previous registry-live: `0.29.0`. `v0.29.0` — the combined Sets 083 + 084 release (verify_session CLI + verification-integrity gate + mandatory Full-tier verification; identity-is-the-underlying-model, dynamic verifier exclusion, stamped evidence, the close backstop; plus a Windows drive-letter path-case fix found by the Set 084 out-of-band Copilot UAT) — published 2026-07-07 (release.yml run [28872574166](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28872574166), tagged commit `477e625`, Test green on the tagged commit; confirmed live on PyPI). 0.29.0 is the number Set 083 never shipped in between (superseded by 084). Previous: `v0.28.0` (Set 078 — Copilot CLI hybrid tier) published 2026-07-04 (release.yml run [28718741271](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28718741271), tagged commit `a391f61`); `v0.27.0` (Set 077) published 2026-07-04 (release.yml run [28718682653](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28718682653), tagged commit `51fc437`). All confirmed live on PyPI. | `pyproject.toml`, `ai_router/CHANGELOG.md` |
| `dabbler-ai-orchestration` VS Code extension | `0.51.0` (live) | **`0.51.0` PUBLISHED 2026-08-15** — live on the VS Code Marketplace, confirmed by Set 133 S2 against the gallery API rather than the workflow's exit code: `extensionquery` returns `DarndestDabbler.dabbler-ai-orchestration` at version `0.51.0`, `lastUpdated` 2026-08-15T15:09:32Z. Tag `vsix-v0.51.0`, tagged commit `6f195bd8` — the same commit as router `v1.0.0`, so the two shipped together as the staging note expected — publish-vscode.yml run [31891572217](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891572217), Test green on the tagged commit run [31891207265](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891207265). **Open VSX: NOT published, and never has been.** That run's *Publish to Open VSX Registry* job is green, but green there means skipped: `OVSX_PAT` is unset in the `openvsx` environment, so the step prints `OVSX_PAT not configured for the openvsx environment; skipping Open VSX publish` and exits 0 by design. The registry confirms it from the other side — `open-vsx.org/api/DarndestDabbler/dabbler-ai-orchestration` is 404, and so is the namespace `DarndestDabbler`. **Read a green Open VSX job as "did not fail", never as "published"**; only the registry API distinguishes the two. *(Corrected here 2026-08-15 by Set 133 S2: the `0.45.0` entry below claimed it was published to Open VSX, and its run [29363250708](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/29363250708) logs the same `OVSX_PAT not configured` skip — no version of this extension has ever reached Open VSX. Enabling it is step 5 of [`docs/planning/marketplace-release-process.md`](planning/marketplace-release-process.md), an operator action.)* Staged 2026-08-10 by Set 114 S3. The Work Explorer's fifth tree level: an **in-flight** session row expands to its step checklist — the plan `start_session` seeded from `spec.md`, reconciled against the logged steps, with the same lifecycle glyphs and `<- here` on the current step. Only the in-flight session expands (a finished one is a leaf), and an absent / unreadable / silent activity log degrades to **no children**, never a stale or invented list. The rule that builds those rows is a TypeScript mirror of `ai_router/session_checklist.py`; drift is gated by `ai_router/tests/fixtures/session-step-parity.json`, a twelve-case corpus **both** suites assert against. **It supersedes `0.50.0` (Set 112's Lightweight-tier removal — the Getting Started form's tier radio and its verification-mode sub-choice gone, provider access now the form's first and only setup question, and the tier's Explorer machinery `switchTier` / `tierLegibility` / `tierMarkerStore` / `tierRewrite` / `verificationModeRewrite` / `setupVerification` / `externalVerification` deleted) and `0.49.0` (Set 110's native-tree Work Explorer), both staged and never published** — all three sets' extension work ships for the first time in this artifact. Requires `dabbler-ai-router` `1.0.0`. Previous registry-live: **`0.47.0` PUBLISHED 2026-07-30** (tag `vsix-v0.47.0`, tagged commit `7f2f2f8`, publish-vscode.yml run [30561157896](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/30561157896), Test green on the tagged commit [30559901882](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/30559901882)). Staged 2026-07-30 by Set 107 S1. The first-run entry point: the `Dabbler: Try a sample project` command and its canonical sample bundle (`docs/templates/sample-project/`, copied to `dist/templates/sample-project/`), plus `azure-pipelines.yml.template` in the consumer-bootstrap bundle (the ADO counterpart to `monorepo-ci.yml`, closing the dead end that made ADO staff abandon the tutorial). **The tag landed on Session 1's last commit, not Session 2's** — the operator tagged from a checkout predating S2's pushes, and `package.json` was already `0.47.0` from S1 so the version check passed. What shipped is exactly S1's tree; **Set 107 S2's two consumer-bootstrap template link fixes are NOT in the published VSIX** and sit under `[Unreleased]` in the extension CHANGELOG (operator decision 2026-07-30: fold into the next release, do not cut `0.48.0` for a link description). Set 107 S3's acceptance walk is valid against this build — the published sample bundle is byte-identical to the one the new `hello-world.md` describes. A router-side fix landed in S1 (`close_session` no longer exits with a raw traceback when stdin has no tty) but **`0.47.0` does not deliver it** — the sample command pip-installs the currently published router, so the fix reaches users only when `dabbler-ai-router` is next published (`ai_router/CHANGELOG.md` `[Unreleased]`; publish operator-gated). **`0.46.0` PUBLISHED 2026-07-29** (tag `vsix-v0.46.0`, publish-vscode.yml run [30438358346](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/30438358346) — corrected 2026-07-30 by Set 107 S1, which found this row still claiming 0.46.0 was staged-not-published and `0.45.0` live; the publish had already happened and the row was never updated). Staged 2026-07-28 by Set 106 S2. Template-bundle-only: the scaffolded `CODEOWNERS`, `monorepo-ci.yml`, and `getting-started.md` re-cut to agree with the rewritten Hello World tutorial (two-module cast; one always-running aggregate job renamed `test`; the deleted review-prompt URL removed). No extension code changed and `dabbler-ai-router` was deliberately not bumped (Set 106 S2 accrued zero `ai_router/` changes). Previous registry-live: `0.45.0`. **Published 2026-07-14 (coordinated release, operator-authorized before vacation).** `0.45.0` — one VSIX carrying Set 102's confirm-gated git-workflow automation commands (Open PR / Finalize merged set / Cut release tag / hotfix / rollback), Set 097's `Dabbler: Set Up Copilot Seat` + the seat-status-revert fix (interim `0.43.0`), and Sets 098–101's module-lifecycle simplification (interim `0.44.0`) (tag `vsix-v0.45.0`, tagged commit `40f94e5`, publish-vscode.yml run [29363250708](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/29363250708), Test green on the tagged commit; published to the VS Code Marketplace — **not** Open VSX, see the correction above). Interim registry-live before this (the earlier "0.40.0" claim below was stale): `0.42.0` — the Work Explorer module-first redesign (Sets 087 + 091–094), published 2026-07-13 (publish-vscode.yml run [29237575741](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/29237575741)). Earlier lineage — **Published (Set 085).** `0.40.0` — template-bundle-only: the scaffolded consumer-bootstrap bundle links the session constitution as the happy-path operating doc and marks the workflow doc on-demand (no extension code changes) — published 2026-07-07 on operator authorization (tag `vsix-v0.40.0`, tagged commit `15f4823`, publish-vscode.yml run [28894990724](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28894990724), Test green on the tagged commit). Previous registry-live: `0.39.0`. `vsix-v0.39.0` — the combined Sets 083 + 084 release: the scaffolded consumer-bootstrap bundle now teaches mandatory Full-tier verification (no skip), the Copilot-seat `--model` requirement / identity-from-model, dynamic verifier exclusion + `verification_unavailable`, and the close backstop; cold-start goldens + bundled dist templates regenerated. Published 2026-07-07 (publish-vscode.yml run [28872587000](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28872587000), tagged commit `477e625`, Test green on the tagged commit). Previous: `0.38.0` Set 082 (omit `verificationMode` from Full-tier scaffolds; extension-only — `ai_router` untouched, router stays `0.28.0`). A Full-tier scaffold no longer renders a `verificationMode:` config line (whole-line `{{VERIFICATION_MODE_LINE}}` token; Lightweight renders the exact pre-082 line, comment included) and no longer writes the `.dabbler/verification-mode` marker (Lightweight-only; on Full the marker is neither written nor deleted, so a prior Lightweight pick survives a tier round-trip). `buildSessionGenPrompt` hard-requirements rescoped: `full` sets OMIT the field. Lightweight scaffold output byte-identical to 0.37.0 (golden-snapshot tripwire). Per-set UAT walked and attested 2026-07-06: all 5 functional items PASS across 3 walks incl. one REAL cold-start Build per tier from fresh empty folders (L-079-3) and a Full-over-Lightweight re-Build asserting marker preservation. Published 2026-07-06 (tag `vsix-v0.38.0`, tagged commit `e9bbfae`, publish-vscode.yml run [28806840851](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28806840851); Test run [28806467299](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28806467299) green on the tagged commit). Previous: `0.37.0` (Set 081) published 2026-07-05 (tag `vsix-v0.37.0`, tagged commit `bc3b43f`, publish-vscode.yml run [28764677942](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/28764677942)). | `tools/dabbler-ai-orchestration/package.json`, `tools/dabbler-ai-orchestration/CHANGELOG.md` |

### Recent version walk

- **`dabbler-ai-router 1.0.0` + Extension `0.51.0`** — the coordinated
  release of 2026-08-15, both **published** from the same commit
  `6f195bd8` (tags `v1.0.0` and `vsix-v0.51.0`; release.yml run
  [31891571451](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891571451), publish-vscode.yml run
  [31891572217](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891572217), Test green on the tagged commit run
  [31891207265](https://github.com/darndestdabbler/dabbler-ai-orchestration/actions/runs/31891207265)). Confirmed live against the registries
  themselves by Set 133 S2 — PyPI's JSON API for the router, the gallery
  `extensionquery` for the extension — because a green publish job is not
  evidence of a publish. **Open VSX was not published and never has been**;
  its job is green because `OVSX_PAT` is unset and the step skips
  non-fatally by design. The router is the **first MAJOR release** and a
  genuinely BREAKING one: a session set can no longer declare
  `tier: lightweight`, and a spec that does now fails at config load with a
  one-line migration message — the `1.0.0` changelog section opens with that
  symptom, the one-line fix, and a link to
  [`docs/cross-repo-lightweight-removal-notice.md`](cross-repo-lightweight-removal-notice.md),
  because a public package cannot have its consumers enumerated and swept.
  It also carries the read-only tool grant (Set 125) that had shipped in code
  but never in the release notes: routed calls now hold `view` / `grep` /
  `glob` instead of `--allow-all-tools`, which matters most for a verifier,
  since one able to edit the code it judges can report VERIFIED on its own
  edit. The extension ships Sets 110, 112 and 114's work for the first time —
  the native-tree AI Work Explorer, the Lightweight tier's removal from the
  Getting Started form, and the in-flight session's step checklist as the
  tree's fifth level — and it is what stops new specs being written with a
  tier line the router now refuses. Rollback: pin
  `dabbler-ai-router==0.34.0`; install extension `0.47.0`
  (`code --install-extension DarndestDabbler.dabbler-ai-orchestration@0.47.0`).

- **`dabbler-ai-router 0.32.0`** — **out-of-band verification-loop
  remediation** (published to PyPI 2026-07-10; the note below was
  written while that publish was still operator-gated). The framework's
  own verification loop had become a "runaway train"
...
- **Extension `0.30.0` + `dabbler-ai-router 0.17.0`** — Sets 061 + 062 combined (Explorer UX polish: `N/M+`, `lw` marker, blocked chain marker, `Switch Tier…`; Lightweight verification affordance: `v?`/`v+` markers, Verification Kickoff prompt, `Set Up Dedicated Verification…`, the `change_verification_mode` sanctioned writer, the UAT fixture workspace; migrator-CLI cp1252 fix). Local operator UAT passed 2026-06-12; both published 2026-06-12 (tags `vsix-v0.30.0` run 27417089796, `v0.17.0` run 27417089861).
...
| [ai_router/changelog.py](../ai_router/changelog.py) | Partitioned changelogs (`python -m ai_router.changelog`, Set 122 S4). `CHANGELOG.md` was a file every session appended to at the same offset, so two concurrent session sets were guaranteed a merge conflict on it. Sessions now write one fragment per contribution under `changelog.d/` — a new file, which git merges cleanly — and `render` concatenates preamble + fragments + released history on demand; `fold` writes the view back and clears the fragments at release time (operator only). A fragment stores the **verbatim slice** of the pending region, so the concatenation equals the pre-partition document by construction, and `changelog.d/.baseline.json` freezes that digest, each fragment's digest, and the fragment ORDER. `check` re-renders from the baseline set alone and fails on a reorder, a drop, or an edit; `restamp` covers a deliberate correction to frozen prose and refuses if any fragment moved. Canonical: [docs/partitioned-append-files.md](partitioned-append-files.md). |
...
| [ai_router/session_log.py](../ai_router/session_log.py) | The sanctioned writer of `activity-log.json`, and the home of the **step-status vocabulary** (Set 120 S1). `SessionLog.log_step()` / `append_entry()` fail closed on any status outside `complete` / `in-progress` / `pending` / `blocked` — readers stay lenient about history, the writer does not. The vocabulary is the *intersection* of what was measured on disk and what the readers can name, which is why `skipped` is refused (no box, not terminal in either the CLI or the Explorer). `require_step_status()` is the chokepoint every activity-log writer calls, including the four that do their own read-modify-write (`contract_gate`, `path_aware_critique`, `dual_surface_verify`, `suggestion_disposition`). Also offers `save_session_review()`, `save_issue_log()`, and `get_next_session_number()`, and it may create `session-reviews/` and `issue-logs/` on first use; the current orchestrator workflow does not require those directories. |
| [ai_router/step_status_drift.py](../ai_router/step_status_drift.py) | The step-status drift inventory and the scoped history migration (`python -m ai_router.step_status_drift`, Set 120 S2). Set 120 S1 stopped *new* drift; this measures and repairs what was already on disk. Default mode inventories every `activity-log.json` per file and per token, sorted into the two populations the operator's ruling treats oppositely: the **lossless synonyms** (`completed`, `done`) which are normalised to `complete`, and the **semantically loaded** entries (absent, prose, JSON, `skipped`, `complete-with-known-failures`) which are preserved intact because normalising them would launder meaning. `--check-premise` falsifies the ruling's premise on three signals (owning session never completed / step re-logged non-terminal / description asserts non-completion) and exits 2 on any unadjudicated flag; `--migrate` plans and `--migrate --in-place` applies, idempotently. **The rewrite is byte-surgical**: it replaces raw-text spans and asserts nothing outside them moved, because the logs carry mixed CRLF/trailing-newline conventions and one `ensure_ascii=False` file that a re-serialize would silently rewrite. A file whose raw scan disagrees with its parse is refused, never rewritten blind. |
...
| [ai_router/close_preflight.py](../ai_router/close_preflight.py) | The close preflight (`python -m ai_router.close_preflight`, Set 119 S2). Reports every close-out obligation in one pass — met and unmet, blocking and advisory — at any point in a session, with **no side effects and no routed call**. It calls the same predicates `close_session` calls rather than carrying its own, and reads blocking-ness from `gate_checks.is_blocking_check`, so it can never refuse something the close allows: it **reports; only the existing gates refuse**. Its one addition is a cost warning — "would the verification backstop fire?", answered via `close_backstop.decide_backstop` using reads alone, because that check is 79 of 214 recorded check-failures and each firing spends a routed call at close time. `--replay-history` measures the tool's own reach over every recorded `closeout_failed` event. |
...
| [tools/dabbler-ai-orchestration/src/providers/](../tools/dabbler-ai-orchestration/src/providers/) | TreeDataProvider implementations: `SessionSetsProvider` (the activity-bar inventory), `ProviderQueuesProvider`, `ProviderHeartbeatsProvider`. Both queue/heartbeat providers render a graceful "ai_router not installed → click to install" tree-item when `python -m ai_router.queue_status` / `heartbeat_status` exit with `ModuleNotFoundError`. The Work Explorer's native tree lives here too: `WorkExplorerTreeProvider.ts` (the thin `vscode` adapter) over `workExplorerTreeModel.ts` (the pure, `vscode`-free row model — five levels: module → status bucket → session set → session → step) and `sessionStepModel.ts` (Set 114 S3 — the step rows, a TypeScript mirror of `ai_router/session_checklist.py`'s row builder whose parity is gated by `ai_router/tests/fixtures/session-step-parity.json`, asserted from both languages). |
...
| tools/dabbler-ai-orchestration/src/commands/openModulePlan.ts | `Dabbler: Open Module Plan` — the Work Explorer's `Open Plan` row action and its Command Palette mirror. **Set 123 S3** deleted `src/wizard/` (`planImport.ts` = `Dabbler: Import Project Plan`, `sessionGenPrompt.ts` = `Dabbler: Generate Session-Set Prompt` / the parallel variant) along with every webview in the extension; this opener was the one command in that directory that was never part of the wizard, so it moved here rather than dying with it (cross-provider verification caught the first cut orphaning it). The Set 021 `WizardPanel.ts` Get Started webview was retired in Set 060 S3, and Set 123 S3 retired its successor, the Work Explorer's Getting Started form — `dabbler.getStarted` (`src/commands/gettingStartedDoc.ts`) now opens the static instructions doc and nothing else, `dabbler.setupNewProject` (`gitScaffold.ts`) drives the no-prompt structure-only scaffold, and `python -m ai_router.verify_type` answers what verifies the project. |
| *(retired)* `src/dashboard/` | The `Dabbler: Show Cost Dashboard` webview, deleted in **Set 123 S3** with `src/utils/metrics.ts` and `src/utils/routerConfig.ts`, which existed only to feed it. `ai_router/router-metrics.jsonl` is still written on every routed call and is still the canonical spend record; read it with `python -m ai_router.report`, which produces the full markdown manager-report the panel only ever visualised a slice of. |
| *(retired)* `src/configEditor/` | The `Dabbler: Open Dabbler Config Editor` visual config editor, deleted in **Set 123 S3**. `ai_router/router-config.yaml`, `ai_router/budget.yaml` and the gitignored `ai_router/local-overrides.yaml` are edited as YAML now. Two of its modules were never about the editor and survive under `src/utils/`: `annotationParser.ts` and `yamlReadWrite.ts` (comment-preserving YAML round-trip via the `yaml` package), both used by `Dabbler: Scan Workspace for @dabbler:outsource-review Annotations`. |
...
| [tools/dabbler-ai-orchestration/CHANGELOG.md](../tools/dabbler-ai-orchestration/CHANGELOG.md) | Per-version release notes for **released** versions. Since Set 122 S4 it is no longer an append target: unreleased entries live one-per-contribution in `tools/dabbler-ai-orchestration/changelog.d/`, and the whole document is produced by `python -m ai_router.changelog render --target extension`. See [docs/partitioned-append-files.md](partitioned-append-files.md). |
| [tools/dabbler-ai-orchestration/changelog.d/](../tools/dabbler-ai-orchestration/changelog.d/) | Pending changelog fragments, one file per session contribution, so concurrent session sets cannot collide on the shared changelog. Folded into a version section by the operator at release time. |
...
| [docs/adoption-bootstrap.md](adoption-bootstrap.md) | **Retired** (Set 063): a URL-stable deprecation stub redirecting to the extension's setup command (VS Code) and the quick-start manual path (without VS Code); re-cut in Set 123 S3 when the Getting Started form it used to name was deleted. Kept at the same path because extension clients ≤ 0.31.0 fetch the raw URL at click time. |
| [docs/budget-yaml-schema.md](budget-yaml-schema.md) | Canonical contract for `ai_router/budget.yaml`: the post-migration shape (Set 063), field semantics, writers/readers -- note Set 123 S3 retired the Getting Started form that used to write the file, so it is authored as YAML now -- and the legacy-compatibility rules for pre-migration files. |
