...
suite("the session from its registration, and each step as it starts", () => {
  ...
  test("gives a session that puts no job output on the terminal a numbered heading anyway", () => {
    ...
    // ...and the session is still headed by its number, once, above its lines.
    const rows = said.split("\r\n");
    const heading = rows.findIndex((line) => line.trim() === "SESSION 133");
    assert.ok(heading > 0, said);
    assert.strictEqual(rows[heading - 1], "═".repeat(79));
    assert.ok(rows.slice(heading).some((line) => line.includes("now=steps")), said);
    terminal.dispose();
    rmrf(root);
  });
});
