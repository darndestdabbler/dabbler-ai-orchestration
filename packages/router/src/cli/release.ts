// `dabbler release` -- the release tag, made by hand.
//
// The publish phase of a driven session makes the same tag itself: a
// session ships unless its plan held it, publication is preapproved after
// the first release, and the verb asks nobody. This is the same act for a
// typed session, or for an operator finishing a publish the phase could
// not: it holds the version, refuses a dirty tree, tags and pushes.
//
...
import { pushReleaseTag, releaseVersion, tagsFor } from "../packaging.ts";
...
function usage(): string {
  return [
    ...
    "",
    "  Tags the release: one tag, one artifact, and nobody is asked. A session",
    "  that must not ship says so in its plan, before the work.",
...
async function run(argv: string[]): Promise<number> {
  ...
  const tags = tagsFor("publish", version);
  writeOut(`release: ${version} ships as, in order:\n`);
  ...
  for (const tag of tags) {
    ...
    if (why !== null) {
      writeErr(`release: refused -- ${why}\n`);
      return EXIT_REFUSED;
    }
    writeOut(`release: ${tag} is on origin\n`);
  }
  writeOut(
