...
export function planModulesMember(shape: SolutionShape): string {
  ...
  return (
    ...
    "A module reaches a sibling by project reference: in .NET a <ProjectReference> to the sibling's project, with both projects listed in the solution file at the root; in Maven a <dependency> on the sibling at ${project.version}, with both modules listed under <modules> in the parent pom.xml and built in one reactor run. A step that writes a module's first project adds it to that root file, and writes the root file where there is none.\n"
  );
}
