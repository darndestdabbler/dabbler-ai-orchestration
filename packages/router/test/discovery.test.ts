...
describe("the record a session start brings up to date", () => {
  ...
  /** The seat catalog, whose refresh is the priced one. */
  function seatCatalog(directory: string, hoursAgo: number): string {
    const path = join(directory, "copilot-catalog.lock");
    seed(directory, {
      "copilot-catalog.lock":
        `[meta]\ncli_version = "x"\nseat_id = "s"\nprobed_at = "${stamp(hoursAgo)}"\n`,
    });
    return path;
  }

  it("re-reads a stale record before the work, and touches nothing that costs", async () => {
    const directory = tempDir("refresh-");
    const recordPath = join(directory, "api-models.lock");
    writeRecord(recordPath, aged(100));
    const seatPath = seatCatalog(directory, 10_000);
    ...
    const lines = await refreshStaleRecords(config(directory), { now: NOW, get: vendor.get });
    ...
    // The priced half is the seat's probe, and it is not in this path at any
    // age: the catalog is a decade stale here and was not touched.
    assert.deepEqual(readFileSync(seatPath), seatBefore);
  });

  it("reads nothing at all against a fresh record", async () => {
    const directory = tempDir("refresh-");
    writeRecord(join(directory, "api-models.lock"), aged(1));
    seatCatalog(directory, 10_000);
    for (const name of KEYS) process.env[name] = "k";
    const vendor = new RecordingGet();

    assert.deepEqual(
      await refreshStaleRecords(config(directory), { now: NOW, get: vendor.get }),
      [],
    );
  ...
  it("leaves the record standing when a vendor cannot be reached, and says which", async () => {
    const directory = tempDir("refresh-");
    const recordPath = join(directory, "api-models.lock");
    ...
    const lines = await refreshStaleRecords(config(directory), { now: NOW, get: exploding });
