...
export function authoringNode(
  ...
  return {
    role: "authoring",
    engine,
    // Whose the authoring model is: the session's where one is in flight and
    // declared it, else the layer that named it, else nobody's.
    selectedBy: model !== null ? "session start" : modelChoice.decidedBy,
    selectedLayer: model !== null ? LAYER_SESSION : layerOfSource(modelChoice.decidedBy),
...
/**
 * What a row's layer reads while a session in flight declared the value.
 * Not one of the layers a choice is read through: it is the record, which
 * nothing chosen here reaches, and a row has to be able to say so.
 */
    ...
    decidedBy:
      inFlight !== null
        ...
        : preferred !== null
          ...
          : "installed on PATH",
    // The same fact as one closed word. A session in flight is a RECORD and
    // not a layer: the row reports it, and no choice here reaches it.
    decidedLayer: inFlight !== null ? LAYER_SESSION : layerOfSource(preferred === null ? null : choice.decidedBy),
...
function reviewingVehicleNode(config: RouterConfig, root: string): Node {
  ...
  return {
    ...
    decidedLayer: layerOfSource(reading.decidedBy),
    appliesTo: "next-session",
  ...
  try {
    ...
    const reviewingNode = (role: string): Node => {
      ...
      return {
        ...
        selectedLayer: layerOfSource(selection.decidedBy),
    ...
    return {
      transport: {
        ...
        decidedLayer: layerOfSource(transport.decidedBy),
      ...
      engines: {
        ...
        decidedLayer: layerOfSource(engineChoice.decidedBy),
