...
class TestLedgerRoundtrip:
    def test_append_and_read(self, tmp_path):
        ledger.append_round(tmp_path, 1, make_row())
        rounds = ledger.read_rounds(tmp_path, 1)
        assert len(rounds) == 1
        assert rounds[0]["verdict"] == "ISSUES_FOUND"

    def test_rounds_are_per_session(self, tmp_path):
        ledger.append_round(tmp_path, 1, make_row())
        assert ledger.read_rounds(tmp_path, 2) == []
    ...
    def test_next_round_number(self, tmp_path):
        assert ledger.next_round_number(tmp_path, 1) == 1
        ledger.append_round(tmp_path, 1, make_row())
        assert ledger.next_round_number(tmp_path, 1) == 2

    def test_missing_ledger_is_empty(self, tmp_path):
        assert ledger.read_rounds(tmp_path, 9) == []


class TestTamperRefusal:
    def _path(self, tmp_path):
        path = ledger.rounds_path(tmp_path, 1)
    ...
    def test_round_two_requires_previous_tree(self, tmp_path):
        ...
        with pytest.raises(ledger.LedgerError, match="previous_tree"):
            ledger.append_round(tmp_path, 2, row)
...
class TestDisputes:
    def test_prose_only_row_refused_on_read(self, tmp_path):
        ...
        path = ledger.disputes_path(tmp_path, 1)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({
        ...
        with pytest.raises(ledger.LedgerError):
            ledger.read_disputes(tmp_path, 1)


class TestRawOutput:
    def test_saved_bytes_unmodified(self, tmp_path):
        content = "VERIFIED\r\nline two\n"
        path = ledger.save_raw_output(tmp_path, 1, 1, content)
...
class TestCritiqueArtifacts:
    def test_invalid_record_is_refused_and_quarantined(self, tmp_path):
        ...
        with pytest.raises(ledger.LedgerError, match="quarantined"):
            ledger.write_review_run(tmp_path, 1, bad_run)
        assert not ledger.review_run_path(
            tmp_path, 1, "abcdef1").exists()
        ...
        with pytest.raises(ledger.LedgerError, match="quarantined"):
            ledger.append_worker_result(tmp_path, 1, blocked)
        assert ledger.read_worker_results(
            tmp_path, 1, "abcdef1") == []

        kept = sorted(
            ledger.quarantine_dir(tmp_path, 1).glob("*.json")
        )
        payloads = [
...
class TestAdjudicationRow:
    def test_type_tagged_row_requires_its_fields(self, tmp_path):
        ...
        ledger.append_round(tmp_path, 1, row)
        assert ledger.read_rounds(tmp_path, 1)[-1][
            "type"] == "adjudication"
