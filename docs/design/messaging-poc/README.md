...
fails closed too: a process listing that did not run, or answered something
that is not a listing, is not an empty list, and the listing has to find the
walk's own process before it may say that nothing else is left.
`--self-test-processes` shows all three: the real listing sees this walk, a
lister that is not installed fails, and one that answers something else fails.
