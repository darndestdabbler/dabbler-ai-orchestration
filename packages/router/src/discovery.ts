...
export function refreshDue(
  ...
  const blocks = Object.entries(catalog.transports);
  if (blocks.length === 0) return true;
  const today = dayOf(now);
  for (const [transport, block] of blocks) {
    if (dayOf(block.refreshed_at) !== today) return true;
    const scope =
      transport === TRANSPORT_SEAT ? seatScope(null, identity) : currentApiScope(config);
    // The seat's CLI version rides in the scope as provenance and moves on
    // its own schedule, so the check is over what identifies the machine.
    const recorded =
      transport === TRANSPORT_SEAT
        ? Object.fromEntries(
            Object.entries(block.scope).filter(([key]) => key !== "cli_version"),
          )
        : block.scope;
    if (!sameScope(recorded, scope)) return true;
  }
  return false;
}
 ...
 */
export function retiredModels(): Map<string, RetiredModel> {
  ...
  // answer anything else, so nothing in it can go stale.
  for (const entry of blockOf(readCatalog(), TRANSPORT_API)?.retired ?? []) {
 ...
 */
function knownModels(): Map<string, string[]> {
  ...
  const catalog = readCatalog();
  for (const entry of blockOf(catalog, TRANSPORT_API)?.models ?? []) add(entry.id, TRANSPORT_API);
  for (const entry of blockOf(catalog, TRANSPORT_SEAT)?.models ?? []) add(entry.id, TRANSPORT_SEAT);
  return known;
}
...
export function computeDrift(config: RouterConfig, now: number = Date.now()): Drift {
  return driftBetween(roleNames(config), knownModels(), checkFreshness(config, now));
}
