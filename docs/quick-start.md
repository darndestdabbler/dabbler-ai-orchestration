...
receive structured events, cancel — sends one prompt, prints every event
the agent produces as one JSON line on stdout as it arrives, prints the
turn's outcome as a final `{"kind":"turn"}` line, and closes:
...
are implementations of the same interface, so what you learn from one
holds for the other: tool calls arrive as `tool` and `tool-update`
events, not as screen paint; a resumed conversation replays its history
first, each event marked `replayed`; `--cancel-after` ends the turn from
the client's side and the final line says `cancelled: true` whatever
word the agent used.
...
What each agent actually answered on this machine is recorded in
`docs/acp-walkthrough.md`.
