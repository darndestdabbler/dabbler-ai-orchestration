...
the container with the rest of the suite. **One test in it skips there**, and
says so: the round trip through a real store, because the image has neither
PowerShell nor `secret-tool`. Everything else — what a listing carries, what
