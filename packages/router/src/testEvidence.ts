...
export function recordRun(
  ...
  if (stage === STAGE_FINAL_FULL) {
    ...
    // The interval the suite ran over, derived from what the caller already
    // reports: a run recorded now that took N seconds started N seconds ago.
    // A swap restored any time after that start overlapped the run.
    const ranFrom = new Date(Date.now() - duration * 1000).toISOString();
    const switched = refuseIfResolvingFromSource(root, "the run of record", { ranFrom });
