...
import { type ImpactPlan, planImpact, readCandidateRecord, writeImpactPlan } from "./impact.ts";
import {
  changedPathsBetween,
  nowIso,
  repoRelativePath,
...
${this.stopArtifacts()}`,
  ...
  private async phaseRunOfRecord(): Promise<void> {
    ...
    const plan = this.impactPlanForSession();
    if (plan !== null) {
      ...
      // One candidate per verified tree: a candidate written after the
      // latest round stands, and the phase re-entered -- after a stop, or
      // while a suite's job runs -- does not pack again, because a re-pack
      // rewrites the record and moves the tree under the suite's green run.
      ...
      const candidateStands =
        standing !== null && verifiedAt !== "" && Date.parse(standing) >= Date.parse(verifiedAt);
