...
/**
 * Every line the driver said, in order, across every driven call this file
 * makes. The lifecycle control at the bottom of the file reads it: the
 * observation point is the suite itself, and it costs nothing to keep.
 */
const spoken: string[] = [];

async function drive(sessionsDir: string, adapter: Engine, maxInvocations: number | null = null) {
  ...
  spoken.push(result.out, result.err);
  return result;
}
...
// --- the control that holds the model to the code -----------------------------

/**
 * `packages/router/lifecycle.json` is a SOURCE, not a diagram.
 *
 * A hand-maintained picture of a state machine is worse than none the moment
 * anyone trusts it, so it is held to the code the way the schemas are held by
 * `check:types`: the suite above drives whole sessions end to end, the driver
 * says every phase it moves to and every phase it stops in, and this reads
 * that back.
 *
 * Three assertions. The first two hold under a filtered run, because neither
 * says a declared edge WAS reached:
 *
 * - every edge taken is declared, which fails when a restructuring quietly
 *   adds a route;
 * - every edge taken is marked `exercised`, which fails when a test starts
 *   driving an edge nobody recorded.
 *
 * The third runs only when the whole file ran, and it is what makes
 * `exercised` a fact rather than a hope: the set of edges marked true must
 * be EXACTLY the set taken. So an edge that stops being driven fails until
 * it is un-recorded, and no declared edge is merely trusted -- each one is
 * either proven taken or proven not taken. The ones proven not taken are
 * the model's own finding about the suite, and `coverage` in the model says
 * so in those words.
 */
describe("the lifecycle model", () => {
  ...
  const model = JSON.parse(
    readFileSync(join(import.meta.dirname, "..", "lifecycle.json"), "utf8"),
  ) as { transitions: Transition[] };

  /** The phase changes and stops the driver announced, in the order it did. */
  function edgesTaken(): Array<{ from: string; to: string }> {
    const edges: Array<{ from: string; to: string }> = [];
    let current: string | null = null;
    for (const line of spoken.join("\n").split("\n")) {
      const started = /run-started session=/.exec(line);
      if (started) {
        current = "plan";
        continue;
      }
      const resumed = /run-resumed session=\S+ phase=(\S+)/.exec(line);
      if (resumed) {
        current = resumed[1];
        continue;
      }
      const moved = /\] phase phase=(\S+)/.exec(line);
      if (moved) {
        if (current !== null && current !== moved[1]) edges.push({ from: current, to: moved[1] });
        current = moved[1];
        continue;
      }
      const stopped = /STOPPED \(([a-z-]+)[^)]*\) in phase '([^']+)'/.exec(line);
      if (stopped) edges.push({ from: stopped[2], to: `STOPPED(${stopped[1]})` });
    }
    return edges;
  }

  function declaredFor(edge: { from: string; to: string }): Transition | undefined {
    return model.transitions.find(
      (row) => row.to === edge.to && (row.from === edge.from || row.from === "*"),
    );
  }

  it("declares every edge this suite's driven sessions actually take", () => {
    const taken = edgesTaken();
    // Under a filtered run nothing was driven and there is nothing to check,
    // which is the property that lets this live beside the tests it reads.
    // When the file DID drive something, an empty set means the parser broke.
    if (spoken.length > 0) expect(taken.length).toBeGreaterThan(0);
    const undeclared = [
      ...new Set(
        taken.filter((edge) => declaredFor(edge) === undefined).map((e) => `${e.from} -> ${e.to}`),
      ),
    ];
    expect(undeclared).toEqual([]);
  });

  it("marks every edge this suite takes as exercised, so the field is a fact", () => {
    const unrecorded = [
      ...new Set(
        edgesTaken()
          .filter((edge) => declaredFor(edge)?.exercised === false)
          .map((e) => `${e.from} -> ${e.to} (${declaredFor(e)?.id})`),
      ),
    ];
    expect(unrecorded).toEqual([]);
  });

  it("marks exercised exactly the edges a whole run of this file takes, and no others", () => {
    // Only on a whole run: a filtered one drives a subset, and demanding
    // equality of it would fail for the wrong reason. This is the assertion
    // that makes `exercised: false` a claim the control proves rather than
    // a boolean somebody typed.
    const taken = edgesTaken();
    if (taken.length < model.transitions.filter((row) => row.exercised).length) return;
    const reached = new Set(taken.map((edge) => declaredFor(edge)?.id));
    const claimed = model.transitions.filter((row) => row.exercised).map((row) => row.id);
    // D40 shares its (from, to) with D11, so no phase line can separate
    // them; the model says as much, and the driver's own
    // `verification-settled` line is what records it.
    const shared = new Set(["D40"]);
    expect(claimed.filter((id) => !reached.has(id) && !shared.has(id))).toEqual([]);
    for (const id of shared) {
      expect(spoken.join("\n")).toContain("verification-settled");
      expect(claimed).toContain(id);
    }
  });

  it("uses the phases and stop kinds the run schema declares, and no others", () => {
    const schema = JSON.parse(
      readFileSync(
        join(import.meta.dirname, "..", "schemas", "driver-run.schema.json"),
        "utf8",
      ),
    ) as {
    ...
    const phases = new Set(schema.properties["phase"].enum);
    // The stop kinds are a $def, because the run schema names them twice --
    // the stop in hand and the history behind it.
    const kinds = new Set(schema.$defs["stopKind"].enum ?? []);
    expect(phases.size).toBeGreaterThan(0);
    expect(kinds.size).toBeGreaterThan(0);

    const strayed: string[] = [];
    for (const row of model.transitions) {
      if (row.from !== "*" && !phases.has(row.from)) strayed.push(`${row.id} from=${row.from}`);
      // `[a-z-]`, not `\w`: two of the ten stop kinds are hyphenated, and a
      // `\w` here would quietly read `rejected-thrice` as a phase.
      const stop = /^STOPPED\(([a-z-]+)\)$/.exec(row.to);
      if (stop) {
        if (!kinds.has(stop[1])) strayed.push(`${row.id} to=${row.to}`);
      } else if (!phases.has(row.to)) {
        strayed.push(`${row.id} to=${row.to}`);
      }
    }
