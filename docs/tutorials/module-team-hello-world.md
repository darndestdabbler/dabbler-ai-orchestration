...
- **On GitHub Copilot + Azure DevOps?** There is a standalone [Copilot + Azure DevOps cut](./module-team-hello-world-copilot-ado.md) of this walkthrough for teams locked to GitHub Copilot as their only agent and hosted on Azure DevOps — same cast and toy program, re-cut for the Copilot seat and ADO branch policies + an executable `azure-pipelines.yml`. *(That doc is a **draft pending live validation** — Session 2 walks it end-to-end; see its banner.)*
...
> policy bootstrap (a **draft pending live validation** as of this writing — see its
