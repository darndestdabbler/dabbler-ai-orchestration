...
if (process.argv.includes("--self-test-processes")) {
  const attempt = (name, lister) => {
    ...
    try {
      return { name, sawThisWalk: inspectorSeesThisWalk(), failed: null };
    } catch (error) {
      return { name, sawThisWalk: null, failed: String(error.message ?? error).slice(0, 160) };
    }
  };
  const results = [
  ...
  const ok = results[0].sawThisWalk === true && results[1].failed !== null && results[2].failed !== null;
...
(async () => {
  ...
  try {
    if (!inspectorSeesThisWalk()) throw new Error("the process listing ran and did not find this walk's own process, so it cannot say what is left");
