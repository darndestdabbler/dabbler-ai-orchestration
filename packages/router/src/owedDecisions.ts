...
const STATE_OF_EVENT: Readonly<Record<string, string>> = {
...
export function foldOwed(rows: readonly Row[]): Map<string, Row> {
  ...
  for (const id of first) {
    const row = merged.get(id) as Row;
    ordered.set(id, { ...row, state: STATE_OF_EVENT[String(row["event"])] ?? null });
  }
  return ordered;
}
