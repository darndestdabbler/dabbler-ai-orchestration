...
import { LedgerError, atomicWriteJsonIndented, sessionRunDir } from "./ledger.ts";
...
export function validateDispositions(record: unknown): DriverDisposition {
  return validateAgainst<DriverDisposition>(record, DISPOSITION_SCHEMA, "driver disposition");
}
...
export function readDispositions(
  ...
  return readArtifact(dispositionsPath(repoRoot, sessionNumber), validateDispositions);
}
...
export function writeDispositions(
  ...
  const valid = validateDispositions(record);
