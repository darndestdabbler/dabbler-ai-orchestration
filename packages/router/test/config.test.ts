...
describe("the engine and a reviewer's model, read through the layers", () => {
  ...
  it("are this checkout's where it names one, and this machine's default where it does not", () => {
    ...
    try {
      ...
      // Each layer is one closed word as well as a sentence: every source a
      // reading names here has one, and nobody's choice is the default.
      assert.deepEqual(explainEngine("codex", mine).layers.map((layer) => layerOfSource(layer.source)), [
        "call",
        "checkout",
        "machine",
      ]);
      assert.equal(layerOfSource(explainRoleModel("reviewer", mine).decidedBy), "checkout");
      assert.equal(layerOfSource(explainRoleModel("reviewer", other).decidedBy), "machine");
      assert.equal(layerOfSource(explainRoleModel("auxiliary-reviewer", mine).decidedBy), "default");
