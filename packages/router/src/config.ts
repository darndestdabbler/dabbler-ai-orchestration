...
export function explainTransport(
  ...
  const candidates: ReadonlyArray<readonly [string, unknown]> = [
    ...
    [
      ...
      checkout === null ? null : (readSettings(checkout).values[SETTING_TRANSPORT] ?? null),
    ],
...
export function explainReviewingTransport(
  ...
  const named: Array<readonly [string, string | null]> = [
    [
      SETTING_REVIEWER_TRANSPORT,
      checkout === null ? null : (readSettings(checkout).values[SETTING_REVIEWER_TRANSPORT] ?? null),
    ],
    [
      `preferences.json ${SETTING_REVIEWER_TRANSPORT.slice("dabbler.".length)}`,
      personalTransport("reviewer_transport"),
    ],
