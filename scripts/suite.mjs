...
/**
 * Where the volumes record WHICH install they hold. See `ensureVolumes`: the
 * question is never "is something installed" but "is what is installed the
 * install this lockfile asks for".
 */
const STAMP = "/repo/node_modules/.dabbler-install";
...
/**
 * The image, built from the `Containerfile` beside `dabbler.yaml` -- and
 * rebuilt when that file changes.
 *
 * The tag is not the identity. An image that merely EXISTS under this name
 * may have been built from an older Containerfile, and this repository has
 * already paid for that reading once: `git` and `procps` went into the image
 * because tests failed without them, and an image cached by tag alone would
 * have gone on failing those tests while the Containerfile said they were
 * fixed. So the build stamps the file's digest on the image as a label and
 * this compares it, which makes the Containerfile the thing that decides.
 */
function ensureImage() {
  const want = createHash("sha256")
    .update(readFileSync(join(ROOT, "Containerfile")))
    .digest("hex");
  const stamped = podman([
    "image",
    "inspect",
    IMAGE,
    "--format",
  ...
  if (stamped.status === 0 && String(stamped.stdout).trim() === want) return;

  note(
    stamped.status === 0
      ? `Containerfile has changed since ${IMAGE} was built; rebuilding`
      : `building ${IMAGE} from Containerfile (first run only)`,
  );
  const built = podman(
    [
      "build",
...
/**
 * The dependency volumes, holding the install THIS lockfile asks for.
 *
 * A volume with nothing in it is populated rather than run against: `node
 * --test` over a tree with no `node_modules` does not fail loudly, it reports
 * the tests it could load, which is a suite with nothing in it reading as a
 * suite that passed.
 *
 * But presence is not the question, currency is. The volumes outlive the
 * checkout by design -- that is what makes them worth having -- so a branch
 * that adds or moves a dependency leaves them holding the previous install,
 * and a run against those is a green suite that proved the wrong tree. So the
 * volumes carry a stamp of the `package-lock.json` they were built from, and
 * a lockfile that does not match it is a reinstall. The stamp is written only
 * after `npm ci` succeeds, so an interrupted install is not mistaken for a
 * complete one.
 *
 * `--ignore-scripts` is the other point of care: `npm ci` would otherwise run
 * the router's `prepare`, which builds `packages/router/dist` -- and `dist` is
 * on the bind mount, so the container would overwrite the operator's own build
...
function ensureVolumes() {
  ...
  // The lockfile's own digest, compared with the one the volumes were built
  // from. Not directory emptiness: this is one workspace install across three
  // volumes and npm hoists the router's dependencies to the root, so
  ...
  // run, forever. And not mere presence either: that answers "something is
  // installed" when the question is "is it this lockfile's install".
  const populate =
    `want=$(sha256sum /repo/package-lock.json | cut -d' ' -f1); ` +
    `have=$(cat ${STAMP} 2>/dev/null || true); ` +
    'if [ "$want" != "$have" ]; then ' +
    'if [ -n "$have" ]; then ' +
    'echo "suite: package-lock.json has moved since these volumes were built; npm ci into them" >&2; ' +
    "else " +
    'echo "suite: the dependency volumes carry no install for this lockfile; npm ci into them" >&2; ' +
    "fi; " +
    // The stamp follows a successful install and never precedes one: written
    // first, an interrupted `npm ci` would leave a half-populated volume
    // claiming to be this lockfile's, and every run after it would believe
    // the claim.
    `npm ci --ignore-scripts && printf %s "$want" > ${STAMP}; ` +
    "fi";
  const result = podman(
