...
export type DriverRun = {
  ...
  waiting?: {
    ...
    /**
     * Whether a waiter has READ what the author owes an answer for. Present only where the owner is the author, because it is the only wait it can be asked about: `session wait` stamps a beacon of its own while it waits and again as it hands an instruction over, and a stamp later than this wait began is the AI having received it. Delivery and not presence -- a waiter prints what it finds and exits, so 'listening right now' is false for every step an AI is busy answering. Absent means the question was not asked, never that the answer was no.
     */
    waiter?: boolean;
  } | null;
  /**
