 ...
 *
 * `replace` is given a string rather than a pattern on purpose -- it then
 * substitutes the first occurrence and treats `$` in the replacement as
 * literal text, which is what Python's `str.replace` does and what a diff of
 * two routers' prompts has to agree on.
 */
export function buildVerificationPrompt(
  ...
  return body
    .replace("{original_task}", originalTask || "(not provided)")
