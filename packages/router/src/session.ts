 ...
 * What is refused is a value that CONTRADICTS the record.
 */
function identityClash(
  ...
  const on = block as Record<string, unknown>;
  const asked: ReadonlyArray<readonly [string, string | null | undefined]> = [
  ...
  for (const [field, value] of asked) {
    ...
    const have = typeof on[field] === "string" ? (on[field] as string).trim() : "";
...
export function start(sessionsDir: string, options: StartOptions): number {
  ...
  try {
    ...
    if (current !== null && requested === current && normalized !== null) {
      const clash = identityClash(sessionRecord(normalized, current), options);
      if (clash !== null) {
        writeErr(
