...
`session hold-release --reason proof` from the engine's shell: exit 3, and the
same sentence again -- "`session hold-release` is a person's verb, never the
engine's: it decides after the work that a session its plan declared
releasable publishes nothing, and that judgement is not the engine's to make.
Report the step blocked and say why; a person holds a release from their own
terminal." (Replayed on the bundle rebuilt after verification round 1, which
found this verb had a sentence of its own.)
...
## What section C proved that it was not meant to

The "person" in section C was this session's own author: an AI, in an AI's
tool shell, with the five markers stripped by `env -u`. It cancelled a
session in flight, and the router took it for a person. That is the hole
verification then named three rounds running -- an engine that carries no
known marker reads as a person, because a list asks what is absent -- and the
proof had walked straight through it.

The verbs that are a person's now ask what is THERE: a click in the editor,
or an interactive terminal with no engine's marker in it. Replayed on the
bundle rebuilt after round 3, the session restored to in flight, the same
stripped shell:

| no marker, no terminal | result |
| --- | --- |
| `session cancel --force --reason replay` | exit 3, refused |
| `session close --force` | exit 3, refused |
| `session hold-release --reason replay` | exit 3, refused |

and the ledger still read `1:in-progress`. Section C's loop behaviour stands
as recorded -- it is the loop's, and does not depend on who cancelled -- but
its cancel could no longer be typed by whoever typed it then. A person's
half of this cannot be replayed from an AI's shell at all, which is the
point; it is held by `session.test.ts` (`personIsPresent`) and by
`cli.test.ts`, where the person is the extension's click.
...
- **Codex.** Not installed on this machine, so its variables are unmeasured.
  It is refused wherever the extension opens its terminal or the framework
  starts it, by `DABBLER_ENGINE_TERMINAL` (row four above; `engines.test.ts`
  reads the marker out of a spawned engine's own environment), and
  everywhere else by having no terminal, as the replay above shows for a
  shell with no marker at all. `docs/design/engine-environment-markers.md`
  says what that is and is not.
- **A waiting mailbox loop.** After verification round 1 asked for it,
  `walk-session.test.ts` also cancels a session while the real mailbox engine
  waits for an answer nobody writes: the loop ends on the driver's own read
  of the ledger, with the same `done`. The exact window between the land's
  readiness judgment and its `git add` has no test of its own: it is read by
  the same `refuseIfEnded` the phase boundary is, and reaching it
  deterministically would take a seam in the land that exists only for a
  test.
