      ...
      options.push({
        ...
        consequence:
          (amendment.relaxes_a_gate
            ...
            : "It relaxes no gate. ") +
          `${amendment.reason} Nothing is amended until you choose this; the ` +
          "framework applies no adviser's proposal on its own authority.",
      });
    }
    try {
...
${this.stopArtifacts()}`,
  ...
  private stopArtifacts(): string {
    ...
    const lines: string[] = [];
    if (artifacts.step !== null) lines.push(`step: ${artifacts.step.id}`);
    if (artifacts.reasons.length > 0) {
      lines.push("the refusals, as written:", ...artifacts.reasons.map((row) => `  ${row}`));
    }
    const dir = `${RUNS_DIRNAME}/s${this.sessionNumber}/${DRIVER_DIRNAME}`;
    lines.push(`the rest is under ${dir}/: run.json, instruction.json and the transcripts.`);
    return lines.join("\n");
  }
  ...
  private async loop(): Promise<number> {
    try {
      ...
      // deadlock, only unattended, and only once per impasse -- and the
      // ladder ends at the human whatever it finds.
      const advice = deadlock && !this.pull ? await this.climbLadder(entry) : null;
      if (deadlock && !this.pull) this.save();
      this.raiseStopDecision(error.kind, reason, advice);
      writeErr(
