...
describe("dabbler session next", { timeout: 120_000 }, () => {
  ...
  it("returns from every call that starts a job rather than waiting for it", async () => {
    const { repo, sessionsDir } = drivenRepo();
    // A suite that takes four seconds, so "did not wait for it" is a
    // measurement rather than a hope.
    writeFileSync(
      join(repo, "tests", "run.mjs"),
      "import { readFileSync } from 'node:fs';\n" +
        "const red = readFileSync('src/widget.py', 'utf8').includes('broken');\n" +
        "setTimeout(() => process.exit(red ? 1 : 0), 4000);\n",
      "utf8",
    );
    git(repo, "add", "-A");
    ...
    // The suite runs twice at four seconds each -- once as the affected
    // tests and once as the run of record -- and the close is behind a job
    // too. No single call sat on any of them.
    expect(Math.max(...elapsed)).toBeLessThan(2500);
    expect(readRecords(repo).map((row) => `${row.stage}:${row.outcome}`)).toEqual([
      "preverify-targeted:passed",
      "final-full:passed",
    ]);
