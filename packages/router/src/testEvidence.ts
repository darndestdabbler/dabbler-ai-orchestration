...
function wholeRunsAfterTargeted(
  ...
  for (const row of records) {
    ...
    if (row.stage === STAGE_FINAL_TARGETED) {
      // A targeted run of record that selected nothing means the session
      // touched none of the suite: it owes no whole run.
      if (row.outcome === OUTCOME_NONE_SELECTED) after.delete(row.suite);
      else after.set(row.suite, null);
