...
/**
 * The refusal an engine meets at a verb that ends a session by force. One
 * sentence for every such verb: the AI is told the same thing wherever it
 * reaches for one.
 */
export function forceIsAPersonsVerb(verb: string, does: string, how: string): string {
  return (
    `refused -- \`session ${verb} --force\` is a person's verb, never the engine's: it ` +
...
export function holdRelease(sessionsDir: string, options: HoldReleaseCliOptions): number {
  ...
  if (options.engine === true) {
    writeErr(
      "hold-release: refused -- whether a session releases is decided by its plan before the " +
        "work, and holding one afterwards is a person's verb, never the engine's. Report the " +
        "step blocked and say why the release should not happen; a person holds it from their " +
        "own terminal.\n",
    );
    return EXIT_BOUNDARY;
  }
