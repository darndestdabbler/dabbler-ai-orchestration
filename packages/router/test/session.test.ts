...
describe("cancelling and restoring through the verb", () => {
  it("records an engine's forced cancel as a decision a person owns, and a person's as nothing", async () => {
    ...
    try {
      ...
      await run(() => restore(state.sessionsDir, 1, { reason: "again" }));
      const engine = await run(() => cancel(state.sessionsDir, 1, { reason: "wrong window", force: true, engine: true }));
      assert.equal(engine.code, EXIT_OK);
      const raised = readOwed(state.repo).find((row) => row["id"] === "forced-cancel/s1");
      assert.ok(raised, "the forced cancel is on the record as a decision");
      assert.equal(raised["class"], "accountability-signoff");
      assert.match(String(raised["question"]), /cancelled with --force by the engine \(wrong window\)/);
    } finally {
      state.restore();
    }
  });
